...
| 注释 | `//` 行注释、`/* */` 块注释 |
| 内建 | `print(...)`、`len(x)` |
| 执行 | **自包含直接编译运行**：lexer → parser → AST → 解释执行，零外部工具链依赖 |

## 实现阶段（暂缓）

Tenet 当前聚焦**设计**：语法、语义与管线已在 [grammar.md](./grammar.md) 固化，
设计取舍记录在 [design-notes.md](./design-notes.md)。
可运行的完整实现（Rust / Python / C++ 三端，语义一致、行为逐字节相同）
曾在本仓库中验证过 v1 设计的可行性，现阶段已移出，待 v2 设计定稿后重建。

> Tenet 的编译器**直接编译运行**：`tenet run file.tenet` 一条命令完成
> 词法 → 语法 → 解释执行，产物不依赖任何外部语言或工具链。
> （v1 曾支持"翻译成 Go 源码"的教学演示，已从 v2 设计中移除。）
...
> 管线到此闭环：**词法 → 语法 → 解释执行**，编译器自包含、直接编译运行。
> 未来的「编译」能力（字节码 + VM）见 [演进方向](#后续演进方向)。
...
- **字节码编译 + VM**：`tenet compile` → `.tbc` 字节码 + 虚拟机执行（吸收 CPython/JVM 模式），自包含的"编译"形态，性能提升一个数量级
