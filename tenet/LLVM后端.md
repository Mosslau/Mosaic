...
**clang 本身就是"把 LLVM 库链接进去的驱动壳"**——clang 方式等于用
...
| 维度 | 链接 LLVM 库（rustc 方式） | clang 驱动（外部进程） |
|------|---------------------------|------------------------|
...
| 能力上限 | JIT / LTO / 自定义 pass / 进程内优化 | 受限于外部驱动暴露的能力 |

rustc 走链接库路线，是因为它需要深度控制（LTO、增量编译、codegen 并行、
JIT、自定义 pass）——这是产品化阶段的工程投入，不是架构必需。
教学起步用 clang 驱动更轻（`tenet ir` 直接看 .ll 文本，IR 即文档）；
本仓库的 compiler-cpp 证明了切换到链接库只需重写后端集成，前端一行不改。
