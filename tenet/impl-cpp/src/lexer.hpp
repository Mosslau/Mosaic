...
// 与 tenet-rs/src/lexer.rs、tenet-py/tenet/lexer.py 语义一致。
#pragma once
...
private:
    ...
    Token next_token() {
        ...
        switch (c) {
            case '=':
                if (n == '=') { advance(); advance(); return make(Kind::Eq); }
                break;
            case '!':
                if (n == '=') { advance(); advance(); return make(Kind::Neq); }
                break;
            case '<':
                if (n == '=') { advance(); advance(); return make(Kind::Lte); }
                break;
            case '>':
                if (n == '=') { advance(); advance(); return make(Kind::Gte); }
                break;
            case '&':
                if (n == '&') { advance(); advance(); return make(Kind::And); }
                break;
            case '|':
                if (n == '|') { advance(); advance(); return make(Kind::Or); }
                break;
            case '-':
                if (n == '>') { advance(); advance(); return make(Kind::Arrow); }
                break;
            default:
                break;
        }
        ...
        while (true) {
            ...
            if (c == '\\') {
                ...
                switch (esc) {
                    ...
                    case '\\': value.push_back('\\'); break;
                    default:
                        throw TenetError::at("未知的转义序列 `\\" + std::string(1, esc) + "`",
                                              line, col);
                }
            } else {
                value.push_back(c);
    ...
    Token lex_single(char c, int line, int col) {
        ...
        switch (c) {
            ...
            default:
                throw TenetError::at("无法识别的字符 `" + std::string(1, c) + "`", line, col);
        }
        Token t;
        t.kind = kind;
        t.pos = Position(line, col);
