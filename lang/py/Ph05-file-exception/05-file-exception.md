...
练习：**读配置文件**（ini 格式手动解析）、**解析 CSV**（CAN 日志统计 CAN ID 出现次数）、**读取 JSON**（车辆配置 round-trip）、**日志分析**（正则提取错误行统计故障频率）、**批量重命名**（扩展名批量修改 + dry-run）。项目：**日志分析工具**（按部件+级别统计输出 CSV）、**文件批处理工具**（按扩展名分类移动/重命名，dry-run 预览）。
