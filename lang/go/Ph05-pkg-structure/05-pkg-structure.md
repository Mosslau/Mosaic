...
验收标准：能使用 `go mod init/tidy` 管理依赖；能解释 `go.mod` 与 `go.sum` 的区别；能使用 `internal` 包并理解编译器强制机制；能组织 `cmd/internal/pkg` 标准布局；能创建 `go.work` 管理多模块本地开发。

练习：拆分单文件程序（Ph03 struct 练习拆为 cmd+internal）、Todo CLI 标准布局、配置加载模块（internal/config 读环境变量/JSON）、多模块 workspace（lib+service-a+service-b）。
