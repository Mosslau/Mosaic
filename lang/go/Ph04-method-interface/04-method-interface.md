...
验收标准：能区分值/指针接收者场景、定义小而清晰的接口并写出多实现、解释隐式实现工程优势、通过接口 mock 依赖、正确使用 ok 断言和 type switch、识别 nil 接口陷阱。

练习：**Sensor 接口**（CANSensor + UARTSensor）、**Storage 接口**（可替换存储层 Todo 服务）、**Logger 接口**（ConsoleLogger + NilLogger）、**nil 接口实验**（验证 `s==nil` 行为）。
