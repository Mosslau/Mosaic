...
> 📚 **简称约定**：《接入层设计》= 《docs/接入层与车端接入网关设计-v1.md》｜《GB32960 映射》= 《docs/GB32960-二进制协议与字段映射-v1.md》（均在本目录 docs/ 下）。
...
> 见《docs/接入层与车端接入网关设计-v1.md》§4.2（唯一源）：`ov/{vin}/status`(QoS0) · `battery`(0/1) · `fault`(**1**) · `bin`(0)。
...
ingest/
├── README.md                    ← 本文件(接入层速览)
├── docs/                        ← 层文档(设计权威 + 协议规格 + 流程说明 + **验收示例集**)
│   ├── 接入层与车端接入网关设计-v1.md   ← 设计权威(契约/可靠性/安全/演进/成熟度)
│   ├── GB32960-二进制协议与字段映射-v1.md ← L1 规格(对固件)
│   ├── 接入层端到端流程说明-v1.md       ← 流程视角综述
│   └── 接入层示例集-v1.md               ← ★ 可复制可判定的验收示例(命令→期望输出→判定)
...
| codec 可观测 | ✅ `/metrics`(消费/解码/DLQ/lag/flush) + `/health`（`:18090`，2026-09-18 补齐） |
