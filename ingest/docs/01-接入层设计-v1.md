...
**执行环境（2026-09-20 如实记录）**：这条自检**以本机为准**。GitHub runner 上 EMQX 的 8883 TLS 监听器
存在环境级问题——宿主与容器网内两种路径下 TLS 握手均成功（`openssl … Verify return code: 0`），但任何
MQTT 客户端发出 CONNECT 后即被断（`unexpected eof while reading`），且 EMQX 侧无任何连接记录；
同环境的**明文 1883 完全正常**（CONNACK 0 → PUBACK 0），故证书链/一车一密/ACL 均被排除，已排除端口映射、
IPv6、协议版本、就绪时序。**结论：CI 的 `security-e2e` 作业为信息性（`continue-on-error`），
不阻塞合并；公开 8883 前的放行判据是"本机八项全过"**，该判据的证据与复现入口见 `.github/workflows/ci.yml`
的 "TLS 分层诊断" 步骤。
