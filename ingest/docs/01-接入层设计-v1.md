...
**codec 可观测（2026-09-18 补齐）**：`codec_consumed_total` · `codec_decoded_total{type}` · `codec_dlq_total{stage}` · `codec_consumer_lag` · `codec_flush_duration_seconds` · `codec_flush_batch_size`，端点 `:18090/metrics` + `/health` + `/debug/pprof/*`（探针不探测 Kafka：无状态服务的依赖抖动交给 lag 指标与重启策略）；Prometheus 已加 `job_name: device-codec` 抓取，Grafana 面板 `device-codec 编解码服务` 与网关面板同形态（provisioning 即代码）。
