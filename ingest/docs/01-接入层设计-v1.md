...
**缺口：没有端到端 SLI（2026-09-18 复评记录，计划第 2 阶段补）**：

现有延迟指标都是**分段**的，两段之间没有桥：

| 已有 | 语义 | 缺什么 |
|---|---|---|
| `gateway_request_duration_seconds` | 网关受理（含 Kafka 落盘确认，p99≈98ms，§7.3.1） | —— |
| `codec_flush_duration_seconds` | codec 微批写出耗时 | —— |
| `codec_consumer_lag` | 积压条数 | **不是时间**，无法回答"设备发出到解码完成过了多久" |

具体缺失：① **设备 `ts` → codec 解码完成**的端到端延迟；② **设备 `ts` → 网关受理**的上行链路延迟（含 EMQX 转发）。
后果：全链路慢在哪一段只能靠分段指标推断，无法直接测量；且 §7.3.1 的 p99 只覆盖"网关受理"这一段。

**落地设计（建议，成本低）**：数据模型里已经有 `ts`（设备侧时间，秒级），codec 解码时就拿到了 ——
在 codec 增加一个直方图 `codec_device_to_decode_seconds = now - report.ts`（按 type 分标签），
即可零新增链路改动地得到端到端 SLI；同理网关侧可加 `ts → 受理` 的上行延迟。
> 已知干扰项：模拟器的 `ts` 是**秒级**、且 `interval` 默认 5s，而 Prometheus 抓取间隔 5s ——
> 秒级粒度会被抓取频率与设备上报周期掩盖。要得到有意义的 SLI 数字，压测时应用**毫秒级 ts**
> 或显著提高上报频率（该结论来自本次评审，尚未落地）。
...
1. `docker compose up -d` **六容器**全部 healthy（`docker compose ps` 看 health，别只看 Up）
...
| 可观测 | 网关 Prometheus 指标（含二进制通道路径标签）+ pprof + 三处对账公式；**codec 指标已补齐**（消费/解码/DLQ/lag/flush, §9）；Grafana 面板已 provisioning；**8 条告警规则已接入**（2026-09-18）；**仍缺端到端 SLI**（设备 ts→解码完成的时延，且分段延迟之间没有桥, §9） | 🟡 分段齐全，**端到端缺**（第 2 阶段） |
