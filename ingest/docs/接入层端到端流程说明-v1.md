...
| 时间字段 | 6B `yy MM dd HH mm ss`，**GMT+8 明文**（国标口径，§8-③） |
...
| Topic | QoS | 理由 |
|---|---|---|
| `ov/{vin}/status` | 0 | 周期数据可丢，网络开销最小 |
| `ov/{vin}/battery` | 0/1 | 视电池安全监控等级 |
| `ov/{vin}/fault` | **1** | 事件必达；重复由下游 `(vin, ts)` 幂等吸收 |
| `ov/{vin}/bin` | 0 | 二进制帧（内含 0x07 时业务侧按事件对待） |

### 2.3 频率与补发策略（国标口径，映射文档 §3.1）
...
| VIN | 去尾部 0x00 填充（§8-①） | —— |
| 时间 | 6B GMT+8 → Unix 秒（§8-③） | 不足 6B → 整帧 DLQ |

### 4.4 版本路由与版本纪律

```
读 proto_ver → 路由表 → v1 解码器
                     └ 未知 → DLQ（不猜）
自定义单元读统一头 [类型][版本][长度] → 版本 v1 解码；未知版本 → 该单元 DLQ
```
