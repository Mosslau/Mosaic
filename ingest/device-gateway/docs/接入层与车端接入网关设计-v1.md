...
| 协议终结与**包头轻解析**（帧同步/VIN/版本/命令字/CRC；二进制全解析在 parser 服务，期②） | 直接写 ClickHouse/MySQL（那是 Kafka 消费者的事） |
...
> L2 标准契约 = 本节的 `VehicleReport`，是 parser 服务的解析产物。网关只解包头，全解析归 parser 服务（期②）。
...
| 期② | **二进制通道 + parser 服务**（GB/T 32960 框架，网关解包头 → `ov.raw.binary.v1` → parser → `vehicle-report-raw`，见映射文档；topic 家族拆分由此从"触发式"变为**必做**）；离线通道 `file-receiver`（文件/多模态 → MinIO）；鉴权接车辆档案服务（**模型需同时覆盖上行与下行**）；DLQ 死信队列（首个用途：解析失败/未知协议版本）；gRPC 内部通道；**限流器 Redis 化**（多副本前提，军规②）；**下行指令通道占位**（§11.1）；**topic 命名规范落地**（军规③）；EMQX 集群化路径文档化（军规④） |
...
| v1.1 增补⑤ | 2026-09-16 | **车端线协议决策：二进制，参照 GB/T 32960 框架**。契约分层落地（§4：L1 线协议/L2 标准契约）；§1.3 修正为"网关只解包头，全解析归 parser 服务"；期②范围新增二进制通道 + parser 服务（§11），topic 家族拆分由触发式变为必做；`schema_version` 启用为线协议版本号。协商底稿：《GB32960-二进制协议与字段映射-v1.md》 |
