...
> Prometheus 已入 compose（`ov-prometheus`，`localhost:9090`，5s 抓取网关 `host.docker.internal:18080/metrics`，配置见 `deploy/prometheus/prometheus.yml`）；Grafana 已 provision Prometheus 数据源。**Grafana 面板待建（期①收尾）**，建好后面板 JSON 落 `deploy/grafana/provisioning` 固化。
...
| 可观测 | Prometheus 4 指标 + pprof + 三处对账公式；Prometheus 采集链路已接通 | 🟢 采集就绪，Grafana 面板期①末建 |
