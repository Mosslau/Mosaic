...
> L1 线协议（二进制帧）由《GB32960-二进制协议与字段映射-v1.md》定义（已定稿 v1.2，§8 对齐结论 9/9）；
> L2 标准契约 = 本节的 `VehicleReport`，是 device-codec 服务的解析产物。网关只解包头，全解析归 device-codec 服务（第 2 阶段）。
...
```json
{
  ...
  "data": {                        // 字段可选, 有范围校验
    ...
    "cell_voltages": [3.276],      // battery_status 可选, 单体电压 V 元素∈[0,5]（0x08 载体, 映射文档 §5.1）
    "probe_temps": [31.0],         // battery_status 可选, 探针温度 ℃ 元素∈[-40,150]（0x09 载体, 映射文档 §5.1）
    "remain_charge_min": 45,       // charging 可选（0x80 载体, 映射文档 §5.2）: 另含 charge_power/charge_kwh/pile_id/station_id/slot_no
    "ride_state": "riding",        // work 可选（0x81 载体, 映射文档 §5.2）: 另含 ride_mode/motor_rpm/motor_torque/motor_power/throttle
  }
}
```
...
| 设备认证 | 匿名（dev）/ token 白名单 | 第 2 阶段接车辆档案服务动态校验 | **一车一密**（EMQX 内置库 / 档案服务 HTTP 认证） |
...
| 阶段 | 接入层新增 |
|---|---|
| 第 1 阶段（当前） | HTTP + MQTT 双通道、双模拟器、基础治理（鉴权/限流/校验/metrics）；**信封 v2 字段**（`schema_version`/`model`，死线：首批固件冻结前，§4.4） |
| 第 2 阶段 | **二进制通道 + device-codec 编解码服务**（GB/T 32960 框架，网关解包头 → `ov.raw.binary.v1` → codec → `vehicle-report-raw`，见映射文档；codec 双向：上行解析 + 下行编码，对接 §11.1；topic 家族拆分由此从"触发式"变为**必做**）；离线通道 `file-receiver`（文件/多模态 → MinIO）；鉴权接车辆档案服务（**模型需同时覆盖上行与下行**）；DLQ 死信队列（首个用途：解析失败/未知协议版本）；gRPC 内部通道；**限流器 Redis 化**（多副本前提，军规②）；**下行指令通道占位**（§11.1）；**topic 命名规范落地**（军规③）；EMQX 集群化路径文档化（军规④） |
...
│   ├── device-codec/             # 新增(第 2 阶段): 编解码服务, 双向
...
要点：① codec 承载双向协议知识（上行解析 + 下行编码）；② 契约搬家后"全平台宪法"从文档表述变成代码结构事实；③ 二进制承载已定 MQTT 载荷先行（映射文档 §8-⑦），TCP 自建监听为 fallback；④ 新模块配齐三件套：设计文档 + README + 单测（照 device-gateway 模板）。
...
| 企业级要求 | 当前状态 | 评级 |
|---|---|---|
| 多协议接入 | HTTP ✅ + MQTT(via EMQX) ✅，TCP 私有协议留位未做（第 2 阶段） | 🟡 双通道达标 |
...
| 安全性 | token 白名单 + webhook 密钥 + 危险配置启动告警 | 🔴 开发级（无 TLS/ACL/一车一密，第 4 阶段） |
| 可配置 | 13 项环境变量 + EMQX 声明式 HOCON | 🟡 配置齐全，热更新第 4 阶段 |
| 可观测 | Prometheus 4 指标 + pprof + 三处对账公式；采集链路与 Grafana 面板已 provisioning（2026-09-16） | 🟢 就绪，压测后视数据调优面板 |
| 持久化语义 | Kafka RequireOne（极端场景可能丢） | 🔴 开发级，生产 RequireAll（第 4 阶段） |
...
- ⏳ 工程深度仍是第 1 阶段——鉴权静态、压测无实测数字、无故障演练、安全为开发档

这符合路线图纪律：**第 1 阶段要的是"结构正确的最小链路"，不是"样样 80 分但链路没通"**。
...
```
第 1 阶段收尾: 压测数字回填 §7.3(✅2026-09-16, 达标线复核待真实车队峰值) + 故障演练 §6(✅2026-09-16, 6✅1❌→§6.1 修复项)
         + 信封 v2 字段(§4.4, ✅2026-09-16) + Grafana 面板(✅2026-09-16 provisioning 落地, 压测后调优)
第 2 阶段:     动态鉴权(覆盖上行+下行) + DLQ + K8s 横扩 + OTel 链路 + 限流器 Redis 化 + topic 命名规范 + EMQX 集群化路径文档 + 对账告警 + runbook
第 4 阶段:     配置热更新 + Schema Registry + 审计入控制面
按路径:  TLS/一车一密/ACL(公网路径开通前置, §8) —— 不再挂第 4 阶段
```
...
| v1.1 增补⑥ | 2026-09-16 | 编解码服务定名 **device-codec**（与 device-gateway 成族；双向覆盖上行解析+下行编码，避免"parser"单向歧义）；职责定义表落映射文档 §7；第 2 阶段目标代码结构冻结（§11.4）：契约迁 `contracts/` 共享模块、新增 `ingest/device-codec/`、二进制先走 MQTT 载荷路线（TCP 自建监听为备选）；架构总览 §1.1 决策注记同步 |
