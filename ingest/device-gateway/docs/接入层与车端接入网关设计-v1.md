...
| 阶段 | 接入层新增 |
|---|---|
| 第 1 阶段（当前） | HTTP + MQTT 双通道、双模拟器、基础治理（鉴权/限流/校验/metrics）；**信封 v2 字段**（`schema_version`/`model`，死线：首批固件冻结前，§4.4）；**二进制链路上行提前量（增补⑩）**：契约迁 `contracts/` → 二进制帧模拟器 → 透传 handler + EMQX 规则 → device-codec 上行 + DLQ（协议规格 v1.4 全定稿，无外部依赖，主线 Flink SQL 不受影响） |
