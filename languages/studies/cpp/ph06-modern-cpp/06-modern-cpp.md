...
**范围边界**：本阶段承接 ph05 模板与泛型；**不涉及**异常安全体系（ph07）、并发与线程（ph08）、对象生命周期、值类别与所有权深入（ph12 对象生命周期、值类别与所有权深入阶段），也不展开协程与 C++20 模块，这些留给后续阶段。
...
本文示例以 **C++17** 为基线（optional/variant/结构化绑定/CTAD 可用），示例 5（std::format/std::println）需 **C++23**（编译加 `-std=c++23`），验证工具链 Apple clang 17（g++ 兼容）。
...
> 说明：每个示例的完整可运行文件在 [`examples/`](./examples/) 目录。验证环境 Apple clang 17（g++ 兼容）：示例 1~4 编译命令统一 `g++ -Wall -Wextra -std=c++17`，示例 5 涉及 `<format>`/`<print>` 用 `-std=c++23`，完整命令见 examples/README.md。

### 示例 1：智能指针管理资源（make_unique / make_shared，RAII 替代裸 new/delete）

```cpp
// examples/ex01-smart-ptr.cpp —— 智能指针管理资源（g++ -Wall -Wextra -std=c++17）
#include <iostream>
#include <memory>
#include <string>
...
完整文件：`examples/ex01-smart-ptr.cpp`

### 示例 2：用 lambda 定义排序规则与回调（std::sort + capture）

```cpp
// examples/ex02-lambda-sort.cpp —— lambda 排序与回调（g++ -Wall -Wextra -std=c++17）
#include <algorithm>
#include <functional>
#include <iostream>
#include <string>
...
完整文件：`examples/ex02-lambda-sort.cpp`

### 示例 3：std::optional 表示查找结果 / 配置项可选值

```cpp
// examples/ex03-optional-lookup.cpp —— optional 表达查找结果（g++ -Wall -Wextra -std=c++17）
#include <iostream>
#include <optional>
#include <string>
...
完整文件：`examples/ex03-optional-lookup.cpp`

### 示例 4：std::variant 建模多种状态（设备状态 / 消息类型）

```cpp
// examples/ex04-variant-state.cpp —— variant 建模多种状态（g++ -Wall -Wextra -std=c++17）
#include <iostream>
#include <string>
#include <type_traits>
#include <variant>
...
完整文件：`examples/ex04-variant-state.cpp`

### 示例 5：std::format 类型安全格式化输出（C++20，替代 printf / iostream 拼接）

```cpp
// examples/ex05-format.cpp —— std::format 类型安全格式化（g++ -Wall -Wextra -std=c++23，GCC 13+ / Clang 17+）
#include <format>
#include <iostream>
#include <print>
#include <string>
...
本阶段练习见 [exercises/](./exercises/)（题目在 exercises/README.md，参考实现 sol-* 先别看）：用 unique_ptr 管理对象、用 optional 表示查找结果、用 std::format 重写字符串拼接、用 lambda 定义排序规则、用 variant 表示多种状态共 5 题。完成 5 题后继续。

### 阶段项目

本阶段综合项目见 [project/](./project/)：**配置管理模块**（`Config` 类用 `std::unordered_map<std::string, Value>` 存键值，`optional` 表达缺失配置项，`std::format` 输出配置清单，`variant` 表示配置值的多种类型）。建议完成练习后再动手。
