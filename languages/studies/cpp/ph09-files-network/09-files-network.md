...
- **ABI 与插件机制深入**（[ph19 ABI、动态库与插件机制阶段](../ph19-abi-dynamic-libs-plugins/19-abi-dynamic-libs-plugins.md)）：符号版本控制、visibility 精细管理、跨语言异常边界——本阶段掌握"接口要稳定 + 版本号"的纪律
- **异步 IO 与高性能网络**（ph18 性能优化与 Profiling 阶段、[ph22 存储引擎与数据库内核专项阶段](../ph22-storage-engine-db-kernel/22-storage-engine-db-kernel.md)）：epoll 事件驱动、io_uring、Boost.Asio 异步模型——本阶段用阻塞 socket + 超时把协议写对，是异步化的前提
