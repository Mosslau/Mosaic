...
### 3.5.1 数组、元组与切片：定长类型回顾与选择

ph01 学过数组 `[T; N]` 和元组的字面写法，这里补全它们在数据结构语境下的定位——**和 Vec 怎么选**：

```rust
fn main() {
    // 数组：长度是类型的一部分，栈上分配，编译期定长
    let fixed: [i32; 5] = [1, 2, 3, 4, 5];
    let buf = [0u8; 1024];                 // 重复初始化语法

    // 切片 &[T]：数组/Vec 的"视图"，不拥有数据
    let mid: &[i32] = &fixed[1..3];        // [2, 3]

    // 元组：匿名的定长异构组合
    let pair: (i32, &str) = (404, "Not Found");
    let (code, msg) = pair;                // 解构
    println!("{} {} | buf[0]={} | mid={:?}", code, msg, buf[0], mid);
}
```

| 类型 | 长度 | 位置 | 所有权 | 选择场景 |
|------|------|------|--------|---------|
| `[T; N]` | 编译期固定 | 栈 | 拥有 | 协议头、固定窗口、嵌入式 |
| `&[T]` | 运行期可知 | 借用 | 不拥有 | 函数参数——"给我一段连续数据看看" |
| `Vec<T>` | 运行期可变 | 堆 | 拥有 | 默认选择，元素数量动态增长 |

**函数签名惯例**：只读参数用 `&[T]` 而不是 `&Vec<T>`——`&Vec<T>` 会自动解引用成 `&[T]`，但前者还能接受数组切片，泛用性更强：
...
fn main() {
    let v = vec![1, 2, 3];
    let a = [4, 5, 6];
    println!("{} {}", sum(&v), sum(&a));   // Vec 和数组都能传
}
```
