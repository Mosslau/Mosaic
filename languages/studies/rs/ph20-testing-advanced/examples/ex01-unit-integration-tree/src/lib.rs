...
//!
//! 本 crate 是 **examples/ex01 的「单元测试 vs 集成测试」演示被测对象**——结构复刻自
//! ph19 阶段项目 `wal-record-parser`（见 ph19-memory-layout-zero-copy/project/wal-record-parser），
//! 格式与 ph19 主文档 3.8 的表格完全一致（字段一律小端）。看点是**测试放哪里**：
//! 单元测试（同文件的 `#[cfg(test)]` 子模块，见文件尾部的 assert_styles 与 tests）
//! 贴着被测代码测内部正确性；集成测试（`tests/` 目录，每个文件是独立 crate）
//! 只走 pub API 测契约。
//!
//! # 格式（教学简化）
//!
...
//!
//! ```rust
//! use ex01_unit_integration_tree::{records, Op, WalWriter};
//!
