//! examples/ex04：proptest 属性测试的被测对象（DUT）。
//!
//! 复刻自 ph19 阶段项目 `wal-record-parser` 的迷你 WAL record 解析器
//! （格式见 ph19 主文档 3.8，字段一律小端）。本 crate 本身不含任何 proptest
//! 代码——属性测试全部落在 `tests/`：被测对象保持「纯 std + 解析不 panic」，
//! 交给 proptest 生成任意字节/构造日志来轰炸（主文档 3.5 对应）。
//!
use std::error::Error;
use std::fmt;
