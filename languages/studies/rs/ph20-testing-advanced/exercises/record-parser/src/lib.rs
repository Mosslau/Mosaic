...
//!
//! 本 crate 是 **ph20 测试体系进阶阶段练习的共享被测对象（DUT）**——结构复刻自
//! ph19 阶段项目 `wal-record-parser`（见 ph19-memory-layout-zero-copy/project/wal-record-parser），
//! 格式与 ph19 主文档 3.8 的表格完全一致（字段一律小端）。练习只允许新增测试、
//! 不许修改解析器——这正是「bug 修复伴随回归测试」要练的姿势。
//!
//! # 格式（教学简化）
//!
//! ```text
...
//!
//! ```rust
//! use record_parser::{records, Op, WalWriter};
//!
