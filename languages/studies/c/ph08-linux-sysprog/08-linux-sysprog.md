...
**范围边界**：这个阶段只涉及 POSIX/Linux 系统接口本身——文件描述符、进程、线程、同步原语、socket 与事件模型，**不涉及 C 标准与可移植性细节、未定义行为深入和 mmap 与 Page Cache 的存储语义** — 那些是 ph09 C 标准、编译器与可移植性阶段、ph10 未定义行为 UB 与常见坑阶段和 ph13 mmap、Page Cache 与可靠文件 IO 阶段（roadmap 第 13 节）的内容；TCP 拥塞控制、TLS、HTTP 框架等网络协议栈深入属于后续阶段。
