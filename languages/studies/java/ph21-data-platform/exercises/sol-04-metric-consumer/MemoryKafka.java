// exercises/sol-04-metrics-consumer/MemoryKafka.java —— 多分区内存版 Kafka(参考实现)
// 验证环境：OpenJDK 17.0.18(Homebrew)，命令：javac -encoding UTF-8 -d /tmp/tl21-sol *.java
//
// 题目要求(roadmap §21 练习「Kafka 指标消费服务」)：
//   - 一个多分区 topic：按 SOURCE_ID 哈希决定分区(保证同g同分区、有序)；
...
public final class MemoryKafka {
    ...
    private final List<List<MetricMsg>> logs = new ArrayList<>();
    ...
    /** 生产：按 SOURCE_ID 哈希分区(同 SOURCE_ID 永远同一分区，保序)。 */
    public int produce(MetricMsg msg) {
        int partition = Math.floorMod(msg.sourceId().hashCode(), PARTITIONS);
        logs.get(partition).add(msg);
        return partition;
    }

    public List<MetricMsg> read(int partition, long fromOffset, int max) {
        List<MetricMsg> log = logs.get(partition);
