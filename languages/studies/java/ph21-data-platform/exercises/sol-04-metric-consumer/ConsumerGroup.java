// exercises/sol-04-metrics-consumer/ConsumerGroup.java —— 消费组(参考实现)
...
public final class ConsumerGroup {
    ...
    private final class Worker implements Runnable {
        ...
        @Override public void run() {
            while (running) {
                ...
                for (int p : partitions) {
                    List<MetricMsg> batch = kafka.read(p, nextOffsets[p], 8);
                    if (batch.isEmpty()) {
                        continue;
                    }
                    for (MetricMsg msg : batch) {
        ...
        private void processMsg(MetricMsg m) {
