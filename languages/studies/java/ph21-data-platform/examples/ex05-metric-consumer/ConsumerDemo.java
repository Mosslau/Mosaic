// examples/ex05-metrics-consumer/ConsumerDemo.java —— Kafka 指标消费服务演示(离线可测)
    ...
    public static void main(String[] args) {
        ...
        for (int v = 1; v <= 5; v++) {
            ...
            for (long seq = 1; seq <= 5; seq++) {
                batch.add(new MetricMessage(sourceId, seq, 40 + seq));   // 5 个数据源 × 5 帧 = 25 条
            }
        }
        kafka.produce(batch);

        MetricConsumerService consumer = new MetricConsumerService(kafka, 0, 7);
        // 阶段 1：正常消费到 log 末尾
