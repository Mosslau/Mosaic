// examples/ex07-trajectory-query/JobHistoryDemo.java —— 作业历史查询演示
// 验证环境：OpenJDK 17.0.18(Homebrew)，命令：
//   javac -encoding UTF-8 -d /tmp/tl21-cls *.java
//   java -cp /tmp/tl21-cls JobHistoryDemo
// 期望：两g各写入 10 个点；时间窗查询只回该窗口内的点；latest 是最末采样点。
import java.util.List;
import java.util.concurrent.atomic.AtomicInteger;

public final class JobHistoryDemo {
    public static void main(String[] args) {
        JobHistoryStore store = new JobHistoryStore();
        String v1 = "LSV0000001";
        String v2 = "LSV0000002";

        // 每个数据源 10 个点：1 分钟一个点（沿作业序号匀速推进，模拟连续作业流）
        long base = 1_700_000_000L;
        for (int i = 0; i < 10; i++) {
            store.append(new JobRecord(v1, base + i * 60L, 31.2304, 121.4737 + i * 0.001, 30 + i));
            store.append(new JobRecord(v2, base + i * 60L, 39.9042, 116.4074 - i * 0.001, 20 + i * 2));
        }

        // 查询：只取 3~6 分钟之间 4 个点
        List<JobRecord> window = store.queryWindow(v1, base + 3 * 60L, base + 6 * 60L);

        AtomicInteger pass = new AtomicInteger();
        check(pass, store.totalPoints() == 20, "两g共 20 个作业历史点入库");
