...
默认 `standard` 分析器适合英文；**中文整句没有空格，`standard` 会把「电动充电电」切得七零八落**，生产中文检索要装 ik 等中文分词插件（本阶段 project 的 message 用英文/拼音字段演示即可，中文分词插件安装不在本阶段展开）。查询时同样的文本也要过分析器——「查询词怎么分词」与「索引词怎么分词」必须一致，这是 match 查询与 term 查询的根本区别（3.9）。
