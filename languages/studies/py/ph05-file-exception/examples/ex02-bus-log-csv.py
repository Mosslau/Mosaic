# examples/ex02-bus-log-csv.py —— BUS 日志 CSV 解析：DictReader 按列名访问并筛选
# 来源：05-file-exception.md 第 6 章示例 2
# 验证环境：Python 3.13.12
# 运行：python3 ex02-bus-log-csv.py
# 验证状态：已验证

"""用 csv.DictReader 读取 BUS 日志 CSV，按列名访问数据并按 BUS ID 筛选消息。"""
...
def load_can_messages(path):
    """读取 BUS 日志 CSV，返回消息字典列表（每项含 ts/id/data）。"""
    try:
        with open(path, "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            return [
                {"ts": row["timestamp"], "id": row["bus_id"], "data": row["data"]}
                for row in reader
            ]
...
def main():
    """生成 BUS 日志 CSV 并统计 0x123 消息的出现次数。"""
