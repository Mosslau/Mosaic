...
class Ph15LikePipeline:
    """与 ph15 ComponentHealthPipeline（health/model.py）同形态：封装回归器 + predict_health。
...
def test_predict_rule_fallback(client: TestClient) -> None:
    ...
    body = r.json()
    assert body["health"] == 80.5 and body["grade"] == "退化" and body["model_type"] == "rule"
...
def test_metrics_endpoint(client: TestClient) -> None:
    ...
    assert r.status_code == 200
    assert "# TYPE health_requests_total counter" in r.text
    assert 'health_model_info{model_type="rule"} 1' in r.text
    assert "health_predict_seconds_count 1" in r.text
...
def test_missing_artifact_not_ready(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    with TestClient(create_app()) as c:
        ...
        assert 'health_model_info{model_type="missing"} 1' in c.get("/metrics").text
...
def test_trained_artifact_roundtrip(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    with TestClient(create_app()) as c:
        ...
        health = c.post("/predict", json=COND).json()["health"]
        rule = RulePredictor().predict_health([1500, 25, 80, 1.0])
        assert abs(health - rule) < 5.0  # 训练数据与规则同源，预测应贴近规则值


def test_ph15_pipeline_artifact_compat(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    """ph15 产物形态（ComponentHealthPipeline）可被 JoblibPredictor 直接消费。

    ph15 项目 languages/py/ph15-ai-ml/project/health/model.py 的产物是 dataclass
    ComponentHealthPipeline：暴露 predict_health(X)/predict_grade(X)，**没有** sklearn 的
    ...
    with TestClient(create_app()) as c:
        ...
        health = c.post("/predict", json=COND).json()["health"]
        rule = RulePredictor().predict_health([1500, 25, 80, 1.0])
        assert abs(health - rule) < 5.0  # 与规则同源，预测应贴近规则值
