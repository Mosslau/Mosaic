...
# 功能：设备遥测上报（单条 + 批量）、按设备/时间查询、分组统计、日志中间件、统一错误处理。
...
对应 roadmap（python.md）ph10「推荐项目」第二个「设备数据上报 API」：设备遥测上报接口
...
def create_app(database_url: str | None = None) -> FastAPI:
    ...
    def report(body: TelemetryIn, session: Session = Depends(get_session)):
        ...
        row = Telemetry(device_id=body.device_id, ts=body.ts,
    ...
    def report_batch(body: list[TelemetryIn], session: Session = Depends(get_session)):
        ...
        for item in body:
            session.add(Telemetry(device_id=item.device_id, ts=item.ts,
                                  speed=item.speed, soc=item.soc))
        session.commit()
        return {"inserted": len(body)}

    @app.get("/telemetry", response_model=list[TelemetryOut])
    def list_telemetry(device_id: str | None = None,
                       since: datetime | None = None,
                       until: datetime | None = None,
                       limit: int = Query(100, ge=1, le=1000),
                       session: Session = Depends(get_session)):
        """按设备/时间查询：可选 device_id、since/until（ISO 8601），按时间升序。"""
    ...
    def telemetry_stats(device_id: str | None = None,
                        since: datetime | None = None,
                        until: datetime | None = None,
                        session: Session = Depends(get_session)):
        """分组统计（呼应 ph09 分组统计思维）：不传 device_id 按设备分组，
        传了则只统计该设备。返回 count / avg_speed / max_speed / avg_soc。"""
        stmt = select(
            Telemetry.device_id,
        ...
        if until:
            stmt = stmt.where(Telemetry.ts <= until)
        stmt = stmt.group_by(Telemetry.device_id).order_by(Telemetry.device_id)
        return [
            {
...
def build_demo_rows() -> list[TelemetryIn]:
    """构造 3 台设备 × 24 条确定性模拟遥测（速度 55~85、SOC 88 递减），供 --demo 离线演示。"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    with TestClient(app) as client:
        ...
        print("批量上报 ->", r.status_code, r.json(), "（3 台设备 × 24 条 = 72 条）")

        r = client.post("/telemetry",
                        json={"device_id": "V999", "ts": "2024-06-01T09:00:00",
                              "speed": 250, "soc": 80})   # 速度超限 → 422
        print("非法上报（speed=250）->", r.status_code,
              r.json()["detail"][0]["loc"], r.json()["detail"][0]["type"])

        r = client.get("/telemetry?device_id=V001&limit=3")
        print("按设备查询 V001 前 3 条 ->", r.status_code, "条数:", len(r.json()),
        ...
        r = client.get("/telemetry/stats?device_id=V001")
        print("单设备统计 V001      ->", r.json())
