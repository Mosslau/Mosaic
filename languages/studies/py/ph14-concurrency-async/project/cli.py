...
用法：
    ...
    python3 cli.py                        # 默认跑一次采集（20 台设备，10% 失败率）
    python3 cli.py --devices 50 --concurrency 10 --fail-rate 0.2 --output-dir /tmp/out
"""
...
async def run_collection(args: argparse.Namespace) -> dict:
    ...
    await sim.start()
    try:
        device_ids = sim.device_ids
        print(
            f"模拟服务器就绪: {sim.base_url}（{len(device_ids)} 台设备，"
        ...
        print(f"\n采集完成: {stats.total} 台设备，耗时 {elapsed:.2f}s")
        print(
        ...
        # 失败清单打印前 5 条
        fails = [s.device_id for s in summarize(results) if not s.ok]
        if fails:
            shown = ", ".join(fails[:5]) + ("..." if len(fails) > 5 else "")
            print(f"失败设备（重试耗尽）: {shown}")
        return {
            "total": stats.total,
...
def run_demo() -> None:
    ...
    async def _demo() -> None:
        sim = TelemetrySimulator(n_devices=10, latency_ms=10, fail_rate=0.0, seed=1)
        await sim.start()
        try:
...
def main() -> None:
    ...
    parser.add_argument("--devices", type=int, default=20, help="模拟设备数（默认 20）")
    parser.add_argument("--concurrency", type=int, default=5, help="采集并发上限（默认 5）")
    parser.add_argument("--max-retries", type=int, default=3, help="单台设备最大重试次数（默认 3）")
