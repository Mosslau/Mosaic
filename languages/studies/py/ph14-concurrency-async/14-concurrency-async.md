...
- 分布式消息队列与跨机集群并发（Kafka / MQ、百万级连接架构）：roadmap 第 18 节数据平台分析 / 自动化方向阶段或团队工程实践——ph10/ph11 曾把「大规模异步与消息」预告到 ph14，本阶段明确其边界：**单机 asyncio 覆盖万级并发网络 IO，消息队列是跨机架构话题**，不在此展开
