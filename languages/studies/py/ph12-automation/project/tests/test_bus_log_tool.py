# project/tests/test_bus_log_tool.py —— BUS 日志批处理工具测试（pytest，离线）
...
def test_filter_id_frames():
    ...
    assert filtered[0].bus_id == 0x123
...
def test_analyze_generates_reports(tmp_path):
    ...
    rows = list(csv.reader((out_dir / "bus_report.csv").open(encoding="utf-8")))
    assert len(rows) == 4  # 表头 + 3 个 ID
    assert rows[0] == ["bus_id", "信号", "条数", "min", "max", "avg"]
    summary = (out_dir / "summary.txt").read_text(encoding="utf-8")
    assert "总帧数: 12 | 无效行: 1" in summary
    assert "0x123 运行速度: 6 条 [30, 60] avg 45.0" in summary
...
def test_analyze_idempotent(tmp_path):
    ...
    first_csv = (out_dir / "bus_report.csv").read_bytes()
    analyze(log_path, out_dir, None, logger)  # 再次运行：报表被确定性覆盖，无副作用
    assert (out_dir / "bus_report.csv").read_bytes() == first_csv
