#!/usr/bin/env python3
# project/bus_log_tool.py —— ph12 阶段项目：BUS 日志批处理工具
# 验证环境：Python 3.13.9（stdlib，无第三方依赖；测试用 pytest 8.4.2、门禁 ruff 0.12.0）
# 运行：python3 bus_log_tool.py --demo                     （离线自检演示）
#       python3 bus_log_tool.py --input can.log --output-dir out/ [--filter-id 0x123]
# 测试：pytest -q；门禁：ruff check . && pytest -q
# 说明：解析 candump 风格 BUS 日志（(秒.微秒) 接口 ID#负载HEX），按 ID 统计信号值
...
# 设备信号映射（简化版：首字节即信号值；完整 DBC 信号矩阵解析属 ph18 数据平台阶段）
...
def filter_id_frames(frames: list[Frame], bus_id: int) -> list[Frame]:
    """按 ID 过滤帧列表（保持原始顺序）。"""
    return [f for f in frames if f.bus_id == bus_id]
...
def stats_by_id(frames: list[Frame]) -> dict[int, IdStats]:
    ...
    for f in frames:
        values.setdefault(f.bus_id, []).append(f.data[0] if f.data else 0)
    stats: dict[int, IdStats] = {}
    for bus_id in sorted(values):
        vals = values[bus_id]
        stats[bus_id] = IdStats(
            count=len(vals),
            min=min(vals),
            max=max(vals),
...
def write_csv_report(stats: dict[int, IdStats], out: Path) -> None:
    """CSV 报表：bus_id / 信号 / 条数 / min / max / avg（确定性内容，可安全重跑）。"""
    with out.open("w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["bus_id", "信号", "条数", "min", "max", "avg"])
        for bus_id, s in stats.items():
            writer.writerow([
                f"0x{bus_id:X}", SIGNALS.get(bus_id, "未知"), s.count, s.min, s.max, s.avg,
            ])


def _format_filter(bus_id: int | None) -> str:
    return f"0x{bus_id:X}" if bus_id is not None else "无（全部 ID）"
...
def write_summary(report: Report, out: Path) -> None:
    ...
    for bus_id, s in report.stats.items():
        lines.append(
            f"  0x{bus_id:X} {SIGNALS.get(bus_id, '未知')}: "
...
def analyze(input_path: Path, output_dir: Path, filter_id: int | None,
    ...
    write_csv_report(stats, output_dir / "bus_report.csv")
    write_summary(report, output_dir / "summary.txt")
    logger.info("报表已写入 %s（bus_report.csv + summary.txt）", output_dir)
    return report
...
def generate_sample_log(path: Path) -> None:
    ...
    for bus_id, value in payloads:
        lines.append(f"({ts:.6f}) can0 {bus_id:X}#{value:02X}{'00' * 7}")
        ts += 0.2
...
def setup_logging(level: str, log_file: str | None) -> logging.Logger:
    ...
    return logging.getLogger("bus_log_tool")
...
def run_demo() -> int:
    ...
    report = analyze(
        ...
        logger=logging.getLogger("bus_log_tool"),
    )
    # 自检：数字与样本定义一致
    ...
    for bus_id, s in report.stats.items():
        print(f"  0x{bus_id:X} {SIGNALS[bus_id]}: {s.count} 条 [{s.min}, {s.max}] avg {s.avg}")
    report_csv = work / "reports" / "bus_report.csv"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="bus_log_tool",
        description="BUS 日志批处理工具：解析 candump 格式日志，按 ID 统计信号值并生成报表",
    )
    parser.add_argument("--input", help="BUS 日志文件路径（candump 格式）")
