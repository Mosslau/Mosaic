...
from bhealth.data import load_battery_data
from bhealth.evaluate import clf_baseline, evaluate_classifier, evaluate_regressor, reg_baseline
from bhealth.model import BatteryHealthPipeline, train_pipeline


def _fixture(n: int = 400) -> tuple:
    data = load_battery_data(n=n, seed=42)
...
def test_same_seed_same_metrics():
    ...
    assert np.allclose(p1.predict_soh(test.X), p2.predict_soh(test.X), atol=1e-10)
    assert np.array_equal(p1.predict_grade(test.X), p2.predict_grade(test.X))


def test_regression_beats_mean_baseline():
    """SOH 回归显著优于「预测均值」baseline（先 baseline 再复杂化的验收）。"""
    train, test = _fixture()
    ...
    assert evaluate_regressor(pipeline, test)["r2"] > 0.9, "SOH 回归 R² 应 > 0.9"
...
def test_pipeline_save_load_roundtrip(tmp_path):
    ...
    loaded = BatteryHealthPipeline.load(path)
    assert path.exists() and path.stat().st_size > 0
    assert np.allclose(loaded.predict_soh(test.X), pipeline.predict_soh(test.X), atol=1e-10)
    assert loaded.features == pipeline.features
    assert loaded.n_train == pipeline.n_train


def test_no_target_leakage_features():
    """教学点固化：特征里不允许出现 soh/grade（目标不能当特征）。"""
    train, _ = _fixture()
    assert "soh" not in train_pipeline(train).features
