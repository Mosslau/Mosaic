#!/usr/bin/env python3
"""电池健康预测命令行入口（project/cli.py）。

流程：合成电池老化数据 → train/test 划分 → SOH 回归 + 等级分类 → 指标 →
模型产物写盘（默认 /tmp/bhealth-model/model.joblib）→ 可选：加载模型做单条预测。
...
用法：
    ...
    python3 cli.py --n 600 --out /tmp/bhealth-model --seed 42
"""
...
from bhealth.data import GRADE_NAMES, load_battery_data
from bhealth.evaluate import summary_metrics
from bhealth.model import BatteryHealthPipeline, train_pipeline

DEFAULT_OUT = Path("/tmp/bhealth-model")


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="电池健康预测（SOH 回归 + 健康等级分类）")
    parser.add_argument("--n", type=int, default=1500, help="合成数据量（默认 1500）")
    parser.add_argument("--seed", type=int, default=42, help="随机种子（默认 42）")
    parser.add_argument("--test-size", type=float, default=0.25)
    parser.add_argument(
        "--out", type=Path, default=DEFAULT_OUT, help="模型产物目录（默认 /tmp/bhealth-model）"
    )
    parser.add_argument(
        "--predict",
        nargs=4,
...
def run_train(args: argparse.Namespace) -> BatteryHealthPipeline:
    """合成数据 → 训练 → 指标 → 产物落盘，返回管线。"""
    data = load_battery_data(n=args.n, seed=args.seed)
    ...
    print(
        f"训练 {metrics['n_train']} / 测试 {metrics['n_test']} 组电池；"
        f"等级分布 {metrics['grade_dist_test']}"
    )
    print(
        f"SOH 回归:   RMSE {metrics['regression']['rmse']:.3f}%  "
...
def run_predict(args: argparse.Namespace) -> None:
    ...
    if not path.exists():
        ...
        raise SystemExit(2)
    pipeline = BatteryHealthPipeline.load(path)
    ...
    print(f"预测 SOH: {soh:.1f}%  → 健康等级: {GRADE_NAMES[grade]}")
...
def run_demo(args: argparse.Namespace) -> None:
    ...
    if args.n == 1500:  # 没显式指定样本量时用小样本快速自检
        args.n = 400
    if args.out == DEFAULT_OUT:  # 产物纪律：demo 产物固定写 /tmp/bhealth-demo
        args.out = Path("/tmp/bhealth-demo")
    ...
    assert metrics["regression"]["r2"] > 0.85, "SOH 回归 R² 应 > 0.85"
    assert metrics["classification"]["accuracy"] > 0.75, "等级分类 acc 应 > 0.75"
    print(
        "\n自检通过：R² {:.3f} > 0.85、acc {:.3f} > 0.75、产物已写 /tmp/bhealth-demo".format(
