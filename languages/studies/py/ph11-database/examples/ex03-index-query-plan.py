...
# 说明：对应主文档 3.5/4.1。两万行数据按 vin 精确查询：先看全表扫描的执行计划与耗时，
...
def setup(cur: sqlite3.Cursor) -> None:
    cur.execute(
        """CREATE TABLE IF NOT EXISTS devices (
               id INTEGER PRIMARY KEY, vin TEXT, model TEXT, online INTEGER
           )"""
    )
    cur.execute("DELETE FROM devices")
    cur.executemany(
        "INSERT INTO devices (vin, model, online) VALUES (?, ?, ?)",
        [(f"VIN{i:06d}", f"EV-{i % 5}", i % 2) for i in range(N_ROWS)],
    )


def measure(cur: sqlite3.Cursor, vin: str) -> tuple[list[tuple], float]:
    """返回执行计划与一次查询耗时（毫秒）。"""
    plan = cur.execute(
        "EXPLAIN QUERY PLAN SELECT * FROM devices WHERE vin = ?", (vin,)
    ).fetchall()
...
def main() -> None:
    ...
    vin = "VIN000123"  # 目标行存在于 20_000 行数据中

    print(f"--- 无索引（{N_ROWS:,} 行全表扫描）---")
    plan, ms = measure(cur, vin)
    print("执行计划:", plan)
    print(f"耗时: {ms:.2f} ms")
