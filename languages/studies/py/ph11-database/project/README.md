...
> 对应 Roadmap（python.md）ph11「推荐项目」第一个「设备管理后端」。devices 表（device_id 唯一约束）＋ device_status 状态表（复合索引）＋ 用户 CRUD ＋ 手写迁移脚本演进结构 ＋ 缓存设备热点查询（TTL 过期 + 写库主动失效）——示例 1/3/5 的合体，也是 ph10 设备管理 API 的数据层升级。

## 需求

实现一个设备管理后端的数据层：用户 CRUD（参数化查询防注入、email 唯一约束）；设备注册（device_id 唯一约束，重复注册拒绝）；设备状态高频批量写入（`executemany`）与按设备 + 时间窗口查询（`(device_id, ts)` 复合索引）；按设备分组统计（count / avg_speed / max_speed）；设备热点查询走缓存（缓存旁路 + TTL 过期 + 写库主动失效防脏读）。表结构由**手写迁移脚本**（`schema_version`）从 v1 逐步演进到 v4——结构变更可版本化、可重放、失败回滚。数据库用 SQLite（标准库 `sqlite3`，零第三方依赖），呼应本阶段「SQL 基础比 ORM 更重要」的必会概念。
...
- [x] 设备注册：`DeviceRepo.register`（device_id 唯一约束自带索引，重复 device_id 抛 `IntegrityError`）、`set_online`、`get_by_device_id`
...
- [x] 测试：`tests/test_device_manager.py` 离线覆盖迁移幂等 / 建表 / 用户 CRUD / email 唯一 / device_id 唯一 / 批量入库 / 窗口查询 / 分组统计 / 缓存 TTL / 缓存失效 / CLI（pytest 13 用例）
- [x] 质量：`pyproject.toml` 内置 ruff 配置，`ruff check . && pytest -q` 一键门禁（本环境实测：ruff 零告警、pytest 13 用例全过）

## 验收标准

- [ ] `python3 device_manager.py --demo` 离线跑通：迁移到 schema v4、用户 CRUD 返回正确、重复 device_id 被拒、72 条状态入库（3 台设备 × 24 条）、分组统计 count=24/avg/max 数字正确、缓存 hits=3 / misses=3、自检通过、退出码 0
...
重复 device_id 被拒 -> UNIQUE constraint failed: devices.device_id
状态批量入库 -> 72 条（3 台设备 × 24 条）
