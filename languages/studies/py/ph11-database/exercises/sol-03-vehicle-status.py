# exercises/sol-03-vehicle-status.py —— 练习 3 参考实现：车辆状态表（高频写入 + 复合索引）
# 验证环境：Python 3.13.9（stdlib，无第三方依赖）
# 运行：python3 sol-03-vehicle-status.py（离线可跑，已验证；数据库写入系统临时目录）
# 验证状态：已验证 —— 实测输出（本机 Python 3.13.9 实际运行）：
#   批量写入 100,000 行 -> 总行数: 100000
#   无索引计划: SCAN vehicle_status，耗时 1.89 ms
...
N_DEVICES = 500
N_ROWS = 100_000  # 批量写入 10 万行状态（500 台车 × 200 条）
...
def main() -> None:
    ...
    # 1. 高频写入：executemany 批量插入 10 万行（500 台车 × 200 条）
    rows = [
    ...
    cur.executemany(
        "INSERT INTO vehicle_status (device_id, ts, status, speed) VALUES (?, ?, ?, ?)",
        rows,
    )
    conn.commit()
    print(f"批量写入 {N_ROWS:,} 行 -> 总行数:",
          cur.execute("SELECT COUNT(*) FROM vehicle_status").fetchone()[0])
    ...
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_status_device_ts ON vehicle_status(device_id, ts)"
    )
    conn.commit()
    plan, ms = measure(cur, 7, "2024-06-01T08:00:00", "2024-06-01T08:10:00")
    print("有索引计划:", plan)
    ...
    plan_only_device = cur.execute(
        "EXPLAIN QUERY PLAN SELECT * FROM vehicle_status WHERE device_id = 7"
    ).fetchall()
