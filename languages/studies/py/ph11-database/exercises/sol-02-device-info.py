# exercises/sol-02-device-info.py —— 练习 2 参考实现：设备信息管理（vin 唯一 + 索引）
# 验证环境：Python 3.13.9（stdlib，无第三方依赖）
# 运行：python3 sol-02-device-info.py（离线可跑，已验证；数据库写入系统临时目录）
# 验证状态：已验证 —— 实测输出（本机 Python 3.13.9 实际运行）：
#   注册 2 台设备 -> 总行数: 2；重复 vin -> UNIQUE constraint failed: devices.vin
#   填充后总行数: 20002
#   vin 查询计划（UNIQUE 自带索引）: SEARCH ... USING INDEX sqlite_autoindex_devices_1 (vin=?)
...
def main() -> None:
    ...
    cur.execute(
        """CREATE TABLE devices (
               ...
               vin TEXT NOT NULL UNIQUE,          -- vin 唯一：同一辆车不能重复注册
               model TEXT NOT NULL,
    ...
    # 1. 注册设备 + 重复 vin 拒绝
    for vin, model in [("V001", "EV-A"), ("V002", "EV-B")]:
        cur.execute("BEGIN")
        cur.execute(
    ...
    try:
        cur.execute("BEGIN")
        cur.execute(
            "INSERT INTO devices (vin, model) VALUES (?, ?)", ("V001", "EV-A")
        )
        cur.execute("COMMIT")
    ...
    cur.execute("BEGIN")
    cur.executemany(
        "INSERT OR IGNORE INTO devices (vin, model) VALUES (?, ?)",
        [(f"V{i:05d}", f"EV-{i % 5}") for i in range(20_000)],
    )
    cur.execute("COMMIT")
    print("填充后总行数:", cur.execute("SELECT COUNT(*) FROM devices").fetchone()[0])

    # 3. 按 vin 查询：UNIQUE 约束自带索引（sqlite_autoindex_*），天生走索引
    plan = cur.execute(
        "EXPLAIN QUERY PLAN SELECT * FROM devices WHERE vin = 'V00042'"
    ).fetchall()
    print("vin 查询计划（UNIQUE 自带索引）:", plan)
    ...
    row = cur.execute("SELECT * FROM devices WHERE vin = 'V00042'").fetchone()
    print("按 vin 查询结果:", row)
    conn.close()
