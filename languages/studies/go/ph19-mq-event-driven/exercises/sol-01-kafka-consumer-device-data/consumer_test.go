...
func publishTelemetry(topic *Topic, device string, ts int64, speed float64) {
	raw, _ := json.Marshal(Telemetry{DeviceID: device, TS: ts, Speed: speed})
	topic.Publish(device, raw)
}

// TestDrainGroupConsumesAllOnce 双成员组消费一轮：每设备 samples == 该设备生产条数，
// 总条数 == 生产条数（无重复、无遗漏）。
func TestDrainGroupConsumesAllOnce(t *testing.T) {
	...
	for veh, want := range produced {
		if got := store.Snapshot()[veh].Samples; got != want {
			t.Fatalf("device %s samples=%d, want %d", veh, got, want)
		}
	}
}
