...
// 一句话说明：按设备聚合的消费统计 store + 组消费循环。store 用锁保护是因为组内
...
// StatsStore 线程安全的按设备聚合（模拟下游时序/聚合写入）。
...
func NewStatsStore() *StatsStore {
	return &StatsStore{stats: make(map[string]*DeviceStats)}
}
...
func (s *StatsStore) Apply(t Telemetry) {
	s.mu.Lock()
	defer s.mu.Unlock()
	v, ok := s.stats[t.DeviceID]
	if !ok {
		v = &DeviceStats{DeviceID: t.DeviceID}
		s.stats[t.DeviceID] = v
	}
	v.Samples++
	if t.TS > v.LastTS {
		v.LastTS = t.TS
...
func (s *StatsStore) Snapshot() map[string]DeviceStats {
	s.mu.Lock()
	defer s.mu.Unlock()
	out := make(map[string]DeviceStats, len(s.stats))
