...
//	Producer: kafka.Writer{Topic:"fleet.telemetry.v1"}（key=vehicleID 保证同车同分区）
...
func main() {
	...
	fmt.Printf("== 车辆遥测消费服务：topic=%d 分区，数据集 %d 条 ==\n", *partitions, published)
	start := time.Now()
	...
	fmt.Println("\n== 每车快照（样例）==")
	ids := make([]string, 0, 6)
	states := snap.Vehicles()
	for id := range states {
		ids = append(ids, id)
	}
	sort.Strings(ids)
	for _, id := range ids {
		if id != "car-001" && id != "car-003" && id != dataset.FlakyVehicle {
			continue
		}
		v := states[id]
	...
	for _, d := range dlog.Entries() {
		fmt.Printf("   msg=%s vehicle=%s reason=%s attempts=%d\n",
			short(d.MsgID), short(d.VehicleID), d.Reason, d.Attempts)
	}
