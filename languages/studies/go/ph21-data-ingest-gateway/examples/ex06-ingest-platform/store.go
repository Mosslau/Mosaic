// 来源：ph21-data-ingest-gateway examples/ex06-metrics-platform/store.go
...
// TSStore 时序存储（series 键 = "source|metric"，值是时间有序点集）。
type TSStore struct {
	mu     sync.RWMutex
...
func (s *TSStore) Append(source, metric string, p Point) {
	key := source + "|" + metric
	s.mu.Lock()
...
func (s *TSStore) get(source, metric string) []Point {
	key := source + "|" + metric
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *TSStore) Recent(source, metric string, since time.Time) []Point {
	pts := s.get(source, metric)
...
func (s *TSStore) RangeAggregate(metric string, from, to time.Time, step time.Duration) []AggBucket {
	// 为了教学简单，聚合所有 source 的同名 metric（真实查询按标签过滤）。
	all := s.all(metric)
	if step <= 0 {
		return nil
	}
...
// Set 更新某数据源最新状态。
func (c *StatusCache) Set(source string, ts time.Time, data map[string]any) {
	c.mu.Lock()
	c.cur[source] = stateEntry{ts: ts, data: data}
	c.mu.Unlock()
}

// Latest 返回某数据源最新状态；TTL 过期视为不存在（僵尸数据不可见）。
func (c *StatusCache) Latest(source string) (map[string]any, bool) {
	c.mu.RLock()
	e, ok := c.cur[source]
	c.mu.RUnlock()
	if !ok {
