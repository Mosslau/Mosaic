...
这个阶段只涉及"测量 → 定位 → 减少分配/竞争 → 验证"的性能方法论闭环，**不涉及 Go runtime 底层实现机制（GMP 调度器、GC 算法细节、channel/map/slice/interface 底层布局、reflect/unsafe/cgo——那些是 [ph14 高级 Go 阶段](../ph14-advanced-go/14-advanced-go.md)的内容，roadmap 第 14 节）、PGO 与生产流量指导优化（profile-guided optimization 属 [ph16 PGO 与高级性能优化阶段](../ph16-pgo-advanced-perf/16-pgo-advanced-perf.md)，roadmap 第 16 节）和语言级并发模型本身（goroutine/channel/context 的用法属 ph06 并发编程阶段）** — 本阶段用标准库工具就能完成全部量化与定位，GC/调度"为什么长这样"留给 ph14，PGO"用生产 profile 指导编译"留给 ph16。
...
**为什么性能优化值得单开一个阶段**：ph06 并发阶段教你"怎么写并发"（goroutine、channel、锁），ph12 部署阶段教你"怎么让服务上线"——但"能跑"和"跑得快且稳"之间隔着一整套方法论：怎么量化（benchmark）、怎么定位（pprof/trace）、怎么改（分配/竞争/阻塞三类手段）、怎么证明改有效（复测）。这套方法论与"并发写法"正交：**写法对但性能差的服务，问题几乎都不在写法，而在看不见的分配与竞争**——这正是本阶段要给的"看见它们"的能力。
...
**怎么读一张 benchmark 表（读表顺序也是排查顺序）**：先看 **allocs/op**——分配次数是 GC 压力的直接来源，通常也是优化空间最大的地方（上面的 999→1 就是典型）；再看 **B/op** 确认分配总量是否同步下降（有时次数降了但单次变大，GC 压力未必改善）；最后才看 **ns/op** 判断"快了多少"。三列一起看才能区分"真的快了"和"只是数字变了"：如果 ns/op 下降但 allocs/op 没动，说明省的是计算；如果 allocs 大降但 ns/op 变化不大，可能是优化对象本身不是热点——回到 3.2 用 pprof 确认瓶颈到底在哪。

**`-benchtime` 怎么选**：默认跑约 1 秒（框架自动调 `b.N`），适合快速看趋势；要复现/对比固定场景用 `-benchtime=2000x` 这类固定次数（本阶段练习对拍就用它）；`-count=3` 连跑多次可看结果是否稳定——**单次数值有噪声，多次一致才可信**。
...
**heap profile 的两种视角要分清**：`-inuse_space`（默认，当前仍存活、占着内存的分配）回答"内存被谁占着"，排查"内存涨不停 / 常驻内存"看它；`-alloc_space` 回答"累计分配了多少"（含已回收的），排查"谁在制造 GC 压力 / 高分配频率"看它——**inuse 高 ≠ alloc 高**：一次性初始化一个大结构是 inuse 高但 alloc 低；热点里反复建临时对象是 alloc 高但 inuse 可能不高（对象很快被回收）。对照 3.1 的三列指标：inuse 对应"驻留"，alloc 对应"GC 压力源"，两个视角配合才不误判（`go tool pprof -sample_index=alloc_space` 可切换视角）。
...
把 3.1~3.6 的工具串成一条可复用的流水线——**每次"性能优化"都是一次这样的会话，顺序不能乱**：

| 步骤 | 做什么 | 用什么 | 对应小节 |
|------|--------|--------|---------|
| ① 量化现状 | 给要优化的函数写 benchmark，跑出三列基线 | `go test -bench` | 3.1 |
| ② 定位热点 | 跑 pprof，确认瓶颈**真的在**你怀疑的函数里 | `go tool pprof -top` | 3.2 |
| ③ 形成假设 | 根据 profile 归类：是分配（逃逸/装箱）、竞争（锁）还是阻塞（等待）？ | 读 -top / 看 trace | 3.2~3.6 |
| ④ 最小改动 | 一次只改一处：预分配 / Builder / 按值返回 / 分片锁 / sync.Pool…… | 对应技巧 | 3.4~3.6 |
| ⑤ 复测对比 | 重跑同一个 benchmark，对比三列指标与基线 | `-benchmem` | 3.1 |
| ⑥ 回归验证 | `go test ./...` 保证优化没改坏行为；必要时看正确性测试 | `go vet` / `go test -race` | — |

两个纪律贯穿始终：**第 ② 步不可跳过**——没有 pprof 确认就动手是"凭感觉优化"，改完往往发现瓶颈根本不在这（3.1 读表顺序里已经埋了这个伏笔）；**第 ④ 步一次只改一处**——同时改三处，⑤ 复测时你无法知道是哪一处带来的收益。

### 3.8 优化预算：什么值得优化

SOP 的 ⑥ 步之后还有一个更上游的问题——**这件事到底值不值得优化**。判断顺序：

1. **先看 flat 占比再决定投入**：pprof 的 `-top` 输出按 flat 排序，某个函数占 CPU/分配的比例（如 90%+）才值得抠；占比个位数的"优化"是自我感动（3.2 里 fib 占 93.64% 才配当示例热点）。
2. **先问频率再问单次**：同样的改动，热点函数里调用 100 万次/秒的收益，是冷路径里的 1000 倍——优化预算应该花在"频繁发生"的地方。
3. **收益 vs 可读性**：预分配、按值返回这类改动几乎不伤可读性，优先做；为省 1 allocs 把代码改得晦涩（如手写 buffer 管理）前，先确认这 1 次分配真的是瓶颈（回到 3.1 读表）。
4. **正确性永远优先**：性能优化不能以牺牲行为正确为代价——SOP ⑥ 的回归验证是硬门槛，不是可选项。

一句话：**profile 告诉你"哪里值得"，budget 告诉你"值得投入多少"**——本阶段教会你前者，后者是工程判断，需要多写多复盘才能养成。
...
### 4.4 一个反直觉的总结：先抠分配，再抠算法

把本阶段的实测放在一起看，会发现一个共同模式：**收益最大的几处优化，动的是"分配次数"而不是"算法复杂度"**——字符串拼接改 Builder 快 38 倍（999→1 allocs）、sync.Pool 快约 50 倍（1→0 allocs）、slice 预分配快 3.2 倍（12→1 allocs）、按值返回留栈快约 11 倍（1→0 allocs）。原因在于：**分配是一种"隐藏的 O(n)"**——每次堆分配都要走分配器、参与 GC 标记，成本不在你的代码行里，profile 之前完全看不见；而算法复杂度至少写在代码里，是"看得见的 O(n)"。所以 Go 性能调优的实操顺序往往是：先用 3.1 的三列指标把分配抠干净（常能拿到数量级收益），再回头评估算法本身是否还有优化空间——**这解释了为什么本阶段把 allocs/op 抬到与 ns/op 同等重要的位置**。
...
**性能反模式清单（别这样做）**——每一条都是本阶段各小节结论的反面：

| 反模式 | 症状 | 正确姿势 | 出处 |
|--------|------|---------|------|
| 不测量就动手"优化" | 改完没变快，瓶颈其实在别处 | 先 benchmark + pprof 定位（SOP ① ②） | 3.1 / 3.7 |
| benchmark 结果没用 sink 落地 | 编译器把整个计算删了，测出假数字 | 结果赋给包级变量或 `runtime.KeepAlive` | 3.1 |
| 基准里夹带 `fmt.Println` | ns/op 被输出 IO 污染 | 计时外做输出；`b.ResetTimer()` 排除准备 | 3.1 |
| 该留栈的对象上了 sync.Pool | 反而更慢（Pool 簿记是纯开销） | 先 `-gcflags='-m'` 看能否留栈 | 3.5 |
| `Get` 后不 `Reset` 直接用 | 复用上一次的脏数据，结果串了 | Get → Reset → 用 → Put | 3.5 |
| 把 sync.Pool 当长期缓存 | GC 一跑对象被清，缓存失效 | Pool 只存"用完可弃"的临时对象 | 3.5 |
| 全库一把锁 / 高竞争计数器用 Mutex | 吞吐随 goroutine 数崩溃 | 分片锁或 atomic，先看 mutex profile | 3.6 |
| 优化后不重跑 benchmark | 不知道改没改、改了多少 | SOP ⑤ 必须重跑同一基准对比 | 3.7 |
| 只报 ns/op 不报 allocs/op | 掩盖了 GC 压力这个真问题 | 三列指标一起报（`-benchmem`） | 3.1 |

**压测与采样的操作纪律**：CPU profile 有采样开销、trace 文件可能很大——开发/压测环境采，别在线上长开；压测客户端与目标服务分开跑，否则压测机自己的开销污染数据（ph12 的压测基础设施正是这么隔离的）。
...
- [ ] 能按 3.7 的 SOP 六步走完一次完整优化会话（量化→定位→假设→改动→复测→回归），并交付五栏优化日志
- [ ] 能识别 5 章"性能反模式清单"里的常见坑，且明白每条对应的正确姿势出处
...
项目验收要求"交付一份性能调优报告"——它就是优化日志的成品。记录模板建议固定五栏，保证"先 profile 再优化、用数据验证效果"的纪律可被审计：

```text
日期 | 优化对象 | 现象（profile 结论） | 改动 | 复测（三列指标 before → after）
```

示例行（数字取自本阶段各节实测，仅示范格式）：

```text
09-01 | 字符串拼接（ConcatPlus） | allocs/op 999，明显分配热点 | 改用 strings.Builder + Grow | 74947→1975 ns/op，999→1 allocs
09-01 | JSON 解析 handler | heap alloc_space 显示 map 装箱 | 用具名 struct | 20910→8009 allocs
```

五栏缺一不可：缺"现象"，别人不知道你为什么改；缺"复测"，无法证明改有效。**报告 = 若干这样的行 + 每行背后的 profile 截图/命令输出**——这也是 ph21 之后进入团队协作时 code review 里最有说服力的东西。

### 下一阶段

[高级 Go 阶段](../ph14-advanced-go/14-advanced-go.md) — 本阶段的性能观测升级为"机制解释"：GMP 调度器与 GC 算法的实现、channel/map/slice/interface 的底层布局、defer/panic/recover 原理、reflection/unsafe/cgo——本阶段攒下的 profile 数据与「分配/竞争」的实证观察，正是 ph14 理解"调度器为什么这样设计、GC 为什么这样回收"的入口；届时本阶段的逃逸分析会升级为"看 SSA 优化管线"，sync.Pool 的 per-P 设计会追溯到调度器的 P 结构。当前已建目录至 ph21（Go 路线末阶段，已收官），roadmap 见 [`languages/studies/go/go.md`](../go.md)。
