...
- `ex01`：干净 test acc **0.930**（但两类故障 recall 只有 0.458/0.500——准确率的骗局在 ex03 展开）→ 标签噪声 25% **0.747** → 缺失两列 60% 均值填充 **0.917** → 泄漏特征 val **0.990**（虚高），换真实输入后 **0.897**
...
- `ex05`：单次划分 val acc 跨 10 个 seed 波动 **0.920~0.973**；5 折 CV **0.941 ± 0.004**；CV 调 depth 选中 4 → test **0.944**（贪 train acc 选 depth 12 → test 0.936）
