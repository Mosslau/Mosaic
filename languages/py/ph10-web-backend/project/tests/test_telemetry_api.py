...
def test_report_single_creates_row(tmp_path):
    with make_client(tmp_path) as client:
        r = post(client, {"vehicle_id": "V001", "ts": "2024-06-01T08:00:00",
                          "speed": 60.5, "soc": 78.5})
...
def test_report_validation_ranges(tmp_path):
    with make_client(tmp_path) as client:
        base = {"vehicle_id": "V001", "ts": "2024-06-01T08:00:00",
                "speed": 60.0, "soc": 80.0}
...
def test_batch_report_and_limits(tmp_path):
    with make_client(tmp_path) as client:
        rows = [{"vehicle_id": f"V00{i}", "ts": "2024-06-01T08:00:00",
        ...
        rows_bad = [{"vehicle_id": "VX", "ts": "2024-06-01T08:00:00",
...
def test_list_telemetry_filters(tmp_path):
    with make_client(tmp_path) as client:
        ...
        for i, ts in enumerate(ts_list):
            post(client, {"vehicle_id": "V001", "ts": ts, "speed": 60.0 + i, "soc": 80.0})
        post(client, {"vehicle_id": "V002", "ts": "2024-06-01T08:00:00",
                      "speed": 90.0, "soc": 70.0})

        assert len(client.get("/telemetry").json()) == 4
        by_vehicle = client.get("/telemetry", params={"vehicle_id": "V001"}).json()
        assert len(by_vehicle) == 3 and all(x["vehicle_id"] == "V001" for x in by_vehicle)
...
def test_stats_aggregation(tmp_path):
    with make_client(tmp_path) as client:
        ...
        for vid, ts, speed, soc in data:
            post(client, {"vehicle_id": vid, "ts": ts, "speed": speed, "soc": soc})

        stats = client.get("/telemetry/stats").json()
        assert stats == [
            {"vehicle_id": "V001", "count": 3, "avg_speed": 60.0,
             "max_speed": 70.0, "avg_soc": 85.0},
            {"vehicle_id": "V002", "count": 2, "avg_speed": 90.0,
             "max_speed": 100.0, "avg_soc": 65.0},
        ]

        one = client.get("/telemetry/stats", params={"vehicle_id": "V002"}).json()
        assert len(one) == 1 and one[0]["count"] == 2 and one[0]["max_speed"] == 100.0

        none = client.get("/telemetry/stats", params={"vehicle_id": "V999"}).json()
        assert none == []
...
def test_demo_rows_deterministic():
    ...
    assert len(rows) == 72                       # 3 车 × 24 条
    assert {r.vehicle_id for r in rows} == {"V001", "V002", "V003"}
