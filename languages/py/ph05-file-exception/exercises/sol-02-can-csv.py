# exercises/sol-02-can-csv.py —— 练习 2 参考实现：解析 CSV（统计各 CAN ID 出现次数）
# 来源：05-file-exception.md 第 7 章「动手练习」练习 2
# 验证环境：Python 3.13.12
# 运行：python3 sol-02-can-csv.py
# 验证状态：已验证

"""统计 CAN 日志 CSV 中各 CAN ID 的出现次数，按次数降序输出。"""
...
def count_can_ids(path):
    """统计每个 CAN ID 出现次数，返回 Counter。"""
    counter = Counter()
    try:
        with open(path, "r", encoding="utf-8") as f:
            reader = csv.DictReader(f)
            for row in reader:
                counter[row["can_id"]] += 1
...
def main():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        stats = count_can_ids(path)
        print("CAN ID 出现次数（降序）:")
        for can_id, count in stats.most_common():
            print(f"  {can_id}: {count} 次")
