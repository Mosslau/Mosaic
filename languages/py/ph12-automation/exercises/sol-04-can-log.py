#!/usr/bin/env python3
# exercises/sol-04-can-log.py —— 练习 4 参考实现：解析 CAN 日志（candump 格式 + 统计 + CSV）
# 验证环境：Python 3.13.9（stdlib，无第三方依赖）
# 运行：python3 sol-04-can-log.py（离线可跑，已验证；样本与 CSV 报表在系统临时目录）
# 验证状态：已验证 —— 实测输出（本机 Python 3.13.9 实际运行）：
#   解析 -> 有效帧 12 条、无效行 1 条
#   按 ID 统计 -> 0x123 车速: 6 条 [30, 60] avg 44.67
#                 0x245 电池电压: 3 条 [95, 112] avg 102.33
#                 0x301 电机温度: 3 条 [90, 110] avg 100.0
#   过滤 0x123 -> 6 条
#   CSV 报表 -> can_report.csv 4 行（含表头）
import csv
import re
import tempfile
...
# 车辆信号映射：0x123 车速、0x245 电池电压、0x301 电机温度（首字节即信号值）
...
def stats_by_id(frames: list[Frame]) -> dict[int, tuple[int, int, int, float]]:
    ...
    for f in frames:
        values[f.can_id].append(f.data[0] if f.data else 0)
    result: dict[int, tuple[int, int, int, float]] = {}
    for can_id, vals in sorted(values.items()):
        result[can_id] = (len(vals), min(vals), max(vals), round(sum(vals) / len(vals), 2))
    return result
...
def write_report(stats: dict[int, tuple[int, int, int, float]], out: Path) -> None:
    with out.open("w", newline="", encoding="utf-8") as f:
        ...
        for can_id, (n, lo, hi, avg) in stats.items():
            writer.writerow([f"0x{can_id:X}", SIGNALS.get(can_id, "未知"), n, lo, hi, avg])
...
def main() -> None:
    ...
    counts = Counter(f.can_id for f in frames)
    for can_id, (n, lo, hi, avg) in stats.items():
        print(f"按 ID 统计 -> 0x{can_id:X} {SIGNALS[can_id]}: {n} 条 [{lo}, {hi}] avg {avg}")

    filtered = [f for f in frames if f.can_id == 0x123]
    print("过滤 0x123 ->", len(filtered), "条")

    report = work / "can_report.csv"
