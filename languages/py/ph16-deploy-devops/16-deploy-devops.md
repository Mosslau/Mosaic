...
**ph17+（roadmap 第 17 节 高级 Python 阶段，目录待建）**——部署视角的「服务能跑」之后，回到语言本身往深处走：迭代器/生成器/装饰器的底层协议、描述符与元类、GIL 与垃圾回收的机制细节（本阶段 4.2 只用到「GIL 逼出多进程」这一结论，ph17 讲清它为什么存在）、协程原理与 C 扩展。届时你会重新理解本阶段的两个「为什么」：uvicorn 的事件循环为什么能一个进程服务上千连接（协程原理），以及「单进程 × 多容器副本」与「多进程 worker」之争背后的 GIL 权衡。
