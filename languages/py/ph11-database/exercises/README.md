...
- **验收**：四条路径输出正确；hit 毫秒级、miss 约 0.3s（本机实测 hit 0.1 ms vs miss 305 ms，快约 3010 倍）；脚本结束 redis-server 干净关闭、当前目录无残留；能说清「过期（TTL）+ 淘汰（LRU）+ 主动失效」三个机制各管什么（主文档 4.5）

> **提示**：练习 1~4 与主文档 3.x 小节一一对应（3.1/3.2 SQL 与 sqlite3、3.5 索引、3.7 Redis 缓存）；做完后对照 `sol-*` 参考实现复盘——先独立完成，再看答案。
