...
> 对应 Roadmap（python.md）ph11「推荐项目」第一个「设备管理后端」。devices 表（vin 唯一约束）＋ device_status 状态表（复合索引）＋ 用户 CRUD ＋ 手写迁移脚本演进结构 ＋ 缓存设备热点查询（TTL 过期 + 写库主动失效）——示例 1/3/5 的合体，也是 ph10 设备管理 API 的数据层升级。
