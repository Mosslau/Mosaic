...
## 6. 代码示例
> 说明：示例均可在 Linux/macOS 上原样运行；示例 4 是故意写错的程序，用于演示 GDB 调试流程。
### 示例 1：多文件项目 + Makefile
...
### 进入下一阶段前
确保能完成以下练习：
- 给前面项目写 Makefile：任选 ph05 通讯录或 ph06 日志系统，参考示例 1；验证"只改一个 .c 时只有它被重编"
- 用 GDB 调试段错误：故意写一个越界程序，参考示例 4；完整走一遍 run → 崩溃 → bt → print → 修正
- 封装静态库：把 math_utils 用 `ar rcs` 打包成 libmath.a，另写 main 链接使用，参考示例 2
- 写一个多文件 C 项目：3 个以上 `.c` + 对应 `.h`，用 Makefile 或 CMake 构建，参考示例 1/5
- 同一份含越界代码分别用 `-O0` 与 `-O2` 编译运行，观察行为差异，体会"UB 随优化级别变化"
- 开启 core dump 并离线分析一次崩溃：`ulimit -c unlimited`、`gdb ./crash core`、`bt`
### 推荐项目
