...
**范围边界**：承接 ph07 编译调试；**不涉及** C 标准与可移植性细节（ph09：C89~C23、GCC/Clang/MSVC 差异）、未定义行为深入（ph10）、mmap 与 Page Cache 的存储语义（ph13：存储引擎视角）、网络协议栈深入（TCP 拥塞控制、TLS、HTTP 框架等，后续阶段）。
## 2. 来源与演变
...
```
```bash
gcc -Wall epoll_demo.c -o epoll_demo && ./epoll_demo   # Linux 专有; macOS 无 epoll
```
...
- 未定义行为的系统化梳理（ph10）
- mmap 与 Page Cache 的存储语义（ph13：存储引擎视角的文件访问）
- 高性能网络框架（协程、io_uring、零拷贝、用户态协议栈，后续阶段）
## 6. 代码示例
> 说明：示例均在 Linux 上以 `gcc -Wall -Wextra -pthread` 编译运行；示例 4/5 编译后可用 `nc 127.0.0.1 <端口>` 测试。
### 示例 1：文件复制程序
...
```
```bash
gcc -Wall -Wextra echo_server.c -o echo_server && ./echo_server
nc 127.0.0.1 8888      # 另开终端: 输入什么回显什么
```
...
### 进入下一阶段前
确保能完成以下练习：
- 简单 shell：循环读命令 → fork + execvp 执行 → waitpid 回收；提示：内置命令（cd、exit）不 fork，execvp 依赖 PATH 查找
- 多线程下载器：每个 URL 开一个线程下载，主线程 join 汇总结果；提示：共享的"已完成计数"必须用 mutex 或原子变量保护
- TCP echo server：参考示例 4，用 nc 验证；提示：先测单客户端，再加循环 accept 处理多个连接
- 简单 HTTP server：在示例 5 基础上解析 `GET /path HTTP/1.1\r\n...` 请求，返回固定 HTML；提示：用双 `\r\n` 判断请求头结束，注意半包
- 日志采集程序：多线程生产日志 → 任务队列 → 单写线程 `O_APPEND` 落盘；提示：直接改造示例 3，验证重启后日志不丢不重
### 推荐项目
