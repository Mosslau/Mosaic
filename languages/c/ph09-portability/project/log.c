 ...
 * 运行/测试：make test
 * 验证状态：已验证（POSIX 分支构建零警告, make test 自检通过, 连续运行 20 次稳定;
 *           _WIN32 分支(CRITICAL_SECTION/_getpid)未在本环境验证——需 Windows）
 */
 ...
 * 文件头; 备份按序递增不覆盖, 保证"轮转不丢行"(保留上限属扩展方向);
 * 失败则继续写原文件 */
static void rotate(void) {
    if (!have_file())
        return;
    char backup[sizeof g_path + 16];
    snprintf(backup, sizeof backup, "%s.%u", g_path, g_rotations + 1);
    fclose(g_fp);
    g_fp = NULL;
