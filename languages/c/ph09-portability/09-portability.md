...
**范围边界**：承接 ph08 系统编程；**不涉及** 未定义行为的系统化梳理与 Sanitizer 工具链（ph10、ph11）、字节序与二进制格式的位级处理（ph12）、跨语言互操作 ABI（ph14）；本阶段重在"标准与编译器的差异认知"和"跨平台写法"，不深入具体 UB 案例。
## 2. 来源与演变
...
```
```bash
gcc -std=c99 -Wall -Wextra std_diff.c -o std_diff && ./std_diff   # 正常
gcc -std=c89 -Wall -Wextra -pedantic-errors std_diff.c -o std_diff  # 硬报错: 循环内声明/注释均为 C99 特性
```
...
**要点（坑必背）**：**标准 C 与 POSIX/Linux API 不是一回事**（本阶段必会概念）——`printf` 到处可用，`open`/`pthread` 只在 Unix 系，`CreateFile` 只在 Windows；一个"可移植"的程序 = ISO C 为主干 + 把平台 API 差异收进小封装（见示例 2/5）。
### 3.4 条件编译与平台抽象
```c
#include <stdio.h>
...
**restrict**：向编译器承诺"两个指针不指向同一对象"，换取优化空间——`memcpy` 的签名就是 `void *memcpy(void *restrict dst, const void *restrict src, size_t n)`；只在确实不重叠时使用，滥用就是自造 UB。这两块都是 ph10 未定义行为的铺垫。
## 4. 底层原理
...
- 调试手段：`gcc -E` 看展开后的完整源码，`gcc -dM -E - </dev/null` 列出编译器全部预定义宏（平台宏、`__STDC_VERSION__` 都在里面）。
...
**不适合**此阶段的事项：
- 未定义行为的深入排查（ph10：数组越界、悬垂指针、整数溢出、严格别名案例）
- Sanitizer 与调试工具链（ph11：ASan/UBSan/TSan 的使用）
- 字节序与二进制格式的位级处理（ph12：大端/小端、htonl、位域布局）
- 跨语言互操作 ABI（ph14：C ABI、FFI、JNI 等）
## 6. 代码示例
> 说明：示例在 Linux/macOS 用 `gcc -std=c11 -Wall -Wextra` 编译；Windows 用 MSVC `cl /W4` 或 MinGW gcc 编译。
...
```
```bash
gcc -std=c11 -Wall -Wextra proto_header.c -o proto_header && ./proto_header
```
要点：`uint32_t` 保证字段宽度不随 int/long 平台差异漂移；`_Static_assert` 把"尺寸契约"写进编译期（**编译不过就不许上线**）；结构体存在对齐填充，上"线"要逐字段 memcpy；**位域布局是实现定义的**，跨平台协议里应改用显式掩码位运算；字节序（大端/小端）仍随平台不同，网络传输前还需 htonl（ph12 深入）。
...
```bash
# Linux / macOS:
gcc -std=c11 -Wall -Wextra path_abstraction.c -o path_abstraction && ./path_abstraction
# Windows (MSVC):
cl /W4 path_abstraction.c
```
要点：`_WIN32` 在 32/64 位 Windows 上都定义，判断平台用它；抽象原则是"**按功能抽象**"——路径分隔符、sleep 这类差异收敛成一组宏，业务代码零 `#ifdef`；扩展练习：再封装目录创建（`mkdir` vs `_mkdir`）与文件删除（`remove` 是 ISO C，可跨平台）。
### 示例 3：跨平台类型宽度探测
...
```
```bash
gcc -std=c11 -Wall -Wextra type_widths.c -o type_widths && ./type_widths
# Linux 安装 gcc-multilib 后可对比 32 位数据模型:
gcc -m32 -std=c11 type_widths.c -o type_widths32 && ./type_widths32
```
...
```
```bash
gcc -std=c99 -Wall feature_macros.c -o fm99 && ./fm99     # 输出 C99
gcc -std=c11 -Wall feature_macros.c -o fm11 && ./fm11     # 输出 C11
gcc -std=c17 -Wall feature_macros.c -o fm17 && ./fm17     # 输出 C17
clang -std=c11 -Wall feature_macros.c -o fmc && ./fmc     # 换编译器对比
```
...
```bash
# Linux / macOS:
gcc -std=c11 -Wall -Wextra portable_log.c -o portable_log && ./portable_log && cat app.log
# Windows (MSVC):  cl /W4 portable_log.c
```
要点：时间（time/localtime/strftime）、文件（fopen/fprintf）、可变参数（va_*）全是 ISO C，天然跨平台；**唯一需要抽象的差异（getpid、路径、线程）用 `#ifdef` 收进小封装**；`"pid=%" PRId64` 是字符串字面量拼接，拼出平台无关的 64 位格式串；扩展方向：加 mutex 保证线程安全（ph08）、按大小轮转日志文件、级别过滤。
## 7. 总结
### 关键要点
...
### 进入下一阶段前
确保能完成以下练习：
- 用 stdint.h 重写协议字段定义：把原来 int/long 的协议头改成 uint16_t/uint32_t 并加 _Static_assert；提示：打印也一并换成 PRIx32/PRIx64
- 为 Windows/Linux 分别封装路径分隔符：参考示例 2，再封装目录创建（mkdir/_mkdir）与文件删除（remove）；提示：mkdir 在 POSIX 有 mode 参数、Windows 只有一个参数，用两个函数或可变参数宏
- 尝试用 GCC 和 Clang 编译同一项目：`gcc -std=c11 -Wall -Wextra -pedantic` 与 `clang ...` 各编一遍对比警告；提示：-pedantic 会暴露依赖扩展的代码
- 用 -std=c89 / -std=c99 / -std=c11 编译同一文件，观察语法差异报错；提示：`//` 注释和循环内声明在 c89 严格模式（-pedantic-errors）下报错
- 写一个平台探测程序：打印 __STDC_VERSION__、编译器宏、平台宏、sizeof 各类型；提示：合并示例 3 与示例 4
### 推荐项目
- **跨平台日志库**：在示例 5 基础上加线程安全（ph08 的 mutex）、按大小轮转日志、级别过滤，用 _Static_assert 校验日志头格式
- **协议字段类型定义库**：在示例 1 基础上扩展为完整消息类型集合（定长头 + 变长载荷），提供打包/解包函数，全部字段用 stdint.h 定义
### 下一阶段
**未定义行为 UB 与常见坑**（ph10-ub，文档规划中）—— 数组越界、悬垂指针、整数溢出、严格别名等 UB 的识别与规避；本阶段埋下的 restrict、类型双关、有符号溢出问题将在那里系统化排查。
