...
本阶段在单文件基础上扩展到多文件项目，**不涉及指针深入、堆内存分配、结构体和复杂数据结构** — 那些是后续阶段的内容。ph01 中提到"`scanf` 需要 `&`"，本阶段会讲清"按值传递"的本质，并预告："要让函数修改调用者的变量，需要指针，ph03 详解"。
...
确保能完成以下练习：
- 字符串工具库：实现字符串长度、复制、比较的封装函数
- 数组排序工具库：实现冒泡排序、选择排序并封装成函数
- 递归阶乘与斐波那契
- 把一个单文件程序拆成多个 `.c` / `.h`

### 推荐项目

- **小型数学工具库**：加减乘除、幂运算、最大公约数、最小公倍数
- **通讯录程序模块化重构**：把单文件版本拆分为 `contact.h`、`contact.c`、`main.c`
