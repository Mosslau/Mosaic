...
**胖指针形态**：裸指针不总是「一个地址」——`*const [T]` / `*mut [T]` 与 `*const dyn Trait` 是**胖指针**（两个机器字：数据指针 + 长度 / 虚表指针）；解引用胖指针时，元数据（长度、虚表）本身也必须合法。把「裸指针 + 长度」手工合成切片引用的 `slice::from_raw_parts` 是零拷贝借用的入口，属 ph19 内存布局、零拷贝与协议解析阶段（目录待建），本阶段只需知道胖指针的存在。
