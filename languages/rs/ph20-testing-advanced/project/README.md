...
- **回归阈值自动化**：用 `--save-baseline` + 解析 `change` 输出做「性能回归 CI 判红」，criterion 深层统计与阈值配套属 [ph22 性能优化与 Profiling 阶段](../../ph22-perf-profiling/22-perf-profiling.md)
- **真引擎对接**：把测试里的假引擎换成 ph25 的真实 MemTable/WAL replay，套件不变——那是 [ph25 Rust 数据基础设施专项阶段](../../ph25-data-infrastructure/25-data-infrastructure.md)的落点
