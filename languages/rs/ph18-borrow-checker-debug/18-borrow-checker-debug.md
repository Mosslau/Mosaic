...
这个阶段只涉及**读懂并修复借用错误**——即「编译器报错 → 归因 → 选择修法家族 → 重构出干净所有权流」这一闭环，**不涉及内存布局、零拷贝与协议解析**（借用切片的零拷贝用法是 ph19 的事，roadmap 第 19 节，目录待建）、**性能优化**（clone 的性能代价在本阶段只作「取舍判据」，实际剖析与基准属 ph22 性能优化与 Profiling 阶段，roadmap 第 22 节，目录待建）、**跨语言 FFI**（所有权跨边界属 ph23 Rust FFI 与跨语言接口设计阶段，roadmap 第 23 节，目录待建）、**依赖与供应链安全**（ph24 安全、供应链与发布阶段，roadmap 第 24 节，目录待建）。
...
| 性能敏感热路径 | **必须重构** | 分配次数是 ph22 的首要优化对象，先在这里把 clone 消灭掉 |
...
**ph19 内存布局、零拷贝与协议解析阶段**（roadmap 第 19 节，目录待建）——本阶段修好的「借用」正是零拷贝的地基：当你用切片 `&[u8]` 直接借用网络缓冲、而不复制成 Vec 时，就是在吃透 E0507/E0597 之后，让借用为自己省下每一次分配。届时把本阶段的「拥有化 vs 借用」判断搬到字节层，就是零拷贝协议解析的核心手艺。
