...
[**ph24 安全、供应链与发布阶段**](../ph24-supply-chain-release/24-supply-chain-release.md)——本阶段把 FFI 的 unsafe 审查纪律止步于「每个 unsafe 写清 soundness 前提」；下一阶段系统回答「怎么让交付物可信」：cargo audit/cargo deny、许可证与 SBOM、secret 管理、可复现构建与 crates.io 发布流程——届时本阶段的 cdylib/pyo3 制品将进入审计、签名与发布流水线，跨语言边界的系统化安全审查在那里补完。
