#!/usr/bin/env bash
# release-check.sh —— 本地可跑的发布前检查流水线（project/relpipe）
#
# 对应 roadmap §24 推荐项目「安全发布流水线：自动测试、审计、构建、打包和生成发布说明」。
...
export CARGO_TARGET_DIR="${CARGO_TARGET_DIR:-/tmp/ph24-relpipe-target}"

VERSION="$(sed -n 's/^version *= *"\([^"]*\)".*/\1/p' Cargo.toml | head -1)"
echo "==== ph24-relpipe v${VERSION} release-check ===="
...
echo
echo "==== release-check 通过：v${VERSION} 可以进入 cargo publish（需账号 + token，真实发布未在本环境验证） ===="
