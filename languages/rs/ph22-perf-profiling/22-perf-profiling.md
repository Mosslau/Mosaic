...
[**ph23 Rust FFI 与跨语言接口设计阶段**](../ph23-ffi-interop/23-ffi-interop.md)——本阶段优化的都是 Rust 进程内路径；下一阶段回答「当热点必须跨语言协作时怎么设计边界」：extern "C" ABI、cdylib、所有权与内存释放规则跨边界、pyo3 加速模块——届时本阶段的「分配在哪一侧」思考会换成「分配由谁负责」，把 ph22 的性能纪律带过 FFI 边界。
