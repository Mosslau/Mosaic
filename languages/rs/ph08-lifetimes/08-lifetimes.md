...
**本阶段边界**：承接 ph07 trait 与泛型（本阶段大量示例是"生命周期参数 + 泛型 + trait bound"的混合签名）；不深入集合与迭代器函数式写法（ph09）、智能指针与内部可变性（ph10，`Rc`/`RefCell` 让共享可变成为可能）、异步生命周期（ph12，借用跨 `.await` 的限制）、复杂自引用结构（后续阶段）。
...
- **只读配置视图**：从配置文本中借用字段并提供查询接口（roadmap 推荐项目，示例 2 已给出核心）。扩展方向：支持 `get(key) -> Option<&str>` 通用查询、按 section 分组、行号定位；再写一个拥有字段（`String`）版本对比生命周期差异，体会"视图 vs 持有"的取舍。
- **文本索引视图**（可选）：对日志文本建立"行号 → 行内容"的借用视图，提供 `line(n) -> Option<&str>` 查询接口——同样是"借用不复制"，与示例 2 同套路。
