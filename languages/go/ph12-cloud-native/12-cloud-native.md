...
本阶段之后是 **ph13+（性能优化阶段，roadmap 第 13 节，目录待建）**：把"能部署、能观察"升级为"能调优"——benchmark 与 pprof 分析（高并发接口压测与优化）、GC 与内存分配分析（减少分配次数缓解 GC 压力）、goroutine 泄漏排查、逃逸分析与 sync.Pool——本阶段的 /metrics 延迟直方图与日志里的 latency_ms 正是 ph13 优化前定位瓶颈的入口；在此之前可先按推荐学习顺序巩固本阶段的练习与项目。
