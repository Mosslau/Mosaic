...
└── internal/
    ...
    ├── vehicle/               # 内存车辆存储（Store 接口 + RWMutex 实现）
...
- `go test -cover ./...` 覆盖率：internal/api 83.9%、internal/auth 57.7%、internal/vehicle 100.0%（实测，go1.25.6）
- `go run ./cmd/api` 启动后（127.0.0.1:18080）：
  ...
  - 带 token 上报 `/api/devices/car-001/report` 返回 200 与更新后的车辆 JSON；不带 token 返回 401
  - 上报速度 301 返回 400；每分钟第 11 次上报返回 429
  - `curl -s http://127.0.0.1:18080/api/devices` 返回三台车 JSON
- 上述冒烟测试已在本环境实测通过（go1.25.6，darwin/arm64），服务测完已 kill，无残留进程

## 扩展方向（可选）

- **持久化**：把 `vehicle.Store` 换成 MySQL/Redis 实现，设备注册与状态落库（ph10 数据库阶段）
