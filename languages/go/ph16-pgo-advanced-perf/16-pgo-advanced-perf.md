...
这个阶段只涉及「用 profile 指导编译与性能回归闭环」，**不涉及 pprof/benchmark 工具的基础用法（ph13 性能优化阶段已讲，本阶段直接复用）、版本与工具链管理（ph15 Go 版本、工具链阶段）、架构分层设计（ph17 架构设计与代码分层阶段，[展开版见 17-architecture-layering.md](../ph17-architecture-layering/17-architecture-layering.md)）、配置管理与发布策略编排（配置中心、feature flag、灰度/滚动发布与回滚、版本号与构建信息属 [ph20 配置管理与发布策略阶段](../ph20-config-release/20-config-release.md)）** — 本阶段把"性能"钉在编译决策这一层：怎么用数据（profile）改变编译器行为、怎么用基线守住优化成果；服务怎么分层、发布怎么编排是后面阶段的事。
...
peek 输出怎么读（上例 `-peek='renderBody'` 的两行是同一个调用关系，`|` 是分隔符不是管道）：

| peek 输出的字段 | 含义 | ex03 实例 |
|---------------|------|----------|
| 首行 `cum 30ms \| main.renderBody` | 被 peek 函数自身的**累计时间**（含它调用的子孙）+ 函数名 | renderBody flat≈0% 却有 30ms——时间全花在"它调用了谁" |
| 续行 `\| runtime.concatstring2 100%` | 一个 callee 的名字 + 占首行 cum 的百分比 | concatstring2 吃掉全部 30ms → `+=` 拼接才是病因 |
...
两种构建的内联决策对照（同一个"预算"概念，输入不同——静态特征 vs 实测热度）：

| 维度 | 普通构建（静态启发式） | PGO 构建（profile 加权代价模型） |
|------|----------------------|--------------------------------|
| 判断依据 | 函数体大小等**静态特征**，没有任何运行期信息 | profile 采样折算的**调用点权重**（热度） |
| 预算怎么花 | 语法树顺序先到先得，冷热同价 | 按热度排序，热调用点优先消耗预算 |
| 大而热的函数（mdurl.Parse） | 超预算阈值 → 放弃内联 | 调用点热 → 突破阈值照常内联 |
| 小而冷的函数 | 体小即内联（不看冷热，白付代码膨胀） | 冷调用点保持调用，省下代码膨胀预算 |
| 内联的下游 | 不内联就没有下游可言 | 逃逸分析看清局部对象不逃逸 → 堆分配变栈分配（mdurl 分配清零）；常量传播有下手空间 |
...
| 基本块 | 普通构建（无热度信息） | PGO 布局（profile 引导） |
|--------|----------------------|-------------------------|
| 热块（99% 走这里） | 位置靠编译器静态猜，可能排在分支后面、每迭代都要跳进去 | 排在**顺序直落**位：类型断言命中后不跳转、顺着执行，不打断指令预取 |
| 冷块（错误/回退分支） | 与热块混杂、占着主路径 | 移出主路径（去虚拟化回退块放函数尾部；完整 FDO 还可做冷热分离） |

**同一个"热的优先"原则贯穿 §4 三处**：4.1 的热调用点多吃内联预算（mdurl 例子）、4.2 的热类型拿到类型断言直连快路径、4.3 的热块拿到顺序直落的位置——都是把机器码资源往"实测会走的那条路"上倾斜，与 4.1 表格里"静态启发式 → 加权代价模型"的转变是同一件事。
...
**跨语言对比**：Go（静态 AOT）与 C/C++/Rust（GCC/LLVM 的编译期 FDO）、Java/JVM（JIT 把 profile 内建在运行期）的逐语言对照与结论，汇总在第 7 章的「跨语言对比」小节（为 analysis/ 与 Tenet 合成积累素材）——本节不重复，直接跳读该小节即可。
...
1. **采集压测 profile**（★★）：并发压测自己的 CPU 密集函数并采窗口内 profile，验证热点压在预期函数上（参考实现实测：`pprof -top` 第一行即业务函数 compressBlock，占绝对多数——sol-01-collect-profile/main.go 文件头验证块实测 84.01%，exercises/README 验收口径同引此数（±10~20% 波动，以文件头为准）；并解释"并发 worker 数 × 窗口时长 > 窗口采样时长"）
...
4. **对比优化前后延迟和吞吐**（★★★）：同一负载分别跑基线与 `-pgo` 二进制，报 p50/p95/p99/均值与吞吐（参考实现实测 p50 1.625µs→0.917µs、吞吐 ~590k→~955k req/s、digest 一致——p99 3.25→3.75µs 微升属尾部离群/负载分布效应（±10~20% 波动内），主要收益在 p50 与吞吐；见 sol-04-latency-throughput/main.go 文件头验证块；`-cpuprofile` 采集与逐请求计时互斥的原因也记录在其注释里）
...
*本文为纯写作交付，未新增任何运行验证。全部"实测"数字与"已验证"声明均引自既有文件——examples/README.md（ex01~ex05 五个示例的实测记录，2026-09-03）、examples/ 各源码文件头验证块（ex01~ex05）、exercises/sol-01~sol-04 文件头验证块（2026-09-02/03）、project/ 各文件头（cmd/apiserver 2026-09-02；cmd/loadgen、internal/workload 2026-09-02 首验 / 2026-09-03 复验），验证环境统一为 go1.25.6（darwin/arm64，Apple M4 Pro），数字随机器与 Go 版本波动 ±10~20%。Go 1.20/1.21 的 PGO 里程碑与官方收益区间（2~4%、2~7%）以 go.dev 官方博客与文档为准；不虚构验证结果。*
