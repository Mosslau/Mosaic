// 来源：ph17-architecture-layering project/internal/handler/device_test.go
// 一句话说明：handler 层集成单测——真实 mux（含 {id} 通配）+ service + 内存存储，
// 覆盖端到端规则（注册/查重/404/离线拒命令/心跳后放行/固件回退 409）与统一错误结构。
...
func register(t *testing.T, mux *http.ServeMux, id, name, fw string) *httptest.ResponseRecorder {
	t.Helper()
	return do(t, mux, http.MethodPost, "/api/devices", createRequest{ID: id, Name: name, Firmware: fw})
}
...
func TestCreateAndGetDevice(t *testing.T) {
	_, mux := newTestHandler()
	rec := register(t, mux, "dev-1", "车队 A", "1.2.3")
	...
	if d.ID != "dev-1" || d.Status != domain.StatusOffline {
		t.Fatalf("created device = %+v, want dev-1/offline（新设备默认离线）", d)
	}

	rec2 := do(t, mux, http.MethodGet, "/api/devices/dev-1", nil)
...
func TestCreateDuplicateReturnsConflict(t *testing.T) {
	_, mux := newTestHandler()
	if rec := register(t, mux, "dev-1", "车队 A", ""); rec.Code != http.StatusCreated {
	...
	if out := decodeErr(t, rec); out.Code != errs.CodeExists {
		t.Fatalf("code = %q, want DEVICE_EXISTS", out.Code)
	}
}

func TestGetUnknownDeviceReturnsNotFound(t *testing.T) {
	_, mux := newTestHandler()
	rec := do(t, mux, http.MethodGet, "/api/devices/ghost", nil)
	if rec.Code != http.StatusNotFound {
		t.Fatalf("status = %d, want 404", rec.Code)
	}
	if out := decodeErr(t, rec); out.Code != errs.CodeNotFound {
		t.Fatalf("code = %q, want DEVICE_NOT_FOUND", out.Code)
	}
}
...
func TestCommandFlowOfflineThenHeartbeatThenAccepted(t *testing.T) {
	...
	// 离线：拒收命令（409 DEVICE_OFFLINE）
	rec := do(t, mux, http.MethodPost, "/api/devices/dev-1/commands", commandRequest{Command: "restart"})
	if rec.Code != http.StatusConflict {
		t.Fatalf("offline command status = %d, want 409", rec.Code)
	}
	if out := decodeErr(t, rec); out.Code != errs.CodeOffline {
		t.Fatalf("code = %q, want DEVICE_OFFLINE", out.Code)
	}

	// 心跳上线
	rec = do(t, mux, http.MethodPost, "/api/devices/dev-1/heartbeat", nil)
	if rec.Code != http.StatusOK {
...
func TestFirmwareDowngradeReturnsConflict(t *testing.T) {
	_, mux := newTestHandler()
	if rec := register(t, mux, "dev-1", "车队 A", "2.0.0"); rec.Code != http.StatusCreated {
		t.Fatalf("create status = %d, want 201", rec.Code)
	}
	rec := do(t, mux, http.MethodPost, "/api/devices/dev-1/firmware", firmwareRequest{Version: "1.0.0"})
	if rec.Code != http.StatusConflict {
...
func TestDeleteDevice(t *testing.T) {
	...
	if rec := do(t, mux, http.MethodDelete, "/api/devices/dev-1", nil); rec.Code != http.StatusNoContent {
		t.Fatalf("delete status = %d, want 204", rec.Code)
	}
	if rec := do(t, mux, http.MethodGet, "/api/devices/dev-1", nil); rec.Code != http.StatusNotFound {
