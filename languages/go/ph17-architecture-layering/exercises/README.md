...
> 先自己做，再对照 `sol-*` 参考实现。每题标注难度（★~★★★）。四题与 roadmap §17 对齐：练习 1 ↔「重构 Todo API 分层」、练习 2 ↔「抽象存储接口」、练习 3 ↔「给 service 层写单元测试」、练习 4 ↔ 学习内容「错误码、接口边界（统一错误结构 + 边界映射）」。roadmap 推荐项目「分层 Web API 模板 / 车联网设备管理服务」由本阶段 project/ 落地（选了后者）。
