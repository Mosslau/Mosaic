// 来源：ph21-data-ingest-gateway project/internal/gateway/gateway_test.go
// 一句话说明：网关核心测试——聚合满批、spool 有界与水位、断网→入池→恢复→补传
...
func gsample(vin string, seq uint64) model.Sample {
	return model.Sample{Vin: vin, Seq: seq, Speed: 30}
}
...
func TestCollectorFlushOnThreshold(t *testing.T) {
	...
	for i := 1; i <= 4; i++ {
		c.Add("edge-001", gsample("veh-001", uint64(i)))
	}
...
func TestGatewayOfflineThenBackfill(t *testing.T) {
	cloud := &fakeUplink{seen: map[uint64]bool{}}
	gw := NewGateway("edge-t", 32, cloud, log.New(io.Discard, "", 0))
	col := NewCollector(2, gw.HandleBatch)

	cloud.offline = true
	for i := 1; i <= 8; i++ { // 8 条 → 4 批全部入 spool
		col.Add("edge-t", gsample("veh-001", uint64(i)))
	}
	if gw.Pending() != 4 {
...
func TestHTTPUplinkAuthFailure(t *testing.T) {
	...
	defer ts.Close()
	up := NewHTTPUplink(ts.URL, "edge-001", "wrong-secret")
	_, err := up.SendBatch(context.Background(), model.BatchUpload{GatewayID: "edge-001", BatchID: 1, Samples: []model.Sample{gsample("v", 1)}})
...
func TestHTTPUplink503Retryable(t *testing.T) {
	...
	defer ts.Close()
	up := NewHTTPUplink(ts.URL, "edge-001", "s")
	_, err := up.SendBatch(context.Background(), model.BatchUpload{GatewayID: "edge-001", BatchID: 1, Samples: []model.Sample{gsample("v", 1)}})
...
func TestHTTPUplinkAckOK(t *testing.T) {
	...
	defer ts.Close()
	up := NewHTTPUplink(ts.URL, "edge-001", "s")
	ack, err := up.SendBatch(context.Background(), model.BatchUpload{GatewayID: "edge-001", BatchID: 7, Samples: []model.Sample{gsample("v", 1)}})
	if err != nil || ack != 7 {
