// 来源：ph21-data-ingest-gateway project/internal/gateway/gateway.go
// 一句话说明：边缘网关核心——本地聚合出批、上行接口抽象、有界断网缓存(spool)、
...
var ErrSpoolFull = errors.New("gateway: spool 满")
...
// Collector 本地聚合：同车样本攒批满 flushSize 触发回调。
...
// Add 收样本，满批 flush。
func (c *Collector) Add(gatewayID string, s model.Sample) {
	c.mu.Lock()
	c.buf[s.Vin] = append(c.buf[s.Vin], s)
	if len(c.buf[s.Vin]) >= c.FlushSize {
		b := model.BatchUpload{GatewayID: gatewayID, BatchID: c.nextID, Samples: c.buf[s.Vin]}
		c.nextID++
		c.buf[s.Vin] = nil
		c.mu.Unlock()
		c.Flush(b)
...
func (g *Gateway) HandleBatch(b model.BatchUpload) {
	...
	if errors.Is(err, auth.ErrUnauthorized) {
		g.reject.Add(1)
		g.log.Printf("网关 %s: 批 %d 被拒（鉴权失败，不重试）: %v", g.ID, b.BatchID, err)
		return
	}
	if serr := g.spool.Enqueue(b); serr != nil {
		g.log.Printf("网关 %s: spool 满，批 %d 按丢弃策略放弃: %v", g.ID, b.BatchID, serr)
		return
	}
	g.queued.Add(1)
...
func (g *Gateway) Flush(ctx context.Context) (int, bool) {
	...
	for _, b := range pending {
		...
		if err != nil {
			...
			g.log.Printf("网关 %s: 补传批 %d 失败: %v", g.ID, b.BatchID, err)
			break // 断点续传：失败处停下重试
		}
		sent++
		g.spool.AckUpTo(ack)
...
func (g *Gateway) Stats() (sent, queued, rejected int64) {
	return g.sent.Load(), g.queued.Load(), g.reject.Load()
}

// Pending 待补传数（运维/测试视图）。
func (g *Gateway) Pending() int { return g.spool.Len() }
