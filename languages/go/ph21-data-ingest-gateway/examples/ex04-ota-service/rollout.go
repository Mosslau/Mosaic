// 来源：ph21-data-ingest-gateway examples/ex04-ota-service/rollout.go
// 一句话说明：OTA 分批灰度决策器——批次按 VIN 名单推进，每台车一次 OnDeviceReport
...
// Batch 一个灰度批次：显式 VIN 名单（真实系统按 VIN 尾号/车队分组生成）。
type Batch struct {
	Index int
	VINs  []string
}

// Rollout 一场 OTA 灰度。规则是数据、状态可查询、决策可离线回放。
type Rollout struct {
	opts      RolloutOptions
	batches   []*Batch
	vinBatch  map[string]int    // vin → 批次下标（决策入口 O(1)）
	startFrom map[string]string // vin → 升级前版本（回滚目标，计划开始时快照）
...
func (r *Rollout) CurrentBatch() (int, []string) {
	...
	if r.cur < len(r.batches) {
		b := r.batches[r.cur]
		return b.Index, append([]string(nil), b.VINs...)
	}
	return -1, nil
}

// RollbackTarget 返回某车的回滚目标版本（升级前版本）。
func (r *Rollout) RollbackTarget(vin string) string { return r.startFrom[vin] }

// OnDeviceReport 一台车报告升级结果，返回本次决策。失败率越限 → rollback；
