// 来源：ph21-data-ingest-gateway examples/ex05-relay-collector/main.go
// 一句话说明：演示主程序——假云端（内存，带批幂等与 ack 水位）+ 采集代理：
// 数据源采样 → 聚合满批上行；人为制造"断网窗口"（期间批入 spool）→ 恢复后 flush
...
// fakeCloud 内存假云端：按 collector+batchID 幂等去重，回 ack 水位。
...
func main() {
	...
	cloud := newFakeCloud()
	gw := NewRelay("relay-001", 32, cloud, l)

	// 聚合器满 3 条出一批。
	flushCount := 0
	col := NewCollector("relay-001", 3, func(b Batch) {
		flushCount++
		gw.HandleBatch(b)
	})
	...
	for i := 1; i <= 6; i++ {
		col.Add(Sample{SourceID: "src-001", Seq: uint64(i), Value: float64(30 + i), Ts: time.Now().Unix()})
	}
