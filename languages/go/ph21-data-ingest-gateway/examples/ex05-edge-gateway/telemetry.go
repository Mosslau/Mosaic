// 来源：ph21-data-ingest-gateway examples/ex05-edge-gateway/telemetry.go
// 一句话说明：车端样本与上行批模型——样本带单调 seq（幂等键），批是"同车一段
...
// Sample 一条车端遥测样本。Seq 由网关在采集时分配（本地单调、重启由 spool 续）。
type Sample struct {
	Vin   string  `json:"vin"`
	Seq   uint64  `json:"seq"`
...
// Batch 上行批：同一网关、同一批内是若干样本（云端按 (vin, batchID) 幂等）。
type Batch struct {
...
func readFrame(r io.Reader) ([]byte, error) {
	...
	if n > maxFrameSize { // 长度上限校验：防恶意/损坏设备申请超大 buffer
