// 来源：ph21-data-ingest-gateway examples/ex05-edge-gateway/ex05_test.go
// 一句话说明：网关四件套测试——长度前缀切帧、本地聚合满批触发、spool 有界与
...
func TestFrameRoundTrip(t *testing.T) {
	payload := []byte(`{"type":"telemetry","len":17}`)
...
func TestCollectorFlushOnThreshold(t *testing.T) {
	var flushed []Batch
	col := NewCollector("edge-1", 3, func(b Batch) { flushed = append(flushed, b) })
	for i := 1; i <= 5; i++ {
		col.Add(Sample{Vin: "veh-001", Seq: uint64(i)})
	}
	if len(flushed) != 1 {
...
func TestGatewayOfflineCacheThenBackfill(t *testing.T) {
	cloud := &flakyUplink{seen: map[uint64]bool{}}
	gw := NewGateway("edge-t", 32, cloud, log.New(io.Discard, "", 0))
	col := NewCollector("edge-t", 2, gw.HandleBatch)
...
func TestGatewayBackfillKeepsOriginalSeqs(t *testing.T) {
	// 断网 3 批（batchID 2/3/4 原号保留，不重新编号）；恢复后按序补传，
	// 云端幂等窗按 batchID 去重，水位推进到全部确认——"补传是重放不是再生成"。
	cloud := &flakyUplink{seen: map[uint64]bool{}}
	gw := NewGateway("edge-t", 8, cloud, log.New(io.Discard, "", 0))
	col := NewCollector("edge-t", 2, gw.HandleBatch)
