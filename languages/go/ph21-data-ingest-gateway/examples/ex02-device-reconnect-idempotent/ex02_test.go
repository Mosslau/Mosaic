// 来源：ph21-data-ingest-gateway examples/ex02-device-reconnect-idempotent/ex02_test.go
...
func TestTokenVerifyWithinWindow(t *testing.T) {
	now := time.Now()
	tok := SignToken("s3cr3t", "veh-001", tokenSlotDur, now)
	if err := VerifyToken("s3cr3t", "veh-001", tok, tokenSlotDur, now.Add(tokenSlotDur/2)); err != nil {
		t.Errorf("窗口内应通过: %v", err)
	}
}

func TestTokenRejectedWrongSecretAndDevice(t *testing.T) {
	now := time.Now()
	tok := SignToken("s3cr3t", "veh-001", tokenSlotDur, now)
	if err := VerifyToken("wrong", "veh-001", tok, tokenSlotDur, now); !errors.Is(err, ErrUnauthorized) {
		t.Errorf("错密钥应拒绝，got %v", err)
	}
	if err := VerifyToken("s3cr3t", "veh-002", tok, tokenSlotDur, now); !errors.Is(err, ErrUnauthorized) {
		t.Errorf("错设备应拒绝，got %v", err)
	}
}
...
func TestTokenExpiresAfterWindow(t *testing.T) {
	...
	if err := VerifyToken("s3cr3t", "veh-001", tok, tokenSlotDur, future); !errors.Is(err, ErrUnauthorized) {
...
func TestDeduperSemantics(t *testing.T) {
	...
	for _, c := range cases {
		if got := d.FirstTime(c.vin, c.seq); got != c.want {
			t.Errorf("(%s,%d) = %v, want %v（%s）", c.vin, c.seq, got, c.want, c.why)
		}
	}
}
...
func TestPlatformHTTPEndToEnd(t *testing.T) {
	platform := NewPlatform(map[string]string{"veh-001": "s3cr3t"})
	ts := httptest.NewServer(platform.Handler())
	defer ts.Close()

	dev := &Reporter{
		Endpoint: ts.URL + "/api/v1/telemetry",
		DeviceID: "veh-001", Secret: "s3cr3t",
		Client: ts.Client(),
	}
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	if err := dev.SendOnce(ctx, Telemetry{Vin: "veh-001", Seq: 1, Speed: 42}); err != nil {
		t.Fatalf("首次上报: %v", err)
	}
	// 同一 seq 重试：平台应判重复但 HTTP 仍 200（accepted=false），不报错。
	if err := dev.SendWithRetry(ctx, Telemetry{Vin: "veh-001", Seq: 1, Speed: 42}); err != nil {
		t.Fatalf("重试上报: %v", err)
	}
	if err := dev.SendOnce(ctx, Telemetry{Vin: "veh-001", Seq: 2, Speed: 43}); err != nil {
		t.Fatalf("第二条: %v", err)
	}
	...
	bad := &Reporter{
		Endpoint: ts.URL + "/api/v1/telemetry",
		DeviceID: "veh-001", Secret: "wrong",
		Client: ts.Client(),
	}
	err := bad.SendOnce(ctx, Telemetry{Vin: "veh-001", Seq: 3, Speed: 0})
