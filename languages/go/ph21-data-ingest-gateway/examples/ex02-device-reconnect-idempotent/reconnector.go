// 来源：ph21-data-ingest-gateway examples/ex02-device-reconnect-idempotent/reconnector.go
// 一句话说明：指数退避 + 抖动的重连循环——网络类错误按 1s→2s→4s 退避重试
// （抖动防海量设备同刻重连），鉴权失败直接上报不重试，退避可注入 clock 离线测。
...
func (r *ReconnectLoop) Run(ctx context.Context) error {
	...
	for attempt := 1; attempt <= r.MaxAttempts; attempt++ {
		...
		delay := backoff
		if r.Jitter != nil {
			// 抖动 = [0, backoff/2) 随机增量：避免整群设备同时重连的惊群。
