...
//   裸跑（不带 TSan）：竞态版本输出无意义（实测 shared 值不定：两次运行 114401 与 200000），
//   修复版本输出 200000 —— 但“碰巧对”不能证明无竞态，必须 TSan 复跑。
...
#else
    std::printf("race:  shared=%d（竞态下值不定，丢失更新使其通常小于 200000，实测两次 114401 / 200000）\n", shared);
#endif
    return 0;
}
...
//       Previous read/write ... by thread T3:
...
//   [裸跑对照] 竞态版本退出码 0 但 shared 值不定（四次实测 114401/200000/123474/117555）——
