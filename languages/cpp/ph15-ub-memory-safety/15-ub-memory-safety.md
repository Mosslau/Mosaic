...
这个阶段只涉及 C++ UB 的分类识别、语义核对与规避，以及把 Sanitizer 当**演示/复现工具**，**不涉及 Sanitizer 工具链的系统化与工程化（ph16 测试、静态分析与代码规范阶段）、并发同步的完整规则与内存序（ph08 并发编程阶段）、对象生命周期与值类别的完整规则（ph12 对象生命周期、值类别与所有权深入阶段）、const_cast 修改真正 const 对象的系统归类（ph14 const 正确性与接口设计阶段已实测）、整数溢出/除零/非法移位等数值类 UB（C 系 ph10 已按 C 语义覆盖，C++ 语义相同，本阶段不重复）和动态库边界的内存约定（[ph19 ABI、动态库与插件机制阶段](../ph19-abi-dynamic-libs-plugins/19-abi-dynamic-libs-plugins.md)）** — 那些是其他阶段的内容。承接 C 系 ph10（UB 的 C 语义与四档分类）、ph10 构建、调试与工具链阶段（ASan/UBSan 的编译/运行基础）、ph12（move 后状态约束、不返回局部引用、悬空引用的编译器告警）与 ph13/14（RAII 防 UAF；const 承诺被破坏是 UB）：C 系 ph10 回答了“C 里哪些写法是 UB”，本阶段回答“**C++ 多了哪些 UB、为什么、怎么用工具实测**”；ph12 讲的是“对象何时销毁”，本阶段讲“销毁后/移动后继续使用会怎样”；ph13 的 RAII 是本阶段大部分释放类 UB 的解药。
...
**UB 现场排查 SOP**（遇到"莫名崩溃/随机值/发布崩调试不崩"时按序走，把玄学变流程）：

| 步骤 | 做什么 | 用到的本阶段知识 |
|------|--------|-----------------|
| ① 最小化复现 | 把可疑代码抽到单文件、固定输入，能稳定触发最好 | 越界/UAF/竞争示例都是这样构造的（ex01~ex05） |
| ② 双档编译对照 | 同一份代码分别 `-O0 -g` 与 `-O2` 编译运行；**行为不同 → 高度指向 UB**（编译器按"无 UB"假设变了优化） | 4.1：const_cast O0 Bus error / O2 折叠打印 42 是同机制实证 |
| ③ 按类别选 Sanitizer | 越界/释放类 → ASan；别名/对齐/数值类 → UBSan；跨线程 → TSan（不能与 ASan 同进程） | 3.1~3.6 各小节"工具覆盖差异"表 |
| ④ 读报告定位 | 报告给出出错行号、访问大小（`WRITE of size 4`）、冲突双方（data race 的 T1/T2） | ex01~ex05 的实测报告关键行 |
| ⑤ 修复并验证 | 按对应小节的"正解"改（`at()`/reserve/RAII/`bit_cast`/锁），改后 Sanitizer 零报告 + 功能测试通过 | 3.1~3.6 每节的安全姿势段 |

两条纪律：**第 ② 步的"双档不一致"是 UB 的高置信信号**（不是玄学，是 4.1 的机制）；**Sanitizer 零报告 ≠ 无 UB**——严格别名与 union 双关 UBSan 就抓不到（3.6 实测），工具之后还要过一遍评审清单（project/ 的 `list` 正是把"工具抓不到的"列全了）。
...
- [ ] 能**按 UB 排查 SOP 处理疑难崩溃**：最小化复现 → `-O0`/`-O2` 双档对照 → 按类别选 ASan/UBSan/TSan → 读报告定位 → 修复后 Sanitizer 零报告（5 章 SOP 表）
