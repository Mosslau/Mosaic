...
本机实测输出要点（已验证）：`ar t` 列出 `ex02-libstat.o`、`ex02-libmath.o` 两个成员；`nm` 显示 `libex02.a(ex02-libstat.o): T __Z3gcdii ...` 与 `libex02.a(ex02-libmath.o): T __Z8is_primei ...`；链接后运行输出 `gcd(48,36)=12  lcm(6,8)=24` 断言 pass；可执行文件符号表确认 is_prime/factorial 未并入——**静态库按成员（.o）粒度按需抽取**。
