...
> ⚠️ const_cast 修改"真正 const"对象是未定义行为（ph15 未定义行为与内存安全阶段，目录待建，会系统归类）。本阶段只需记住：**先问 mutable 能不能解决，再问签名是不是该改，const_cast 是最后手段且要注释**。
...
4. **视图的生命周期必须短于数据源**：`string_view` / `span` 悬挂是借用式接口的延伸（ph12 讲过的借用心智），悬挂后解引用属 UB（ph15，目录待建）。
...
8. **视图生命周期必须短于数据源**：悬挂解引用是 UB（承接 ph12 借用心智，系统归类属 ph15，目录待建）
...
**ph15+（roadmap 第 15 节，目录待建）：未定义行为 UB 与内存安全阶段** — 本阶段是当前最后一个有目录的阶段（roadmap 第 15~23 节均为规划中，目录待建）。本阶段把"const 正确性"讲成接口设计语言（const 承诺 + 只读视图 + 逻辑/物理 const）；ph15 将系统梳理 const 承诺被破坏的后果——`const_cast` 修改真正 const 对象的 UB、视图/引用悬挂后的解引用、use-after-move、数据竞争等全部 UB 分类，并用 ASan/UBSan 实测。届时本阶段的"视图生命周期必须短于数据源""const 不变量是编译器的优化前提"会直接成为 ph15 判断"这行代码是否 UB"的依据。在 ph15 落地前，可先按 roadmap 第 15 节「学习内容」（越界访问、悬空引用、重复释放、数据竞争）自查 UB 直觉。
