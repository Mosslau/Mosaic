...
[Rule of 0/3/5 与 RAII 进阶阶段](../ph13-rule-of-035-raii/13-rule-of-035-raii.md) — ph13 目录已建，是当前最后一个有目录的阶段（roadmap 第 14~23 节均为规划中，目录待建）。本阶段讲清了"对象何时创建/移动/销毁、值类别如何分类、所有权如何通过类型体现"；ph13 将在此基础上深入资源类设计的完整规则——何时必须手写特殊成员函数、Rule of 0/3/5 的抉择、自定义 deleter、`unique_ptr` 管理 C 资源、异常安全下的资源释放；届时本阶段的"move 后状态约束""构造/析构顺序"会直接成为判断"这个类要不要手写析构"的依据。
