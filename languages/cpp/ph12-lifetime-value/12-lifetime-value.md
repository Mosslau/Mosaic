...
- **特殊成员函数手写与 Rule of 0/3/5 抉择**（ph13，目录待建）：本阶段只要求"能用移动承载所有权"，手写五函数与完整规则留给 ph13
- **const 正确性系统化**（ph14，目录待建）：本阶段的 `const&` 借用是"用到了 const"，不展开 const 成员函数/顶层底层 const 体系
- **UB 系统化梳理**（ph15，目录待建）：本阶段只处理与值类别/生命周期直接相关的悬空引用与 use-after-move 概念，不展开全部 UB 分类
...
**ph13+（roadmap 第 13 节，目录待建）：Rule of 0/3/5 与 RAII 进阶阶段** — 本阶段是当前最后一个有目录的阶段（roadmap 第 13~23 节均为规划中，目录待建）。本阶段讲清了"对象何时创建/移动/销毁、值类别如何分类、所有权如何通过类型体现"；ph13 将在此基础上深入资源类设计的完整规则——何时必须手写特殊成员函数、Rule of 0/3/5 的抉择、自定义 deleter、`unique_ptr` 管理 C 资源、异常安全下的资源释放；届时本阶段的"move 后状态约束""构造/析构顺序"会直接成为判断"这个类要不要手写析构"的依据。
