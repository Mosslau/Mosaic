...
这个阶段只涉及**对象级**的代码组织：roadmap §17 点名的四个模式与其直接变体、依赖注入、分层架构与模块边界、接口隔离与 pimpl、进程内同步的事件驱动设计，**不涉及模板/编译期形态的模式改写（CRTP、policy-based design——ph05 模板与泛型编程、元编程阶段）、并发与异步形态的事件驱动（线程安全通知、事件循环、actor——ph08 并发编程阶段）、模式引入的间接层的性能实测与基准（[ph18 性能优化与 Profiling 阶段](../ph18-perf-profiling/18-perf-profiling.md)）、跨动态库边界的稳定接口与插件 ABI（ph19 ABI、动态库与插件机制阶段，目录待建）、跨语言互操作层的接口设计（ph20 C++ 与 C / Python / Rust 互操作阶段，目录待建）和真实存储引擎内核（WAL/MemTable/SSTable/索引——ph22 存储引擎与数据库内核专项阶段，目录待建）** — 那些是其他阶段的内容。
