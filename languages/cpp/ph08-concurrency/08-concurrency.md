...
**范围边界**：本阶段承接 ph07 异常安全——锁与并发代码必须异常安全，RAII 锁在栈展开时自动解锁，线程函数内未捕获的异常会直接 terminate；**不涉及**文件/网络系统编程（ph09）、对象生命周期深入（ph12）、性能优化（ph18），也不展开无锁数据结构与复杂内存序优化（后续阶段）——本阶段默认使用 seq_cst 与基本 acquire/release，把程序写对是首要目标。
...
- 传递引用参数必须用 `std::ref`（否则按值拷贝）；线程函数抛出未捕获异常也会 terminate（见 ph07）
...
int main() {
    ...
    x.join(); y.join();           // 永远等不到：用 timeout 2 ./ex 观察挂起
    return 0;
}
```
要点：
...
结论：**协程不是线程、也不是线程的替代**——它不涉及调度器，挂起不阻塞线程；它是"用同步的写法表达异步流程"的工具，与线程、atomic、同步原语互补（ph09 异步 IO 阶段再深入）。
...
**不适合**此阶段的事项：
- **文件/网络系统编程**（ph09）：socket、进程间通信、epoll 等异步 IO 模型——本阶段只谈进程内多线程
- **复杂内存序优化与无锁数据结构**（后续阶段 / ph18）：本阶段默认 seq_cst 与基本 acquire/release，先把正确性做对再谈性能
- **跨进程并发**：多进程共享内存、跨进程信号量等留给 ph09 的 IPC
- 分布式一致性与并发算法理论（Raft / Paxos 等）：超出本阶段范围

## 6. 代码示例
### 示例 1：多线程计数器（mutex 保护 + 对比 atomic）
```cpp
// 编译：g++ -std=c++20 -pthread ex1_counter.cpp -o ex1
#include <atomic>
#include <iostream>
#include <mutex>
#include <thread>
...
```cpp
// 编译：g++ -std=c++20 -pthread ex2_queue.cpp -o ex2
#include <condition_variable>
#include <deque>
#include <iostream>
#include <mutex>
...
```cpp
// 编译：g++ -std=c++20 -pthread ex3_pc.cpp -o ex3
#include <condition_variable>
#include <iostream>
#include <mutex>
#include <queue>
...
```cpp
// 编译：g++ -std=c++20 -pthread ex4_pool.cpp -o ex4
#include <condition_variable>
#include <functional>
#include <iostream>
#include <mutex>
...
```cpp
// 编译：g++ -std=c++20 -pthread ex5_jthread.cpp -o ex5
#include <chrono>
#include <iostream>
#include <thread>
...
### 进入下一阶段前
确保能完成以下练习：
- **多线程计数器**：8 个线程各累加 10 万次，分别用 mutex 与 atomic 实现并验证结果等于 80 万（提示：结果不对 = 数据竞争，用 `-fsanitize=thread` 复现定位）
- **线程安全队列**：实现 push / pop（非阻塞）/ wait_pop（阻塞）三个接口（提示：wait_pop 用 `cv.wait(lock, pred)` 而非裸 wait）
- **生产者消费者**：容量 4 的有界队列，生产 20 个、消费 20 个（提示：满时生产者等待需要第二个 cv：not_full_）
- **简单线程池**：固定 N 个 worker + 任务队列，submit 任意函数（提示：析构顺序是置 stop_ → notify_all → join；worker 退出条件 `stop_ && tasks_.empty()`）
- **jthread + stop_token 可取消任务**：周期任务每 200ms 打印一次，收到停止请求后打印清理日志并退出（提示：线程函数第一个参数写 `std::stop_token`）
- 进阶（可选）：用 std::latch 实现"N 个阶段任务全部完成才继续"，用 counting_semaphore 模拟连接池限流
### 推荐项目
