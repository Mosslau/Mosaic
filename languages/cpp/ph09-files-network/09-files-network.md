...
> 面向高性能系统、存储引擎方向，本阶段能用 C++ 写真实系统程序——从 fstream 文件读写、JSON 配置解析，到 socket 网络编程、Linux 系统调用与动态库插件机制，让代码真正"落地到操作系统"。

## 1. 概述
本阶段定位：**能用 C++ 写真实系统程序——用 fstream/std::filesystem 读写文件与目录，用 nlohmann/json 解析配置，用 POSIX socket 实现 TCP/HTTP 通信，用 RAII 封装系统资源（文件、socket、fd），并掌握进程、动态库与插件机制**。学完后能写出带完整错误处理的日志系统、TCP echo server、HTTP server 与文件传输工具，能说清"IO 为什么会失败、配置错误怎么报、粘包怎么解、插件接口为什么必须稳定"。

| 核心维度 | 覆盖内容 |
|----------|---------|
| 文本与二进制文件 | fstream（ifstream/ofstream）、read/write、字节序 |
| 文件系统 | std::filesystem（路径、遍历、复制，C++17） |
| 配置解析 | nlohmann/json、key=value 解析、可诊断错误 |
...
**范围边界**：本阶段承接 ph08 并发——多线程程序要把日志刷盘、socket 收发放进独立线程；**不涉及**构建调试工具链（ph10）、对象生命周期深入（ph12）、ABI 与插件机制深入（ph19），也不展开 epoll/io_uring 异步 IO 与高性能网络框架（后续阶段）——本阶段用阻塞 socket 把协议写对，是异步化的前提。
...
- **坑：`read` 可能读不满**（EOF 提前）——实际读入字节数用 `in.gcount()` 查询（见示例 5）
...
- `recursive_directory_iterator` 递归遍历目录树；`fs::last_write_time` 拿修改时间，是文件同步工具做差异比对的基础
- **坑：路径编码**——Linux 文件名是任意字节序列，`fs::path` 用平台编码（约定 UTF-8），来自用户输入/网络的路径要先校验；**路径拼接永远用 `operator/`，不要手拼字符串**

### 3.4 JSON 与配置文件解析（nlohmann/json 入门，语义保留）
JSON 不在 C++ 标准内，**nlohmann/json** 是事实标准：单头文件、类型安全访问、解析失败抛带字节偏移的 `json::parse_error`。配置解析的硬要求是**可诊断错误**——报错必须说清"哪个文件、哪一行/哪个字节、为什么"。
```cpp
#include <fstream>
#include <iostream>
#include <nlohmann/json.hpp>   // 单头文件库：apt install nlohmann-json3-dev 等
using json = nlohmann::json;
int main() {
    const std::string path = "/tmp/ph09_conf.json";
    json cfg = {{"server", {{"port", 8080}}}, {"workers", 4}};
    std::ofstream out(path);
    out << cfg.dump(4) << "\n";                 // dump(4)：语义保留的缩进输出
    try {
        std::ifstream in(path);
        json loaded = json::parse(in);          // 解析失败抛 json::parse_error
        std::cout << "port=" << loaded["server"]["port"].get<int>()
                  << " workers=" << loaded["workers"].get<int>() << "\n";
...
```
要点：
- **必会概念：配置解析要给出可诊断错误**——`json::parse_error::what()` 带字节偏移，`json::type_error` 说明键的类型不对
- **坑：错误信息差**——只报 "parse failed" 等于没报；好错误 = 文件路径 + 位置 + 期望 + 实际值
- **坑：裸 `[]` 访问不存在的键会插入空值**——读配置用 `cfg.contains()` 或 `cfg.value(key, 默认值)`；`at()` 越界抛异常更安全
- **语义保留**：`dump()`/`parse()` 往返不丢数据；要保键的插入顺序用 `nlohmann::ordered_json`（默认 json 底层是 std::map，会按键排序）
### 3.5 socket 编程基础（POSIX socket + RAII 封装）
POSIX socket 用裸 `int fd` 表达连接，**必须用 RAII 类封装**：构造时拿 fd、析构时 `close()`、禁拷贝允移动。核心流程：服务端 `socket → bind → listen → accept`，客户端 `socket → connect`，之后双方 `recv`/`send`（完整流程见示例 2）。
```cpp
#include <iostream>
...
int main() { return 0; }   // 自环测试可用 socketpair；实战调用见示例 2 与 4.3
```
要点：
...
- **坑：一次 recv 拿不到完整响应**——示例只读一次是教学简化；**生产用 libcurl**（`-lcurl`）处理重定向、TLS、超时、cookie，手写 HTTP 只用于学习和极简内部协议（server 侧见示例 3）
...
int main() {
    PosixFile f("/tmp/ph09_posix.txt", O_CREAT | O_RDWR | O_TRUNC);
    const char* msg = "via posix\n";
    ::write(f.fd(), msg, std::strlen(msg));
    return 0;
}
```
要点：
...
- **std::thread 不能表达"换一个程序"**——执行外部命令、内存隔离、崩溃互不牵连只能用进程；进程间通信（IPC）本阶段掌握 socket 即可

### 3.10 动态库与插件机制入门（dlopen/dlsym）
插件机制 = **运行时加载动态库**：Linux 用 `dlopen/dlsym/dlclose`（glibc 2.34+ 已并入 libc），Windows 用 `LoadLibrary/GetProcAddress`。核心纪律：**插件接口必须稳定**——用 `extern "C"` 关掉 C++ 名字修饰（name mangling），用固定签名、固定结构体布局作为插件 ABI。
```cpp
// 编译：g++ -std=c++17 ex10_plugin.cpp -o ex10
// 插件侧：extern "C" int plugin_add(int a, int b) { return a + b; }
//         g++ -std=c++17 -shared -fPIC plugin.cpp -o libplugin.so
#include <dlfcn.h>
#include <iostream>
...
| 非阻塞 + epoll | epoll 事件驱动 | 内核通知就绪 | 高并发服务器（后续阶段） |

设置超时：`struct timeval tv{tv_sec, 0}; setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));`（`SO_SNDTIMEO` 同理管发送）；超时后 `recv` 返回 -1 且 `errno == EAGAIN || errno == EWOULDBLOCK`。
要点：
...
└───────── 消息 1 ─────────┘
```
实现要点：长度用 `ntohl` 转主机字节序（写端 `htonl`）；**长度必须校验**（如上限 16MB），否则恶意/损坏的头部会让 recv_full 尝试读巨量字节；示例 2 的升级方向就是把"读到 \n 即一条消息"改成 length-prefix。
...
| 程序配置加载 | nlohmann/json / key=value 解析、可诊断错误 |
...
**不适合**此阶段的事项：
- **构建、调试与工具链**（ph10）：CMake 深入、vcpkg/conan 依赖管理、Sanitizer、性能分析工具——本阶段示例全部单文件 `g++` 编译即可
- **ABI 与插件机制深入**（ph19）：符号版本控制、visibility 精细管理、跨语言异常边界——本阶段掌握"接口要稳定 + 版本号"的纪律
- **高性能网络框架**（后续阶段）：epoll 事件驱动、io_uring、Boost.Asio 异步模型——本阶段用阻塞 socket + 超时把协议写对，是异步化的前提
- **分布式系统**（后续阶段）：跨主机一致性、服务发现、RPC 框架——本阶段只做单机进程间/本机网络通信

## 6. 代码示例
### 示例 1：日志文件系统（fstream 追加 + 时间戳 + 错误处理）
```cpp
// 编译：g++ -std=c++17 ex1_logfs.cpp -o ex1
#include <chrono>
#include <ctime>
#include <fstream>
#include <iostream>
#include <stdexcept>
#include <string>
class LogFile {                       // 日志文件系统：追加 + 时间戳 + 错误处理
public:
    ...
    void write(const std::string& level, const std::string& msg) {
        ...
        if (!out_) throw std::runtime_error("log write failed");   // 失败必报
    }
private:
...
```
### 示例 2：TCP echo server（RAII 封装 socket + 客户端测试）
```cpp
// 编译：g++ -std=c++17 ex2_echo.cpp -o ex2
// 用法：./ex2 server 9000   （另开终端）./ex2 client 9000
#include <arpa/inet.h>
#include <cstring>
#include <iostream>
#include <stdexcept>
...
static void run_server(int port) {
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            ::send(conn.fd(), buf, n, MSG_NOSIGNAL); // 原样回显
        }
    }
}
static void run_client(int port) {
    ...
    ::send(c.fd(), msg, std::strlen(msg), 0);
    char buf[128];
...
int main(int argc, char** argv) {
    if (argc < 3) return 1;
    try {
        if (std::string(argv[1]) == "server") run_server(std::stoi(argv[2]));
        else run_client(std::stoi(argv[2]));
    } catch (const std::exception& e) {
...
```
### 示例 3：简单 HTTP server（解析请求行 + 返回响应）
```cpp
// 编译：g++ -std=c++17 ex3_http.cpp -o ex3
// 测试：./ex3 后访问 http://127.0.0.1:8080/
#include <arpa/inet.h>
#include <cstring>
#include <iostream>
#include <string>
...
int main() {
    ...
    if (::bind(fd, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0 ||
        ::listen(fd, 16) < 0) { std::cerr << "bind/listen failed\n"; return 1; }
    std::cout << "http server on 8080 (Ctrl-C to stop)\n";
    for (;;) {
        ...
        size_t sp1 = line.find(' ');                           // 方法结束
        size_t sp2 = line.find(' ', sp1 + 1);                  // 路径结束
        if (line.empty() || sp1 == std::string::npos ||
            sp2 == std::string::npos) { ::close(c); continue; }
        std::string method = line.substr(0, sp1);
        std::string path = line.substr(sp1 + 1, sp2 - sp1 - 1);
        std::string body = "<html><body><h1>ph09 http server</h1>"
                           "<p>" + method + " " + path + "</p></body></html>\n";
...
```cpp
// 编译：g++ -std=c++17 ex4_config.cpp -o ex4
#include <fstream>
#include <iostream>
#include <map>
#include <stdexcept>
...
public:
    ...
    int get_int(const std::string& key, int def) const {
        ...
        catch (...) { throw ConfigError("", 0, "key '" + key +
                                               "' value '" + it->second +
                                               "' is not an int"); }
    }
private:
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::cerr << "config error: " << e.what() << "\n";   // 例：xxx.conf:3: expected key=value
        return 1;
    }
    return 0;
}
```
### 示例 5：文件传输工具（二进制读写 + 分块 + 校验）
```cpp
// 编译：g++ -std=c++17 ex5_filexfer.cpp -o ex5
// 用法：./ex5 send <src> <dst>  或  ./ex5 recv <src> <dst>
#include <cstdint>
#include <fstream>
#include <iostream>
#include <string>
...
static bool copy_chunked(const std::string& src, const std::string& dst,
    ...
    for (;;) {
        ...
        std::streamsize n = in.gcount();           // 实际读到的字节数（可能不满）
        if (n > 0) {
            out.write(buf, n);
...
10. **阻塞模型先写对再谈性能**：本阶段用阻塞 socket + 超时把协议写对，epoll/io_uring 是后续阶段的事
### 跨语言对比：文件与网络编程
| 维度 | C++ | C | Go | Java | Rust |
|------|-----|---|----|------|------|
...
| 配置/序列化 | nlohmann::json | cJSON / jansson | encoding/json | Jackson / Gson | serde_json |
...
- 能设计**基础配置模块**：JSON 或 key=value 解析，支持注释与类型转换，错误带文件与行号
- 能处理**网络协议边界**：recv/send 循环 + length-prefix 解决粘包/半包，socket 带超时
- 能写出**稳定插件接口**：extern "C" + 固定签名 + 版本号检查
- 能独立完成四个练习：日志文件系统、TCP echo server、简单 HTTP server、文件传输工具
### 进入下一阶段前
确保能完成以下练习：
- **日志文件系统**：追加模式 + 时间戳 + 级别过滤 + 写后错误检查（提示：`std::ios::app` 打开、每次 write 后查 `!out`、`'\n'` 与 `flush()` 区分使用）
- **TCP echo server**：RAII 封装 socket + recv/send 循环（提示：send 加 `MSG_NOSIGNAL` 防 SIGPIPE；先单连接，进阶用 thread-per-connection 支持多客户端）
- **简单 HTTP server**：解析请求行 + 返回带 Content-Length 的响应（提示：先处理 `GET /path HTTP/1.1`；进阶：按 Content-Length 循环读完整 body、区分 200/404）
- **文件传输工具**：二进制分块读写 + 校验和（提示：`in.read(buf, n)` + `in.gcount()` 处理不满块；用 FNV/CRC 校验，接收端验证）
- **配置文件解析**：key=value 或 JSON，错误带行号/字节偏移（提示：错误消息统一格式 `文件:行号: 原因`；JSON 用 `json::parse` 的异常 + `what()`）
- 进阶（可选）：给 echo server 加 length-prefix 帧格式；用 fork + exec 实现"命令执行器"；用 std::filesystem 递归统计目录大小
### 推荐项目
