// ex03-http.cpp —— 简单 HTTP server：解析请求行 + 200/404 响应（Content-Length）
// 验证环境：Apple clang 21（g++ 兼容），C++20
// 编译：c++ -std=c++20 -Wall -Wextra ex03-http.cpp -o ex03
// 运行：
//   ./ex03 selftest               —— 自动验证：fork 子进程发 GET / 与 GET /nope
//   ./ex03 <port>                 —— 手动模式：起服务，浏览器访问 http://127.0.0.1:<port>/
...
// 写满 n 字节；MSG_NOSIGNAL 防对端关闭后 SIGPIPE 杀进程
...
static void handle_connection(int fd) {
    ...
    if (line.empty() || sp1 == std::string::npos ||
        sp2 == std::string::npos) return;                        // 非法请求，直接关闭
    ...
    if (path == "/") {
        status = "200 OK";
        body = "<html><body><h1>ph09 http server</h1>"
               "<p>GET / 200 OK</p></body></html>\n";
    }
    const std::string resp = make_response(status, "text/html", body);
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "usage: ./ex03 selftest | <port>\n";
        return 1;
    }
    try {
        if (std::string(argv[1]) == "selftest") return run_selftest();
        if (argc >= 2) return run_server(std::stoi(argv[1]));
        return 1;
    } catch (const std::exception& e) {
