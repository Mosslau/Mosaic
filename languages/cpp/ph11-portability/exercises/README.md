...
- **提示**：**坑：不要用 `reinterpret_cast` 直接把结构体当字节流**——结构体布局（padding/对齐）跨编译器不同，序列化必须逐字节手动拼；大端序正是网络协议（TCP/IP 协议头、HTTP/2 帧头长度字段）的约定；HTTP/1.1 的 Content-Length 是 ASCII 十进制文本，不是二进制大端
