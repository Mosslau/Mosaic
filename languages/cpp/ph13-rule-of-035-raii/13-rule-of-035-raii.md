...
| 拷贝构造 | 用户没声明任何拷贝/移动/析构 | 逐成员拷贝（浅拷贝！） | 裸资源需要深拷贝时 |
| 拷贝赋值 | 同上 | 逐成员拷贝赋值 | 同上 |
| 移动构造 | 用户没声明拷贝/析构/移动赋值 | 逐成员移动 | 持有裸资源需要转移所有权时 |
| 移动赋值 | 用户没声明拷贝/析构/移动构造 | 逐成员移动赋值 | 同上 |

**关键规则**：用户一旦声明了其中任何一个（尤其析构），**编译器就不再隐式生成其他特殊成员**（拷贝/移动会被隐式"禁用"直到显式声明）。这就是"写了析构却没写拷贝"的类，拷贝会退化为逐成员浅拷贝的原因——`ex02` 的 `-DPH13_SHALLOW` 对照实验用 ASan 实测了这个后果：**double-free**。
...
4. **被移动的对象处于"空句柄"态**——析构时判空，`fclose(nullptr)` / `close(-1)` 是 no-op，杜绝 double-free。
...
**要点**：`open_file` 按值返回 `FilePtr` 是 C++17 保证省略下的零拷贝所有权产出（衔接 ph12）；异常路径下 deleter 在栈展开时自动调用（`ex05` [3] 实测：`close(fd=3)` 先于 `catch`）；malloc 内存同样可以交给 `unique_ptr` 托管。这正是 roadmap §13 示例 `using FilePtr = std::unique_ptr<FILE, decltype(&fclose)>;` 的完整展开。
...
```text
用户声明了析构 / 拷贝 / 移动 中的任意一个？
        │
        ├── 是 ──▶ 不再隐式生成"未声明的其他特殊成员"
        │            （拷贝/移动被隐式禁用，直到显式 =default / 手写）
        │
        └── 否 ──▶ 按需生成全部五个
                     │
...
这就是为什么"写了析构忘了拷贝"会静默退化为浅拷贝：编译器看到你声明了析构，就不再为你生成拷贝构造——**拷贝变成逐成员指针复制**，两个对象持有同一块内存，作用域结束 double free（`ex02` 的 ASan 实测）。理解这条规则，Rule of 5 就不再是"教条"而是"编译器规则的直接推论"。
...
实测要点：`Widget` 成员全是值类型，编译器生成的拷贝是**逐成员深拷贝**（`b` 改 `data` 不影响 `a`）、移动是**逐成员移动**（源置空）；[1] 按值返回 0 拷贝 0 移动（ph12 保证省略）。
...
1. **五个特殊成员函数**（析构/拷贝构造/拷贝赋值/移动构造/移动赋值）是编译器给资源管理的默认实现；**用户声明其中任意一个，其余不再隐式生成**——"写了析构忘了拷贝"会静默退化为浅拷贝
...
8. **`unique_ptr` 管理 C 资源**：`decltype(&std::fclose)` / `&std::free` 直接作 deleter；按值返回是保证省略下的零拷贝所有权产出（衔接 ph12）
...
本阶段练习见 [`exercises/`](./exercises/)（题目在 exercises/README.md，参考实现 sol-* 先别看），共 5 题：判断类是否需要自定义特殊成员函数（★）、把 FILE* 封装成 RAII 类（★★）、把 POSIX fd 封装成 RAII 句柄并用 fcntl 实测关闭（★★★）、改造手动释放资源的旧代码（★★★）、异常安全的资源类（★★★）——练习 1/2/3 与 roadmap ph13「练习」小节的三个承诺一一对应，练习 4/5 覆盖「学习内容」中的 Rule 抉择与异常安全资源释放。完成 5 题后继续。

### 阶段项目

本阶段综合项目见 [`project/`](./project/)：**RAII 系统资源库（安全句柄封装）**——header-only 的 `raii/` 库（`CFile` 封装 `FILE*` + `UniqueFd` 封装 POSIX fd）+ 演示 CLI（write/read/copy 三子命令）+ 自测（编译期 `static_assert` move-only + 16 组运行期断言，含**异常路径 fd 计数归零**实测；普通版 + ASan 版）+ Makefile，是 roadmap 推荐项目①「RAII 系统资源库」的落地（推荐项目②「安全句柄封装」即本库形态）。建议完成练习后再动手。
...
**ph14+（roadmap 第 14 节，目录待建）：const 正确性与接口设计阶段** — 本阶段是当前最后一个有目录的阶段（roadmap 第 14~23 节均为规划中，目录待建）。本阶段把"资源类的内部实现"讲透（特殊成员函数、RAII、异常安全）；ph14 将在此基础上把视角转向**接口设计**——const 成员函数、顶层/底层 const、`const&` 参数、逻辑 const 与物理 const，把"资源类不变量"用 const 表达成接口承诺；届时本阶段的"move-only 类为什么天然 const 友好""RAII 类的接口如何设计"会直接成为判断"这个接口要不要标 const"的依据。在 ph14 落地前，可先按 roadmap 第 14 节「推荐项目」（配置读取只读接口 / 设备状态快照模型）自查 const 直觉。
