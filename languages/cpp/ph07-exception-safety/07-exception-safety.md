...
本文示例以 **C++17** 为基线（异常安全体系成熟于 C++11 后、noexcept 成函数类型一部分），验证工具链 Apple clang 17（g++ 兼容）。
...
> 说明：每个示例的完整可运行文件在 [`examples/`](./examples/) 目录（ex01~ex05，与下面示例 1~5 一一对应），验证环境 Apple clang 17（g++ 兼容），编译命令统一 `g++ -Wall -Wextra -std=c++17`（示例 4 需 `-pthread`，命令见 examples/README.md）。
...
完整文件：`examples/ex01-raii-file.cpp`
...
完整文件：`examples/ex02-copy-swap.cpp`
...
完整文件：`examples/ex03-error-layering.cpp`
...
完整文件：`examples/ex04-logger.cpp`（需 `-pthread`）
...
本阶段练习见 [`exercises/`](./exercises/)（题目在 exercises/README.md，参考实现 sol-* 先别看）：日志模块、配置读取模块、错误码体系、给已有代码补单元测试，共 4 题。完成 4 题后继续。

### 阶段项目

本阶段综合项目见 [`project/`](./project/)：**工程化配置库**——INI/键值解析、类型转换与默认值、错误码体系 + 异常分层、日志接入、头文件规范与命名空间分层，覆盖本阶段几乎全部知识点。
