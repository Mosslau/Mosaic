...
这个阶段只涉及 **JVM/JMM/并发底层机制与大型工程的通用模式与建模方法**，**不涉及车联网业务的整合落地（车辆接入协议、告警规则引擎、OTA 平台、轨迹查询如何用这些机制实现）** — 那是 ph21 车联网 / 智能电动车方向 Java 阶段（roadmap 第 21 节，目录待建），本阶段只在 DDD 与场景两处以「车联网将用到」点题不展开。**ph10 已讲过的 JVM 基础入门（运行时数据区五件套、GC Roots、类加载五阶段流程、jps/jstack/jmap/jstat 的命令用法）不重复展开，本阶段只在其之上做机制深水与工程化增量**；**ph09 已讲过的并发入门（synchronized/volatile 基本用法、ReentrantLock 用法、线程池七参数怎么配、虚拟线程怎么用）同样不重复，只深入其源码与模型**。设计模式部分**不重讲 GoF 每种模式的语法示例**（那些收进模式索引一张表，重点是「JDK 里长什么样」），DDD **不涉及具体行业业务规则**。
...
**ph21 车联网 / 智能电动车方向 Java 阶段**（roadmap 第 21 节，目录待建）——本阶段积累的所有机制将汇入真实业务：JMM/并发（车辆数据接入的高吞吐）、CHM/线程池（实时状态缓存与接入服务）、反射/代理/SPI（设备协议插拔与规则引擎）、Netty（车载长连接网关的底层形态）、DDD 的限界上下文（设备管理 / OTA / 告警规则按领域切分）——到 ph21 把这些机制按「车联网数据链路：车辆 → 接入服务 → Kafka → 清洗/告警/存储 → 后台平台」组织成完整系统。
