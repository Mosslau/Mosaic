...
ph18 缓存与高并发阶段（roadmap 第 18 节，目录待建，届时以 roadmap 为准）——本阶段用 MQ 削峰解耦、用 ES 扛检索，下一层防护是缓存与限流：Redis/Caffeine 缓存穿透/击穿/雪崩、分布式锁与限流、秒杀架构、连接池与批量优化。本阶段埋的前置：消息队列只是把「瞬时洪峰」摊平成「匀速消费」，消费端背后的数据库与搜索仍可能被打穿——ph18 的缓存分层（本地缓存 → Redis → 数据库）就是消费链路的下一道防线；roadmap 第 21 节的车辆数据实时缓存也建立在本阶段车辆数据消费练习之上。
