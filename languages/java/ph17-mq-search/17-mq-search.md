...
- **通用数据接入（ph21 铺垫）**：roadmap 第 21 节的 `数据源 → 接入服务 → Kafka → 清洗/告警/存储 → 运维后台` 与本阶段练习 2（车辆数据消费）同构——上报量大、多消费者（实时监控/告警/历史存储各一组）、按车辆 sn 分区保证单车有序。
