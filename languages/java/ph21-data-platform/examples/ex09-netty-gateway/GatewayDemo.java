// examples/ex09-netty-gateway/GatewayDemo.java —— 网关集成演示：JDK Socket 模拟车端注册 + 心跳
...
//
// 车端侧故意用纯 JDK Socket 而非 Netty client——证明「网关是 Netty、车端协议是行文本」，
...
public final class GatewayDemo {
    public static void main(String[] args) throws Exception {
        ...
        try (Socket s = new Socket("127.0.0.1", port)) {
            ...
            check(pass, send(in, out, "REG|LSV0000001").startsWith("ACK"), "车端 REG 注册被网关接受");
            check(pass, send(in, out, "HB|LSV0000001|10").startsWith("ACK"), "在线车心跳被接受");
            check(pass, send(in, out, "HB|LSV0000001|20").startsWith("ACK"), "心跳序号推进到 20");
            check(pass, send(in, out, "HB|LSV0000999|5").startsWith("ERR"), "未注册 VIN 心跳被拒(先 REG 后 HB)");
            check(pass, send(in, out, "GARBAGE").startsWith("ERR"), "坏命令返回 ERR");
        } finally {
            gateway.stop();
        }
        check(pass, gateway.onlineCount() == 1, "在线表恰好 1 台车(未注册心跳不产生条目)");
