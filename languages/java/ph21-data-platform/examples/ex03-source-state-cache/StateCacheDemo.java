// examples/ex03-source-state-cache/StateCacheDemo.java —— 并发上报实时状态缓存演示
// 验证环境：OpenJDK 17.0.18(Homebrew)，命令：
//   javac -encoding UTF-8 -d /tmp/tl21-cls *.java
//   java -cp /tmp/tl21-cls StateCacheDemo
// 期望：两路网关对同一 SOURCE_ID 乱序上报，缓存最终收敛到最大 seq，且丢弃计数 > 0 证明原子比较生效。
...
public final class StateCacheDemo {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int v = 1; v <= VIN_COUNT; v++) {
            ...
            writers.add(new Thread(() -> pump(cache, sourceId, SEQ_MAX, 30), "gw-B-" + sourceId));
        }
        ...
        check(pass, cache.staleDropped() > 0, "乱序旧帧被丢弃计数=" + cache.staleDropped()
                + " > 0(原子比较真的在工作)");
        check(pass, cache.snapshot().stream().allMatch(s -> s.cpuPct() >= 0 && s.cpuPct() <= 100),
                "快照中的状态物理量都在量程内(无半写脏数据)");
        System.out.println("末帧样例: " + cache.get("LSV" + String.format("%06d", 1)));
        System.out.printf("ALL PASS: %d/4%n", pass.get());
    }

    private static void pump(SourceStateCache cache, String sourceId, int max, long preDelayMs) {
        if (preDelayMs > 0) {
            try {
        ...
        for (long seq = 1; seq <= max; seq++) {
            cache.update(SourceState.of(sourceId, seq, 30 + (seq % 70)));
        }
    }
