// examples/ex02-fake-vehicle-ingest/IngestService.java —— 接入服务：按 VIN 分片的有界队列 + 单消费者保序
// 验证环境：OpenJDK 17.0.18(Homebrew)，命令：javac -encoding UTF-8 -d /tmp/tl21-cls *.java
//
// 教学点（兑现 ph20 线程池/JMM 预告 + 车联网数据链路的核心纪律）：
//   「同一辆车的帧必须有序」决定了接入层不能用「多个 worker 抢一条队」——那会并发乱序，
//   把稍晚到达的低 seq 帧误判成重复。正确形态是**按 VIN 哈希路由到 lane**：
//   每个 lane 一条有界队列 + 一个消费者线程，同 VIN 永远进同一 lane → 帧内严格 FIFO，
//   多 VIN 之间又能并行消费(与 Kafka 按 key 分区、Netty 连接绑定 EventLoop 同一哲学)。
//
//   1. lane 队列有界：生产(车端线程)与消费解耦，队满 = 该 lane 过载，丢弃并计数(背压)；
//   2. 每 lane 一个常驻线程：同 VIN 顺序处理，去重逻辑因此只需比较「上一条 seq」；
...
public final class IngestService {
    ...
    private final AtomicLong duplicates = new AtomicLong(); // 重传/乱序帧(同 VIN 内)
    private final AtomicLong backlogDropped = new AtomicLong();  // 队满丢弃(背压)
    private volatile boolean running = true;

    /** lane 内的单消费者：自己的队列 + 每 VIN 独立基线，天然无竞争。 */
    private final class Lane implements Runnable {
        ...
        private void process(String raw) {
            TelemetryFrame frame = FrameCodec.decode(raw).orElse(null);
            if (frame == null) {
                rejected.incrementAndGet();
                return;
            }
            // 单线程、per-VIN 独立序号：同车帧即使被其它车的帧隔开，判重也不受干扰
            long last = lastSeqByVin.getOrDefault(frame.vin(), -1L);
            if (frame.seq() <= last) {
                duplicates.incrementAndGet();   // 重传/乱序：丢弃
                return;
            }
            lastSeqByVin.put(frame.vin(), frame.seq());
    ...
    public void submit(String rawLine) {
        ...
        if (!lane.offer(rawLine)) {
            backlogDropped.incrementAndGet();   // 队满：丢弃并计数(可接受延迟类遥测才这么干)
        }
    }

    /** 提取路由键：正常帧取 VIN，坏帧取固定常量(仍会走解析被拒)。 */
    ...
    private static void maybePublish(TelemetryFrame f) {
        // 真实场景在此序列化并发往 Kafka topic(按 vin 做分区键)
    }
}
