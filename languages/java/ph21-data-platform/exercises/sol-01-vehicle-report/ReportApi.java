// exercises/sol-01-vehicle-report/ReportApi.java —— 车辆数据上报 API(参考实现)
// 验证环境：OpenJDK 17.0.18(Homebrew)，命令：javac -encoding UTF-8 -d /tmp/tl21-sol *.java
//
// 题目要求(roadmap §21 练习「车辆数据上报 API」)：
//   - 实现车辆上报 API 的判定核心：非法帧拒绝、越界帧拒绝、每车乱序去重；
//   - 线程安全：多路网关并发上报同一 VIN 不丢帧、不乱序覆写；
//   - 可查询：能回答某车当前状态、全平台处理统计。
...
public final class ReportApi {
    ...
    private final ConcurrentHashMap<String, VehicleReport> latestByVin = new ConcurrentHashMap<>();
    ...
    public Verdict handle(VehicleReport report) {
        if (!report.vinValid()) {
        ...
        latestByVin.compute(report.vin(), (vin, cur) -> {
    ...
    public VehicleReport latest(String vin)          { return latestByVin.get(vin); }
