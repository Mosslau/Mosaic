...
> **与 roadmap「21. 数据平台 / 数据中心后端方向 Java 阶段」练习小节的对应**：四题一一对应 roadmap 列出的四个练习——练习 1 = 数据源数据上报 API、练习 2 = 节点管理系统、练习 3 = 版本发布 升级平台、练习 4 = Kafka 指标消费服务。
...
| 3 版本发布 升级平台 | sol-03-release-platform/ | 版本库/批次编排/节点状态机/审计（3.6） | ex06 |
...
**目标**：做一个含版本库的 版本发布 平台：版本只能递进发布，批次绑定已发布版本，批内每个数据源独立状态机并可并发推进。
**要求**：
...
全部通过后继续到 [`project/`](../project/)：数据平台后台平台(收官项目)。
