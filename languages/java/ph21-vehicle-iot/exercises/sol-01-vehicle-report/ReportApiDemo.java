// exercises/sol-01-vehicle-report/ReportApiDemo.java —— 上报 API 验收演示
// 验证环境：OpenJDK 17.0.18(Homebrew)，命令：javac -encoding UTF-8 -d /tmp/tl21-sol *.java
//   然后 java -cp /tmp/tl21-sol ReportApiDemo
// 场景：单 VIN 双路并发各报 1000 帧(seq 都从 1 到 1000)，坏 VIN/越界帧混入；
...
public final class ReportApiDemo {
    private static final String VIN = "LSV0000001";
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        check(pass, api.latest(VIN) != null && api.latest(VIN).seq() == 1000,
                "最终状态收敛到 seq=1000(旧帧未覆写新帧)");

        // 2) 坏帧：非法 VIN / SOC 越界
        check(pass, api.handle(new VehicleReport("VIN", 1, 50, 60, 40)) == ReportApi.Verdict.REJECT_INVALID_VIN,
                "非法 VIN 被拒");
        check(pass, api.handle(new VehicleReport(VIN, 1001, 105.0, 60, 40)) == ReportApi.Verdict.REJECT_RANGE,
                "SOC 越界被拒");
        check(pass, api.handle(new VehicleReport(VIN, 5, 50, 60, 40)) == ReportApi.Verdict.DUPLICATE,
                "旧 seq 帧被判重复");
    ...
    private static void pump(ReportApi api, List<ReportApi.Verdict> out, int startDelayMs) {
        ...
        for (long seq = 1; seq <= 1000; seq++) {
            out.add(api.handle(new VehicleReport(VIN, seq, 50.0, 60.0, 40.0)));
        }
    }
