// exercises/sol-04-telemetry-consumer/ConsumerGroupDemo.java —— Kafka 遥测消费服务验收演示
// 验证环境：OpenJDK 17.0.18(Homebrew)，命令：javac -encoding UTF-8 -d /tmp/tl21-sol *.java
//   然后 java -cp /tmp/tl21-sol ConsumerGroupDemo
// 场景：300 台车 × seq 1..2 = 600 条唯一消息(按 VIN 哈希入 4 分区)，其中 200 条故意重复投递；
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int v = 1; v <= VIN_COUNT; v++) {
            ...
            for (long seq = 1; seq <= 2; seq++) {
                kafka.produce(new TelemetryMsg(vin, seq, 40 + seq));
                if (v % 3 == 0 && seq == 2) {      // 每 3 台车重发最后一条
                    kafka.produce(new TelemetryMsg(vin, seq, 40 + seq));
                    dup++;
                }
            }
