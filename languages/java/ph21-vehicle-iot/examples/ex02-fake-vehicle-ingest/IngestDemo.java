// examples/ex02-fake-vehicle-ingest/IngestDemo.java —— 接入层吞吐演示主入口
// 验证环境：OpenJDK 17.0.18(Homebrew)，命令：
//   javac -encoding UTF-8 -d /tmp/tl21-cls *.java
//   java -cp /tmp/tl21-cls IngestDemo
//
// 场景一(正常负载)：队列容量充足，10 台车 × 300 帧，期望零背压丢弃、坏帧与重复帧被精确拦截；
...
public final class IngestDemo {
    public static void main(String[] args) throws InterruptedException {
        // ---- 场景一：正常负载，容量够用 ----
        int vehicles = 10;
        int framesPerVehicle = 300;
        IngestService ingest = new IngestService(/*laneCount*/ 4, /*queuePerLane*/ 25_000);
        ingest.start();
        List<Thread> sims = new ArrayList<>();
        List<FakeVehicle> fakes = new ArrayList<>();
        for (int i = 1; i <= vehicles; i++) {
            FakeVehicle v = new FakeVehicle("LSV" + String.format("%07d", i), framesPerVehicle, ingest);
            fakes.add(v);
            Thread t = new Thread(v, "vehicle-" + i);
            sims.add(t);
            t.start();
        }
        ...
        long dupSent = fakes.stream().mapToLong(FakeVehicle::dupSent).sum();   // 每 50 帧重复一次
        long badSent = (long) vehicles * 2;                                    // 每车 2 条坏帧
        System.out.printf("场景一 正常负载：10 台车×%d 帧，接收端有效=%d，去重=%d，坏帧拒绝=%d，背压丢弃=%d，"
                + "吞吐≈%.0f 帧/s%n", framesPerVehicle, ingest.accepted(), ingest.duplicates(),
                ingest.rejected(), ingest.backlogDropped(), ingest.accepted() / seconds);

        AtomicInteger pass = new AtomicInteger();
        check(pass, ingest.accepted() == (long) vehicles * framesPerVehicle,
                "正常负载：有效帧 = 10×300(重复与坏帧都被拦截)");
        check(pass, ingest.rejected() == badSent, "正常负载：坏帧全部被量程/格式校验拒绝");
        check(pass, ingest.duplicates() == dupSent, "正常负载：重传帧全部被 VIN+seq 去重");
        check(pass, ingest.backlogDropped() == 0, "正常负载：队列容量充足，零背压丢弃");

        // ---- 场景二：过载压测，单 VIN 打满单 lane ----
        ...
        for (int i = 1; i <= vehicles; i++) {
            Thread t = new Thread(() -> {
                for (long seq = 1; seq <= 5000; seq++) {
                    overload.submit("T|LSV0000999|" + seq + "|60|40|50");   // 单 VIN 快速连发
                }
            }, "burst-" + i);
            burst.add(t);
            t.start();
        ...
        check(pass, accounted == (long) vehicles * 5000, "过载压测：帧计数守恒(每帧要么入队处理要么丢弃计数，无一漏记)");
