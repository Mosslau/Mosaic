...
4. **内容域里的仓名自称（残留）71 行**：`docs/superpowers/` 之外共 71 行——`.md` 60 行（engineering 39 / `.dsh` 14 / languages 6 / 顶层 `README.md` 1）+ 其他文件类型 11 行（`.py` / `.mjs` / `.yml` / `.sh` / `.json` / `.toml`）。按决策 D10 **刻意保留**，留待后续「文案统一」轮。
...
2. **文案统一**：内容域仓名自称（71 行）+ Go module 路径（63 个匹配，28 个 `.go` / `.mod` 文件）+ 站点标签（`languages/website` 的站点标题/描述仍是 `TenetLang`）。
