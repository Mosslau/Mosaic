...
//!
//! 在 s16 基础上新增自治机制：scan_unclaimed_tasks 任务板扫描 + idle_poll
//! （60s 有界轮询：收件箱优先 → 任务板认领 → 超时关机）+ 身份重注入 +
//! WORK 阶段 10 轮上限。队友生命周期 WORK → IDLE → SHUTDOWN。
...
//! ```
//!
//! 对照 Python 版 ../../python/s17_autonomous_agents/code.py。
//! 运行：cargo run -p s17_autonomous_agents
...
async fn run_teammate(
    ...
    loop {
            ...
            if let ContentBlock::ToolUse { id, name: tool_name, input } = block {
                let output = if tool_name == "send_message" {
                    ...
                    match serde_json::from_value::<TaskIdInput>(input.clone()) {
                        Ok(ti) => claim_task(cwd, &ti.task_id, name),
        ...
        match idle_poll(cwd, name, &mut messages).await {
            IdleOutcome::Work => {
...
/// s17: 单次 idle 检查（抽出可测，对齐 Python idle_poll 的单次轮询体）：
/// ① 收件箱优先——shutdown_request 立即响应退出；其余消息原样注入
///    `<inbox>` 回 Work（对齐 Python：idle 阶段不细分协议，模型自行理解）；
/// ② 任务板——找到未认领任务 → claim(owner=自己) → 成功注入
///    `<auto-claimed>` 回 Work（失败黄字提示后继续轮询）；
/// ③ 都空 → None（外层继续下一轮）。
fn idle_poll_once(cwd: &Path, name: &str, messages: &mut Vec<Message>) -> Option<IdleOutcome> {
    // ① 收件箱（优先）
    let inbox = bus_read_inbox(cwd, name);
    if !inbox.is_empty() {
        for msg in &inbox {
...
async fn idle_poll(cwd: &Path, name: &str, messages: &mut Vec<Message>) -> IdleOutcome {
    for _ in 0..(IDLE_TIMEOUT_SECS / IDLE_POLL_INTERVAL_SECS) {
        tokio::time::sleep(Duration::from_secs(IDLE_POLL_INTERVAL_SECS)).await;
        if let Some(outcome) = idle_poll_once(cwd, name, messages) {
            return outcome;
        }
    }
    eprintln!(
...
async fn main() {
    ...
    println!("s17: Autonomous Agents — 自己看板，自己认领");
...
data: {"type":"message_stop"}"#,
    ...
    fn idle_poll_once_shutdown_returns_shutdown() {
        ...
        assert_eq!(outcome, Some(IdleOutcome::Shutdown));
        // 响应已发出（in idle 分支）
        let msgs = bus_read_inbox(&cwd, "lead");
        assert_eq!(msgs.len(), 1);
        assert_eq!(msgs[0].msg_type, "shutdown_response");
    ...
    fn idle_poll_once_plain_message_returns_work() {
        ...
        let mut messages: Vec<Message> = Vec::new();
        let outcome = idle_poll_once(&cwd, "alice", &mut messages);
        assert_eq!(outcome, Some(IdleOutcome::Work));
        assert_eq!(messages.len(), 1);
