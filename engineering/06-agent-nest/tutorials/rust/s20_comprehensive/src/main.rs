...
//!
//! s01–s19 全部机制在同一个 agent_loop 里协同：工具/权限/Hooks/规划/子代理/
//! 技能/压缩(含 transcript 留底)/记忆/prompt 组装/错误恢复/任务图/后台·流式·
//! 并行/cron/团队/协议/自治/worktree/MCP(配置驱动)。对齐 Python s20 的
//! 综合集成：MCP 工具接入权限管线（mcp__*deploy* 走 Gate 3 确认——s19 的
//! "仅注解"升级为"注解+拦截"）；system prompt 增加 Current time 段（缓存键外
//! 拼接，命中时仍刷新）与 Skills catalog 恒包含段；transcript 落盘对齐
//! Python 的 .jsonl 格式与 [compact]/[reactive compact] 标记。
//!
//! 对照 Python 版 ../../python/s20_comprehensive/code.py。
//! 运行：cargo run -p s20_comprehensive
...
/// 注意：Current time 段不在本函数里——它在 get_system_prompt 层拼接
/// （缓存键外，命中缓存时也刷新，见 get_system_prompt）。
fn assemble_system_prompt(ctx: &PromptContext) -> String {
    ...
    // skills — 恒包含（s20 对齐 Python：Skills catalog + Use load_skill 提示；
    // 空技能时 list_skills 返回 "(no skills found)"，不会出现空段）
...
/// s20: 追加 Current time 段（对齐 Python 的
/// `Current time: {datetime.now().isoformat(timespec='seconds')}`）。
/// 时间在缓存键**之外**：PromptContext 不含时间字段，缓存 key 不随秒变化；
/// 命中缓存时返回的前缀 + 此刻时间——缓存保留且时间始终新鲜。
fn append_current_time(prompt: String) -> String {
    ...
    if let (Ok(ref lk), Ok(ref lp)) = (LK.lock(), LP.lock()) {
        if let (Some(ref k), Some(ref p)) = (lk.as_ref(), lp.as_ref()) {
            if k.as_str() == key.as_str() {
                println!("  \x1b[90m[cache hit] system prompt unchanged\x1b[0m");
                return append_current_time(p.to_string());
            }
        }
    }
    let prompt = assemble_system_prompt(ctx);
    if let (Ok(mut lk), Ok(mut lp)) = (LK.lock(), LP.lock()) { *lk = Some(key); *lp = Some(prompt.clone()); }
    let mut loaded = vec!["identity", "tools", "workspace", "skills"];
    if !ctx.memories.is_empty() { loaded.push("memory"); }
    if !ctx.connected_mcp.is_empty() { loaded.push("mcp"); }
    println!("  \x1b[32m[assembled] sections: {}\x1b[0m", loaded.join(", "));
    append_current_time(prompt)
}
...
/// 写入会话转录到 .transcripts/ 目录（s20 对齐 Python 的 .jsonl 格式：
/// 每行一条消息的紧凑 JSON，便于 tail 增量查看；s08 原为整数组 pretty JSON）
fn write_transcript(messages: &[Message]) -> String {
    let dir = Path::new(".transcripts");
    fs::create_dir_all(dir).ok();
    let ts = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs();
    let path = dir.join(format!("transcript_{ts}.jsonl"));
...
async fn compact_history(
    ...
    println!("  \x1b[36m[compact] transcript saved: {transcript_path}\x1b[0m");
...
async fn reactive_compact(
    ...
    let transcript_path = write_transcript(messages);
    println!("  \x1b[31m[reactive compact] transcript saved: {transcript_path}\x1b[0m");
    let tail_start = tail_keep_start(messages);
    let summary = if tail_start == 0 {
        "(no older history to summarize)".to_string()
    } else {
...
fn check_rules(name: &str, input: &serde_json::Value) -> Option<&'static str> {
    match name {
        ...
        // ── s20: MCP 工具接入权限管线（对齐 Python s20 permission_hook） ──
        // s19 的 destructive 仅注解不拦截；s20 对 mcp__*deploy* 工具升级为
        // Gate 3 用户确认。Python 是 `startswith("mcp__") and "deploy" in name`
        // 的粗糙匹配——mcp__deploy__status 也会被拦（对齐，不细化）。
        n if n.starts_with("mcp__") && n.contains("deploy") => {
            Some("MCP destructive-looking tool (requires approval)")
        }
...
async fn main() {
    ...
    println!("s20: Comprehensive Agent — 全部机制，归到一个循环");
...
mod tests {
    ...
    fn assemble_prompt_without_memories() {
        ...
        // s20: Skills catalog 段恒包含（措辞对齐 Python，不再是 Available skills）
        assert!(prompt.contains("Skills catalog:"));
        assert!(prompt.contains("Use load_skill(name) when a skill is relevant."));
    }
    ...
    fn assemble_prompt_includes_skill_catalog() {
        ...
        let prompt = assemble_system_prompt(&ctx);
        // s20: 措辞对齐 Python 的 "Skills catalog:" + "Use load_skill" 提示
        assert!(prompt.contains("Skills catalog:"));
        assert!(prompt.contains("Use load_skill(name) when a skill is relevant."));
...
data: {"type":"message_stop"}"#,
    ...
    fn prompt_cache_invalidates_on_mcp_connect() {
        ...
        // 缓存命中：ctx 不变时返回同一份 prompt（Current time 段在缓存键外，
        // 命中时拼接此刻时间——同一秒内相等、跨秒只差时间段，故比前缀）
        let p3 = get_system_prompt(&ctx2);
        // 闭包带生命周期参数不行，内联两次拆分（时间段在缓存键外，前缀必须相同）
        let p2_prefix = p2.split("\n\nCurrent time: ").next().unwrap();
        let p3_prefix = p3.split("\n\nCurrent time: ").next().unwrap();
        assert_eq!(p2_prefix, p3_prefix);
        assert!(p2.contains("Current time: "));
    }
    ...
    // ═══════════════ s20: 综合章测试（MCP 权限拦截 / Current time / transcript / Skills catalog） ═══════════════

    #[test]
    fn check_rules_mcp_deploy_requires_confirm() {
        // s20: mcp__*deploy* 走 Gate 3（对齐 Python 的 name 包含 "deploy" 粗糙匹配）
        assert!(check_rules("mcp__deploy__trigger", &serde_json::json!({"service": "api"})).is_some());
        assert!(check_rules("mcp__deploy__status", &serde_json::json!({"service": "api"})).is_some());
        assert!(check_rules("mcp__docs__search", &serde_json::json!({"query": "x"})).is_none());
        assert!(check_rules("bash", &serde_json::json!({"command": "echo ok"})).is_none());
    }

    #[test]
    fn check_permission_mcp_deploy_allow_and_deny() {
        // y 放行
        let mut allow = Cursor::new(b"y\n".to_vec());
        let r = check_permission(&mut allow, "mcp__deploy__trigger", &serde_json::json!({"service": "api"}));
        assert_eq!(r, None);
        // n 拒绝
        let mut deny = Cursor::new(b"n\n".to_vec());
        let r = check_permission(&mut deny, "mcp__deploy__trigger", &serde_json::json!({"service": "api"}));
        assert!(r.is_some());
        assert!(r.unwrap().contains("Blocked by user"));
        // 非 deploy 的 mcp 工具直接放行（不询问）
        let mut docs = Cursor::new(Vec::new());
        let r = check_permission(&mut docs, "mcp__docs__search", &serde_json::json!({"query": "x"}));
        assert_eq!(r, None);
    }

    #[test]
    fn current_time_section_format_and_cache_freshness() {
        // Current time 段在缓存键外：get_system_prompt 两次调用都带时间
        let ctx = PromptContext {
            connected_mcp: Vec::new(),
            enabled_tools: vec!["bash".into()],
            workspace: "/tmp".into(),
        ...
        let p1 = get_system_prompt(&ctx);
        assert!(p1.contains("\n\nCurrent time: "), "{p1}");
        let time_part = p1.split("\n\nCurrent time: ").nth(1).unwrap_or("");
        // ISO 秒级格式：2026-08-26T20:18:05（恰 19 字符）
        assert_eq!(time_part.len(), 19, "{time_part}");
        assert_eq!(&time_part[4..5], "-");
        assert_eq!(&time_part[7..8], "-");
        assert_eq!(&time_part[10..11], "T");
        assert_eq!(&time_part[13..14], ":");
        assert_eq!(&time_part[16..17], ":");
        // 缓存命中路径同样带时间（前缀一致性见 prompt_cache_invalidates_on_mcp_connect）
        let p2 = get_system_prompt(&ctx);
        assert!(p2.contains("\n\nCurrent time: "));
    }

    #[test]
    fn write_transcript_creates_jsonl() {
        let msgs = vec![
            Message::user_text("hello"),
            Message::assistant_text("hi"),
        ];
        let path = write_transcript(&msgs);
        assert!(path.ends_with(".jsonl"), "{path}");
        let text = std::fs::read_to_string(&path).unwrap();
        let lines: Vec<&str> = text.lines().collect();
        assert_eq!(lines.len(), 2);
        // 每行都是合法 JSON（对齐 Python 的 .jsonl 每消息一行）
        for ln in &lines {
            serde_json::from_str::<serde_json::Value>(ln).expect("jsonl line must be JSON");
        }
        // 自清（.transcripts/ 已在 .gitignore，测试仍保持工作区干净）
        std::fs::remove_dir_all(".transcripts").ok();
    }

    #[test]
    fn assemble_prompt_skills_catalog_unconditional() {
        // 空技能也含 Skills catalog 段（list_skills 返回 "(no skills found)" 时）
        let ctx = PromptContext {
            connected_mcp: Vec::new(),
            enabled_tools: vec!["bash".into()],
            workspace: "/tmp".into(),
            memories: String::new(),
            skills: "(no skills found)".into(),
        };
        let p = assemble_system_prompt(&ctx);
        assert!(p.contains("Skills catalog:\n(no skills found)"), "{p}");
    }
}
