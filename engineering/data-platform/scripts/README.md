...
| ⑪ Markdown 围栏健康 | ` ``` ` 计数必须偶数（奇偶错位 → 渲染时后半篇代码/正文反转；2026-09-20 deploy/README.md §5 的 ⑨ 块就这么崩过——不报错、不挂 CI，只在渲染层崩）；开围栏必须带语言标签（目录树/输出一律 `text`，已补齐全仓 39 处）。负向对照：剥标签 / 删闭围栏 → 逐个变红 |
...
```bash
scripts/check-realtime-e2e.sh                 # 全过 exit 0（典型 ~1.5 分钟, 最坏 ~3 分钟）
E2E_TIMEOUT=300 scripts/check-realtime-e2e.sh # 放宽等待窗口（默认 240s, 从心跳结束开始计时）
```

> 耗时构成：注入（**不等整分钟边界**）→ 心跳**按需 2~7 次**（把事件时间推过"窗口结束 + 10s 容忍"，
> 窗口才会触发）→ 四条断言**共享一个截止时间**轮询。
> 2026-09-20 提速：旧版是"等整分钟（≤60s）+ 固定 12 次心跳（120s）"= 最多 180s 空等，
> 而断言只要求"窗口起点 ≥ 注入那一分钟"，与是否对齐边界无关 → 现为最多 70s，实测 1m20s。
> **注意重启后的补读**：位点是 `group-offsets`，作业刚重启会先从**已提交位移**续读停机期间的积压，
> 积压大时自检的窗口要排队等 → 放宽预算（如 `E2E_TIMEOUT=600`）；CI 里每次都是新 topic，不受影响。
