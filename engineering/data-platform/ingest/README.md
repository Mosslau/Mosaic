...
| **C 联调排障** | 《示例集》§7（现象→结论）→ `bash scripts/check-pipeline-health.sh` → 《示例集》§1~§4 | 跑不起来/数据不对，先查哪里 |
...
> 已演练三项的细节与自检脚本见《接入层设计》§8.3（自检脚本：`ingest/device-simulator` 的 `security-check`）；**生产替换物**：正式 CA 证书 + 强随机一车一密 + 去掉 dev 放行规则。
