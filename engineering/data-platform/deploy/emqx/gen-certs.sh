#!/usr/bin/env bash
# gen-certs.sh — 生成 Mosaic 本地联调用自签 CA + EMQX 服务器证书
...
SAN="DNS:localhost,DNS:emqx,DNS:mosaic-emqx,DNS:host.docker.internal,DNS:host.lima.internal,IP:127.0.0.1"

if [ ! -f certs/ca.crt ]; then
  # ① 自签 CA
  openssl req -x509 -newkey rsa:2048 -nodes -days $DAYS \
    -keyout certs/ca.key -out certs/ca.crt \
    -subj "/CN=Mosaic-Dev-CA/O=Mosaic/C=CN"
  echo "✅ CA 生成"
fi
