...
//! ```
//!
//! 对照 Python 版 ../../python/s04_hooks/code.py。
//! 运行：cargo run -p s04_hooks
//! 可选环境变量（同 s03）：EFFORT_LEVEL / MAX_TOKENS / ANTHROPIC_BETA / S01_DEBUG
...
// ── Anthropic Messages API 的数据模型（同 s01/s02/s03） ──────────────────
...
// ── 核心模式：s03 的循环 + Hook 触发（本章唯一的实质改动） ────────────────
//
// s03:
...
fn agent_loop(
    ...
    loop {
        ...
        if stop_reason.as_deref() != Some("tool_use") {
            // s04: Stop hook —— 返回 Some(msg) 则追加消息并继续循环
        ...
        for block in &resp.content {
            if let ContentBlock::ToolUse { id, name, input } = block {
                ...
                // ── s04 变化：PreToolUse hook 取代硬编码的 check_permission ──
                ...
                // ── s04 变化：PostToolUse hook（大输出警告等） ──
                trigger_post_tool_use(hooks, name, input, &output);
...
// ── 配置与入口（同 s01/s02/s03，system prompt 措辞与 s03 一致） ──────────
...
fn load_config() -> Result<Config, String> {
    ...
    Ok(Config {
        ...
        model,
        // s04 沿用 s03 的 system prompt（后续章节可能改为更通用的措辞）
        system: format!(
...
fn main() {
    ...
    println!("s04: Hooks — 扩展逻辑挂在 Hook 上，循环保持干净");
