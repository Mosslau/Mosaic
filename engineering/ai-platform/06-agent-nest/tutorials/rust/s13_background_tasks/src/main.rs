...
//!
//! 在 s12 基础上引入 tokio：主循环 async 化，三个面——
//! 后台任务（对齐 Python s13）、流式 SSE 输出、并行工具执行（Rust 前向扩展）。
...
async fn main() {
    ...
    println!("s13: Background Tasks — 慢操作放后台 + 流式输出 + 并行工具");
    ...
    loop {
        ...
        match agent_loop(&http, &cfg, &cwd, &mut history, &hooks).await {
            Ok(()) => {
