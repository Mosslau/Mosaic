...
//!
//! ```text
//!   Startup (load_config)                 Runtime (agent_loop)
//!   ┌──────────────────────┐             ┌──────────────────────┐
//!   │ build_system()       │             │ load_memories()      │
//!   │ "Skills: ...         │             │ → select_relevant    │
//!   │  Memories:           │             │   (LLM → 关键词回退)  │
//!   │  - [pref](file.md)   │             └──────────┬───────────┘
//!   │  - [fact](proj.md)"  │                        │
//!   └──────────────────────┘                        ▼
...
//! ```
//!
//! 对照 Python 版 ../../python/s09_memory/code.py。
//! 运行：cargo run -p s09_memory
//! 可选环境变量（同 s08）：EFFORT_LEVEL / MAX_TOKENS / ANTHROPIC_BETA / S01_DEBUG
...
// ── Anthropic Messages API 的数据模型（同 s01/s02/s03/s04/s05/s06/s07/s08） ──
...
fn build_system(cwd: &Path, catalog: &str) -> String {
    let index = read_memory_index(cwd);
    let memory_section = if index.is_empty() {
        String::new()
    } else {
        format!("\n\nMemories available:\n{index}\nRelevant memories are injected below. Respect user preferences from memory.\nWhen the user says 'remember' or expresses a clear preference, extract it as a memory.")
    };
    format!(
        "You are a coding agent at {}. Do not impersonate any specific AI assistant (Claude, GPT, etc.). You are a coding agent — that is your identity.. Skills available:\n{}\nUse load_skill to get full details when needed.{}",
        cwd.display(), catalog, memory_section
    )
}
...
fn spawn_subagent(
    ...
    for _turn in 0..MAX_SUB_TURNS {
        let resp = match call_llm(http, cfg, &cfg.sub_system, &messages, &tools, cfg.max_tokens) {
            Ok(r) => r,
            Err(e) => {
...
fn select_relevant_memories(http: &reqwest::blocking::Client, cfg: &Config, cwd: &Path, messages: &[Message]) -> Vec<String> {
    ...
    if let Ok(resp) = call_llm(http, cfg, "", &msgs, &tools, MEMORY_SELECT_MAX_TOKENS) {
...
fn extract_memories(http: &reqwest::blocking::Client, cfg: &Config, cwd: &Path, messages: &[Message]) {
    ...
    let msgs = vec![Message::user_text(&prompt)];
    let tools: [Tool; 0] = [];
    let resp = match call_llm(http, cfg, "", &msgs, &tools, MEMORY_EXTRACT_MAX_TOKENS) { Ok(r) => r, Err(_) => return };
    let text = extract_text(&Message::assistant_blocks(resp.content).content);
...
fn consolidate_memories(http: &reqwest::blocking::Client, cfg: &Config, cwd: &Path) {
    ...
    let msgs = vec![Message::user_text(&prompt)];
    let tools: [Tool; 0] = [];
    let resp = match call_llm(http, cfg, "", &msgs, &tools, MEMORY_CONSOLIDATE_MAX_TOKENS) { Ok(r) => r, Err(_) => return };
    let text = extract_text(&Message::assistant_blocks(resp.content).content);
    let (si, ei) = match (text.find('['), text.rfind(']')) { (Some(s), Some(e)) => (s, e+1), _ => return };
    let items: Vec<serde_json::Value> = match serde_json::from_str(&text[si..ei]) { Ok(v) => v, Err(_) => return };
    let dir = memory_dir(cwd);
...
fn summarize_history(
    ...
    let resp = match call_llm(http, cfg, "", &summary_messages, &tools, SUMMARIZE_MAX_TOKENS) {
        Ok(r) => r,
...
/// reactive_compact 最大重试次数
const MAX_REACTIVE_RETRIES: u32 = 1;
...
// ── API 客户端（同 s01/s02/s03/s04/s05/s06/s07/s08） ─────────────────────────
...
fn agent_loop(
    ...
    loop {
        ...
        // 1. 把完整历史发给模型（s08: try/except 捕获 prompt_too_long）
        let resp = loop {
            match call_llm(http, cfg, &cfg.system, &request_messages, &tools, cfg.max_tokens) {
                Ok(r) => {
                    reactive_retries = 0;
                    break r;
                }
                Err(e) => {
                    ...
                    if (err_str.contains("prompt_too_long") || err_str.contains("too many tokens"))
...
// ── 配置与入口（同 s08，build_system 含 memory 索引） ──────────────────
...
fn load_config() -> Result<Config, String> {
    ...
    let system = build_system(&cwd, &catalog);
    ...
    Ok(Config {
        ...
        model,
        // s07: system prompt 动态组装（包含 skill 目录）
        system,
...
fn main() {
    ...
    println!("s09: Memory — persistent cross-session knowledge");
...
mod tests {
    ...
    fn test_cwd(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("lcc-s09-test-{}-{tag}", std::process::id()));
    ...
    fn select_relevant_memories_empty_when_no_files() {
        ...
        let result = select_relevant_memories(
            ...
            &Config { system: "".into(), sub_system: "".into(), model: "".into(), base_url: "".into(), api_key: None, auth_token: None, max_tokens: 8000, effort: None, beta: None, debug: false },
            &cwd, &msgs);
