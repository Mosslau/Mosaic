...
//! ```
//!
//! 对照 Python 版 ../../python/s08_context_compact/code.py。
//! 运行：cargo run -p s08_context_compact
//! 可选环境变量（同 s07）：EFFORT_LEVEL / MAX_TOKENS / ANTHROPIC_BETA / S01_DEBUG
...
// ── Anthropic Messages API 的数据模型（同 s01/s02/s03/s04/s05/s06/s07） ──
...
fn build_system(cwd: &Path, catalog: &str) -> String {
    format!(
        "You are a coding agent at {}. Skills available:\n{}\nUse load_skill to get full details when needed.",
        cwd.display(),
        catalog
    )
}
...
fn summarize_history(
    ...
    // 内部辅助调用：不传 system（对齐 Python 版——摘要只需要对话内容，
    // 不需要技能目录/记忆索引），token 用固定预算而非 cfg.max_tokens
...
/// 摘要调用的独立 token 预算（对齐 Python 版的 2000）。
/// 主循环预算可被 MAX_TOKENS=300 压小以演示截断升级，但摘要被截断是无声的——
/// 压缩后目标丢失、模型答非所问（实测教训）。摘要用固定预算与主循环解耦。
...
// ── API 客户端（同 s01/s02/s03/s04/s05/s06/s07） ─────────────────────────
...
// ── 核心模式：s07 的循环 + 四层压缩管线 + compact 工具 ──────────────────
//
// s03:
...
fn agent_loop(
    ...
    loop {
        ...
        let resp = loop {
            match call_llm(http, cfg, &cfg.system, messages, &tools, cfg.max_tokens) {
                Ok(r) => {
                    reactive_retries = 0;
                    break r;
                }
...
// ── 配置与入口（同 s07，system prompt 由 build_system() 动态组装） ────────
...
fn main() {
    ...
    println!("s08: Context Compact — four-layer compaction pipeline");
...
mod tests {
    ...
    fn parse_frontmatter_with_yaml() {
        ...
        assert_eq!(desc, "Review code thoroughly");
    }
    ...
    fn parse_frontmatter_no_delimiters() {
        ...
        let (name, desc) = parse_frontmatter(text);
        assert_eq!(name, "");
        // 无 frontmatter 时 description 回退为第一行（去掉 # 与首尾空白）
        assert_eq!(desc, "Just a heading");
    }
