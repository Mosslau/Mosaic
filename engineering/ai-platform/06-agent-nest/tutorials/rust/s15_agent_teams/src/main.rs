...
//!
//! 在 s14 基础上新增团队机制：MessageBus 文件收件箱（.mailboxes/*.jsonl）、
//! 队友 tokio 任务（spawn_teammate）、Lead 收件箱唤醒（select! 第三路）。
...
//! ```
//!
//! 对照 Python 版 ../../python/s15_agent_teams/code.py。
//! 运行：cargo run -p s15_agent_teams
...
fn run_check_inbox(cwd: &Path) -> String {
    ...
    msgs.iter()
        .map(|m| format!("  [{}] {}", m.from, truncate_chars(&m.content, 200)))
        .collect::<Vec<_>>()
        .join("\n")
}
...
/// 队友 system prompt（对齐 Python 的 f-string）。
...
/// 队友工具集：bash / read_file / write_file / send_message（4 个，对齐 Python；
/// 比 s06 子代理少 edit_file/glob——教学版聚焦通信机制）。
fn teammate_tools() -> [Tool; 4] {
    [
        Tool {
            name: "bash",
            description: "Run a shell command.",
...
/// 队友回合数上限（教学版防死循环；真实 CC 用 idle loop）。
const MAX_TEAMMATE_ROUNDS: usize = 10;
...
async fn run_teammate(
    ...
    for _ in 0..MAX_TEAMMATE_ROUNDS {
        // 每轮开头读自己收件箱（队友之间的消息 / Lead 的指示）
        let inbox = bus_read_inbox(cwd, name);
        if !inbox.is_empty() {
            let json = match serde_json::to_string(&inbox) {
                Ok(j) => j,
        ...
        let window: Vec<Message> = messages[window_start..].to_vec();
...
async fn main() {
    ...
    println!("s15: Agent Teams — 一个搞不定，组队来");
    ...
    loop {
        ...
        tokio::select! {
            ...
            Some(()) = wake_rx.recv() => {
                ...
                let inbox = bus_read_inbox(&cwd, "lead");
...
data: {"type":"message_stop"}"#,
    ...
    fn check_inbox_formats_messages_and_consumes() {
        ...
        assert!(out.contains("  [alice] Schema done"));
        assert!(out.contains("  [bob] Client written"));
        // 消费式：再读为空
        assert_eq!(run_check_inbox(&cwd), "(inbox empty)");
        let _ = fs::remove_dir_all(&cwd);
    }
    ...
    fn check_inbox_truncates_long_content() {
        ...
        // 200 字符截断 + 无多余内容
        let body = out.trim_start_matches("  [alice] ");
    ...
    fn format_inbox_block_matches_python_format() {
        let msgs = vec![
            MailMessage { from: "alice".into(), to: "lead".into(), content: "Schema done".into(), msg_type: "result".into(), ts: 1.0 },
            MailMessage { from: "bob".into(), to: "lead".into(), content: "Client written".into(), msg_type: "result".into(), ts: 2.0 },
        ];
        assert_eq!(
    ...
    #[test]
    fn teammate_system_prompt_mentions_name_role_and_lead() {
        let p = teammate_system_prompt("alice", "backend dev");
        assert!(p.contains("'alice'"));
        assert!(p.contains("backend dev"));
        assert!(p.contains("send_message to 'lead'"));
    }

    #[test]
    fn teammate_tools_are_exactly_four() {
        let names: Vec<&str> = teammate_tools().iter().map(|t| t.name).collect();
        assert_eq!(names, vec!["bash", "read_file", "write_file", "send_message"]);
    }

    #[test]
    fn all_tools_contains_team_tools() {
