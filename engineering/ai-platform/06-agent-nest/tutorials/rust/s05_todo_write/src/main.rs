//! s05: TodoWrite — 在 s04 hooks 基础上新增规划工具 + nag 提醒
//!
//! s05 在 s04 的 Hook 系统上加了一个 todo_write 工具，以及监督提醒机制。
...
//! ```
//!
//! 对照 Python 版 ../../python/s05_todo_write/code.py。
//! 运行：cargo run -p s05_todo_write
//! 可选环境变量（同 s04）：EFFORT_LEVEL / MAX_TOKENS / ANTHROPIC_BETA / S01_DEBUG
...
// ── Anthropic Messages API 的数据模型（同 s01/s02/s03/s04） ──────────────
...
// ── 核心模式：s04 的循环 + nag 提醒 + todo_write 分发（本章实质改动） ────
//
// s03:
...
// ── 配置与入口（同 s04，system prompt 加入规划引导） ──────────────────────
...
fn load_config() -> Result<Config, String> {
    ...
    Ok(Config {
        ...
        model,
        // s05: system prompt 加入规划引导
        system: format!(
            "You are a coding agent at {}. Before starting any multi-step task, use todo_write to plan your steps. Update status as you go.",
            cwd.display()
        ),
...
fn main() {
    ...
    println!("s05: TodoWrite — plan before execute, nag if you forget");
