...
// 与 tenet-rs/src/parser.rs、tenet-py/tenet/parser.py 语义一致。
//
// 优先级（低 → 高）：
//   ||  <  &&  <  == !=  <  < <= > >=  <  + -  <  * / %  <  一元 - !
#pragma once
...
private:
    ...
    std::string parse_type() {
        switch (peek().kind) {
            ...
            default:
                throw error("期望类型 `int` / `float` / `bool` / `string`，但遇到 "
                            + describe(peek().kind, peek()));
        }
    }
    ...
    ExprPtr parse_binary(int min_prec) {
        ...
        while (true) {
            ...
            advance();
            ExprPtr rhs = parse_binary(prec + 1);  // 左结合
    ...
    ExprPtr parse_primary() {
        ...
        switch (tok.kind) {
            case Kind::Int:
                ...
                e->int_val = tok.int_val;
                return e;
            case Kind::Float:
                advance();
                e->kind = Expr::Kind::Float;
                e->float_val = tok.float_val;
                return e;
            case Kind::Str:
                advance();
                e->kind = Expr::Kind::Str;
                e->str_val = tok.str_val;
                return e;
            case Kind::True:
                advance();
