#!/usr/bin/env bash
# submit-jobs.sh — 把 realtime/sql/ 下的三个 Flink SQL 作业提交到 session cluster
#
...
#
# 用法: bash realtime/submit-jobs.sh [作业名...]     # 缺省提交全部三个
...
" "${name}")
  ...
  out=$(docker run --rm --network "${NET}" --memory=640m \
      -v "${ROOT}/realtime/conf/sql-client-flink-conf.yaml:/opt/flink/conf/flink-conf.yaml:ro" \
      -v "${ROOT}/realtime/sql:/opt/flink/sql:ro" \
  ...
  # 强判据: 轮询集群, 直到该作业名出现（最多 60s）
  ok=""
  for _ in $(seq 1 20); do
    if curl -s --max-time 5 "${REST}/jobs/overview" 2>/dev/null | grep -q "\"name\":\"${name}\""; then ok=1; break; fi
    sleep 3
  done
  if [ -n "${ok}" ]; then echo "✅ 已在集群中运行（${name}）"; else echo "❌ 提交后集群里没有该作业"; fail=1; fi
done
