...
|---|---|---|---|---|
|实体 Entity|业务中相对稳定、有唯一标识和生命周期的对象|实体档案是所有明细数据挂载的锚点，实体打通决定后续分析能不能关联|实体档案表 + 拉链表 / 快照表，保留标识映射与状态变迁|设备 Device（device_id、型号、绑定账号、激活时间）；用户 User（user_id、注册渠道、生命周期状态）|
|事件 Event|在某个时间点发生的、不可变的事实记录|事件表通常是平台数据量最大、接入最频繁的部分，决定接入与计算成本|事实表 + 时间分区；以 event_id 保证幂等，以事件时间支持乱序|设备状态上报（device_id、status、ts）；业务动作事件（下单、支付、提交工单）|
