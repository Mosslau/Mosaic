...
> 📌 接入层决策（2026-09-16）：车端线协议定为**二进制（参照 GB/T 32960 帧框架）**；接入网关只做包头轻解析（帧同步/VIN/协议版本/CRC），编解码归独立 **device-codec 服务**（第 2 阶段，双向：上行解析 + 下行编码），解析产物为统一契约 `VehicleReport`。详见《ingest/device-gateway/docs/GB32960-二进制协议与字段映射-v1.md》与《接入层与车端接入网关设计》§4/§11.4。
