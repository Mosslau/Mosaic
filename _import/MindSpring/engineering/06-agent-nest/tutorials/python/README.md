...
This repository contains a single canonical track: the root-level `s01_*` ... `s20_*` chapters.
Each chapter contains a full narrative README, translations, runnable `code.py`, and diagrams where needed.

The legacy 12-lesson track (`agents/`, `docs/`) has been removed. The web app renders the current
s01-s20 chapters (content generated under `web/src/data/generated/`).
...
The web app renders the current s01-s20 chapters (content in `web/src/data/generated/`).
...
repo root/
  ...
  tests/                   # unit/smoke tests for chapter code
