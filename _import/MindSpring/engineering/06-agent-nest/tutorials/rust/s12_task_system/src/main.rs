...
//!
//! 在 s11 基础上新增文件持久化的任务图：.tasks/{id}.json + blockedBy 依赖。
//! s01–s11 全部机制保留（错误恢复/权限/Hook/压缩/记忆/技能/子代理照旧）。
//!
//! ```text
//!   Agent loop: dispatch
//!                   │
...
//! ```
//!
//! 对照 Python 版 ../../python/s12_task_system/code.py。
//! 运行：cargo run -p s12_task_system
...
fn main() {
    ...
    println!("s12: Task System — .tasks/ 持久任务图 + blockedBy 依赖");
