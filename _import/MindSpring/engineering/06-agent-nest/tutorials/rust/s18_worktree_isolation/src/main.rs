...
//!
//! 在 s17 基础上新增工作区隔离：Task 加 worktree 字段 + .worktrees/ git
//! worktree 系统（create/remove/keep 三工具 + 事件日志 + 名称校验）+ 队友
//! worktree cwd 上下文（认领绑定 worktree 的任务后，bash/read/write 都在
//! 该 worktree 目录内执行，互不覆盖）。
//! s01–s17 全部机制保留（后台/流式/并行/任务图/错误恢复/权限/Hook/压缩/记忆/技能/子代理/cron/团队/协议/自治 照旧）。
//!
//! ```text
//!  ┌─────────────────────────────────────────────────────┐
...
//! ```
//!
//! 队友: 认领绑定 worktree 的任务 → cwd 切换到 .worktrees/{name}
//!       → bash/read/write 都在该目录（write 越界拒绝）
//!       → complete 后 cwd 重置回主仓库
//!
//! 对照 Python 版 ../../python/s18_worktree_isolation/code.py。
//! 运行：cargo run -p s18_worktree_isolation
...
async fn main() {
    ...
    println!("s18: Worktree Isolation — 各干各的，互不干扰");
