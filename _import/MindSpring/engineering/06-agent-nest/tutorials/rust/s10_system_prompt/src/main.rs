...
//! ```
//!
//! 对照 Python 版 ../../python/s10_system_prompt/code.py。
//! 运行：cargo run -p s10_system_prompt
//! 可选环境变量（同 s09）：EFFORT_LEVEL / MAX_TOKENS / ANTHROPIC_BETA / S01_DEBUG
...
// ── API 客户端（同 s01/s02/s03/s04/s05/s06/s07/s08/s09） ─────────────────────────
...
fn agent_loop(
    ...
    loop {
        ...
        let resp = loop {
            match call_llm(http, cfg, &system, &request_messages, &tools, cfg.max_tokens) {
                Ok(r) => {
                    reactive_retries = 0;
                    break r;
                }
...
fn main() {
    ...
    println!("s10: System Prompt — runtime assembly with caching");
...
mod tests {
    ...
    fn select_relevant_memories_empty_when_no_files() {
        ...
        let result = select_relevant_memories(
            ...
            &Config { sub_system: "".into(), model: "".into(), base_url: "".into(), api_key: None, auth_token: None, max_tokens: 8000, effort: None, beta: None, debug: false },
            &cwd, &msgs);
        assert!(result.is_empty());
        let _ = fs::remove_dir_all(&cwd);
    }
