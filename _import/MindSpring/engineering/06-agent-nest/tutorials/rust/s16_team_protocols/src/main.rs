...
//! ```
//!
//! 两种协议一套机制：shutdown_request/response（Lead→队友，体面关机握手）、
//! plan_approval_request/response（队友→Lead，计划审批）。request_id 贯穿全链路。
//!
//! 对照 Python 版 ../../python/s16_team_protocols/code.py。
//! 运行：cargo run -p s16_team_protocols
...
fn all_tools() -> [Tool; 23] {
    [
        Tool {
            name: "bash",
            description: "Run a shell command. Set run_in_background=true for slow operations (installs, builds, long-running tests) so the agent can continue working; results are injected as notifications when complete.",
        ...
        Tool {
            name: "spawn_teammate",
            description: "Spawn a teammate agent in a background task.",
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "name": { "type": "string" },
...
fn complete_task(cwd: &Path, task_id: &str) -> String {
    ...
    let unblocked: Vec<String> = list_tasks(cwd)
        ...
        .collect();
    println!("  \x1b[32m[complete] {} ✓\x1b[0m", task.subject);
    let mut msg = format!("Completed {} ({})", task.id, task.subject);
    if !unblocked.is_empty() {
        let names = unblocked.join(", ");
        msg = format!("{msg}\nUnblocked: {names}");
        println!("  \x1b[33m[unblocked] {names}\x1b[0m");
    }
    msg
}
...
async fn agent_loop(
    ...
    loop {
        ...
        for (i, slot) in slots.iter().enumerate() {
            ...
            let name2 = name.clone();
            let input2 = input.clone();
            let handle = tokio::task::spawn_blocking(move || execute_sync(&name2, &input2));
            batch.push((i, handle));
        }
...
async fn run_teammate(
    ...
    // s16: idle loop（对齐 Python while not shutdown_requested）——
    // 无回合数上限，靠 shutdown 协议退出；每轮先分发协议消息再走 LLM turn
    ...
    while !shutdown_requested {
        ...
        if stop_reason.as_deref() != Some("tool_use") {
            // s16: idle 等待（对齐 Python：LLM 无工具调用后不退出，
            // 每秒轮询 inbox——shutdown_request → 响应退出；新消息 → 继续工作）
            match teammate_idle(cwd, name, &mut messages).await {
                IdleOutcome::Shutdown => {
                    // 队友已响应 shutdown_request 并应退出
                    // （对齐 Python shutdown_requested = True 后 break）
                    break;
                }
                IdleOutcome::Continue => {
                    // 新消息已注入，回 while 顶部走 LLM turn
                    continue;
                }
            }
        }
        ...
        for block in &resp.content {
            if let ContentBlock::ToolUse { id, name: tool_name, input } = block {
                let output = if tool_name == "send_message" {
                    ...
                    // bash/read_file/write_file 复用主循环 handler。不触发权限
                    // hook：Gate3 需 stdin 交互，后台队友任务不可行（对齐 Python
                    // 教学版；真实 CC 用权限冒泡，见 s16）
...
async fn run_spawn_teammate(
    ...
    format!("Teammate '{}' spawned as {}", input.name, input.role)
}
...
/// 队友 idle 等待阶段（对齐 Python 的 idle 内层循环，抽出可单测）：
/// 每秒轮询自己的收件箱；收到 shutdown_request → 响应并返回 Shutdown；
/// 收到其他消息 → 注入 `<inbox>` 并返回 Continue（回 LLM turn）。
#[derive(Clone, Copy, Debug, PartialEq)]
enum IdleOutcome {
    Continue,
    Shutdown,
...
async fn teammate_idle(cwd: &Path, name: &str, messages: &mut Vec<Message>) -> IdleOutcome {
    let mut interval = tokio::time::interval(Duration::from_secs(1));
    loop {
        interval.tick().await;
        let inbox = bus_read_inbox(cwd, name);
        ...
        for msg in &inbox {
            let t = msg.msg_type.as_str();
            if t == "shutdown_request" || t == "plan_approval_response" {
                if handle_inbox_message(cwd, name, msg, messages) {
                    should_stop = true;
                    break;
                }
            } else {
        ...
        if !non_protocol.is_empty() {
            let json = serde_json::to_string(&non_protocol).unwrap_or_else(|_| "[]".to_string());
            messages.push(Message::user_text(format!("<inbox>{json}</inbox>")));
...
async fn main() {
    ...
    println!("s16: Team Protocols — 队友之间要有约定");
...
mod tests {
    ...
    fn task_json_matches_python_schema() {
        ...
        assert_eq!(
            keys,
            ["blockedBy", "description", "id", "owner", "status", "subject"]
                .iter()
                .map(|s| s.to_string())
                .collect::<Vec<_>>()
        );
...
data: {"type":"message_stop"}"#,
    ...
    #[test]
    fn teammate_system_prompt_mentions_protocol_hint() {
        let p = teammate_system_prompt("alice", "backend dev");
        assert!(p.contains("'alice'"));
        assert!(p.contains("backend dev"));
        // s16: 提示检查协议消息（对齐 Python）
        assert!(p.contains("Check inbox for protocol messages"));
        assert!(p.contains("shutdown_request"));
    }

    #[test]
    fn teammate_tools_are_exactly_five() {
        let names: Vec<&str> = teammate_tools().iter().map(|t| t.name).collect();
        assert_eq!(
            names,
            vec!["bash", "read_file", "write_file", "send_message", "submit_plan"]
        );
    }
    ...
    #[tokio::test]
    async fn teammate_idle_shutdown_returns_shutdown() {
        ...
        let mut messages: Vec<Message> = Vec::new();
        let outcome = teammate_idle(&cwd, "alice", &mut messages).await;
        assert_eq!(outcome, IdleOutcome::Shutdown);
        // 响应已发出
        assert_eq!(bus_read_inbox(&cwd, "lead").len(), 1);
        let _ = fs::remove_dir_all(&cwd);
    }
    ...
    #[tokio::test]
    async fn teammate_idle_plain_message_returns_continue() {
        let cwd = team_test_cwd("idle-plain");
        bus_send(&cwd, "lead", "alice", "有新任务", "message");
        let mut messages: Vec<Message> = Vec::new();
        let outcome = teammate_idle(&cwd, "alice", &mut messages).await;
        assert_eq!(outcome, IdleOutcome::Continue);
        assert_eq!(messages.len(), 1);
