...
//! 两层注入：第一层 system prompt 里放 skill 目录（~100 tokens），
//! 第二层 Agent 调 load_skill 时返回完整 SKILL.md（~2000 tokens）。
//!
//! ```text
//!   Startup                         Runtime
//!   ┌──────────┐                   ┌──────────────────┐
...
//! ```
//!
//! 对照 Python 版 ../../python/s07_skill_loading/code.py。
//! 运行：cargo run -p s07_skill_loading
//! 可选环境变量（同 s06）：EFFORT_LEVEL / MAX_TOKENS / ANTHROPIC_BETA / S01_DEBUG
...
// ── Anthropic Messages API 的数据模型（同 s01/s02/s03/s04/s05/s06） ──────
...
// ── API 客户端（同 s01/s02/s03/s04/s05/s06） ─────────────────────────────
...
// ── 核心模式：s06 的循环 + load_skill 分发（本章实质改动） ────────────────
//
// s03:
        ...
        for block in &resp.content {
            if let ContentBlock::ToolUse { id, name, input } = block {
                ...
                let output = match name.as_str() {
                    ...
                    // s07: load skill content on demand
...
// ── 配置与入口（system prompt 由 build_system() 动态组装） ────────────────
...
fn main() {
    ...
    println!("s07: Skill Loading — catalog in SYSTEM, content on demand");
