...
**混合类型规则**：`int` 与 `float` 可互通（int 自动提升为 float）；
**字符串与任何非字符串混合运算（含 `+`、比较）是编译错误**——
`"a" + 1`、`"a" < 1` 均报 `运算符 X 不能作用于 string 和 int`。
**bool 不能参与数值与比较运算**（`&&`/`||`/`!` 与条件除外），报 `运算符 X 不能作用于 bool 和 int`；
`&&`/`||`/`!` 与 `if`/`while` 条件则要求操作数必须是 bool。
（三个编译器在前端报同样的 `[行:列]` 错误，绝不漏出非法 IR/汇编——
早期 rs/cpp 曾把 string 当数值生成非法 IR、arm64 曾静默按数值处理，均已修复。）

静态检查（类型检查器阶段）：变量先声明后使用、运算数类型匹配（混合类型 / bool /
`%` 仅 int）、`let` 声明/赋值/函数参数/返回类型核对、条件必须 bool、
`match` 穷尽（Option 必须含 None 分支）、函数参数/返回类型核对。
...
**能写**：纯数值算法（递归/循环/运算）、控制流逻辑（FizzBuzz/乘法表）、
字符串拼接与比较输出、自定义函数库。
...
| 能力 | 缺口 |
|------|------|
| 数组 / 排序 / 集合 | 无 `array<T>`（设计已定） |
| 逐字符处理文本 | 无字符串索引/切片 |
| 数字格式化（int→string 拼接） | 无转换函数——`line + 1` 是编译错误 |
| 交互输入 / 文件 IO | 无 read / 标准库 |
| 复杂数据建模 | 无 `struct`（设计已定） |
| 显式错误处理 | 无 `Result`/`?`（设计已定） |

当前 Tenet 定位：**纯计算的算法语言**——能写一切不需要容器、输入、
格式化的数值与逻辑程序。容器（array/struct）与输入输出是第一批演进项。
...
| 实现 | 数量 | 断言方式 |
|---|---|---|
| compiler-rs | 41 | 词法/语法文本 + IR 模式 + 类型错误消息/`[行:列]` 断言 |
| compiler-cpp | 74 | 词法/语法文本 + **LLVM Module 结构断言** + 类型错误消息/位置断言 |
| compiler-arm64 | 68 | 词法/语法文本 + **汇编模式断言** + 类型错误消息/位置断言 |
