# examples/ex05-batch-rename.py —— 批量重命名 + 自定义异常 + raise from 异常链
# 来源：05-file-exception.md 第 6 章示例 5
# 验证环境：Python 3.13.12
# 运行：python3 ex05-batch-rename.py
# 验证状态：已验证

"""批量修改文件扩展名，支持 dry-run 预览；用自定义异常封装失败原因并保留根因。"""

import os
import tempfile


class BatchRenameError(Exception):
    """批量重命名异常，携带失败文件路径和原因。"""

    def __init__(self, path, reason):
        self.path = path
        self.reason = reason
        super().__init__(f"重命名失败: {os.path.basename(path)} - {reason}")


def batch_rename_ext(dir_path, old_ext, new_ext, dry_run=True):
    """批量修改文件扩展名，dry_run=True 时只预览不实际操作。"""
    if not old_ext.startswith("."):
        old_ext = "." + old_ext
    if not new_ext.startswith("."):
        new_ext = "." + new_ext
    renamed = 0
    for fname in os.listdir(dir_path):
        if not fname.endswith(old_ext):
            continue
        old_path = os.path.join(dir_path, fname)
        new_path = os.path.join(dir_path, fname[: -len(old_ext)] + new_ext)
        if os.path.exists(new_path):
            raise BatchRenameError(fname, f"目标文件已存在: {new_path}")
        try:
            if not dry_run:
                os.rename(old_path, new_path)
            renamed += 1
            print(
                f"{'[DRY RUN] ' if dry_run else ''}{fname} -> "
                f"{os.path.basename(new_path)}"
            )
        except OSError as e:
            raise BatchRenameError(fname, str(e)) from e
    return renamed


def main():
    """在临时目录演示 dry-run 预览，随后实际执行批量重命名。"""
    with tempfile.TemporaryDirectory() as d:
        for name in ["can_001.log", "can_002.log", "can_003.log", "diag.txt"]:
            path = os.path.join(d, name)
            with open(path, "w", encoding="utf-8"):
                pass

        count = batch_rename_ext(d, ".log", ".csv", dry_run=True)
        print(f"预览完成: {count} 个文件将被重命名")
        remaining_logs = [n for n in os.listdir(d) if n.endswith(".log")]
        print(f"dry_run 后仍有 {len(remaining_logs)} 个 .log 文件（未实际修改）")

        count = batch_rename_ext(d, ".log", ".csv", dry_run=False)
        csv_files = [n for n in os.listdir(d) if n.endswith(".csv")]
        print(f"实际执行完成: {count} 个文件已重命名, 现有 {len(csv_files)} 个 .csv 文件")


if __name__ == "__main__":
    main()
