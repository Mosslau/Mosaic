...
public final class NodeManager {
    ...
    public Source register(String sourceId, String model, String firmware) {
        ...
        Source prev = sources.putIfAbsent(sourceId, created);
        if (prev != null) {
    ...
    public void registerAll(List<Source> imports) {
        if (imports.stream().anyMatch(v -> sources.containsKey(v.sourceId()))) {
            throw new IllegalArgumentException("批量导入含已注册 SOURCE_ID");
        }
        imports.forEach(v -> sources.put(v.sourceId(), v));
    }
    ...
    public Source changeStatus(String sourceId, Status target) {
        ...
        return sources.compute(sourceId, (k, v) -> new Source(v.sourceId(), v.model(), v.firmware(), target));
    }
    ...
    public List<Source> listByStatus(Status s) {
        return sources.values().stream().filter(v -> v.status() == s)
                .sorted((a, b) -> a.sourceId().compareTo(b.sourceId())).toList();
    }
    public long fleetSize()                                 { return sources.size(); }
}
