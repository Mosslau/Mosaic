// examples/ex05-metrics-consumer/MiniKafka.java —— 内存版 Kafka：分区顺序日志 + offset 语义
...
public final class MiniKafka {
    private final List<MetricMessage> log = new ArrayList<>();   // 单个分区，按序追加

    /** 写入一批消息，返回这批消息的起始 offset。 */
    public long produce(List<MetricMessage> batch) {
    ...
    record ConsumedResult(List<MetricMessage> messages, long nextOffset) { }
}
