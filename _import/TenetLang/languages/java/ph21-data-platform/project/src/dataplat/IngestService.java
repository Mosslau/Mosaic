// project/src/dataplat/IngestService.java —— 数据源数据接入服务(同步校验/去重 → 投递总线)
package dataplat;
// 验证环境：OpenJDK 17.0.18(Homebrew)，命令：javac -encoding UTF-8 -d /tmp/tl21-proj src/dataplat/*.java
//
// 接入服务的职责边界：解码(已由 MetricFrame.decode 完成)、单帧量程校验、
// per-SOURCE_ID 乱序去重，然后把干净帧 produce 到总线。计数全部原子，多g线程并发安全。
...
public final class IngestService {
    ...
    public IngestService(MetricBus bus) {
        this.bus = bus;
    }

    /** 上报一行(采集端网关按行写入)；返回是否被接受并投递。 */
    public boolean submit(String rawLine) {
        MetricFrame frame = MetricFrame.decode(rawLine).orElse(null);
        if (frame == null) {
