...
- **查询执行算子插件**：把本项目 `engine_api` 换成算子接口（`open/next/close` + 行批量语义），实现 scan/filter 两个算子插件——ph17 IExecutor 抽象越过动态库边界的样子，衔接 roadmap 第 23 节向量化执行（roadmap 第 22/23 节）
