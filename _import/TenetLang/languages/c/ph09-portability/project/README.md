...
- [x] 按大小轮转：超过 `MAX_LOG_SIZE` 把当前文件改名为 `app.log.N`（序号跨运行单调递增、不覆盖旧备份，轮转不丢行），重开新文件写头
...
- [ ] `make clean && make test`（即 `./logdemo`）输出 `本轮新增行数: 802 (期望 802, 文件合计 ...)` 与 `自检结果: 通过`，退出码 0（802 = 4 线程 × 200 条 INFO + 主线程 2 条；每线程 50 条 DEBUG 被级别过滤正确丢弃；轮转次数随行宽略有浮动，只要 ≥1 即通过）
- [ ] `touch log.c && make` 只重编 log.o（增量构建生效）
- [ ] 不清除旧日志连续运行 20 次自检均通过（自检只统计本轮新增行数，轮转序号跨运行单调，不受上次运行残留影响）
- [ ] `make clean` 清空全部产物（含轮转备份 `app.log.*`）

## 扩展方向（可选）

- 加备份保留上限（如只留最近 N 份，轮转时删除最旧备份）——为 roadmap 第 16 节存储引擎的日志/清理策略打底
- 用 `clang -fsanitize=thread`（TSan）验证无数据竞争 — 属于 ph11 Sanitizer / 静态分析 / 单元测试阶段的内容
- 日志文件头做字节序处理（`htonl` 等）以支持跨机器交换 — 属于 ph12 字节序、内存对齐与二进制格式解析阶段（roadmap 第 12 节）的内容
...
- 验证状态：已验证（构建零警告；`make test` 自检通过、退出码 0；不清除旧日志连续运行 20 次自检均通过；追加重开不重复写头、不丢数据；`_WIN32` 分支（`CRITICAL_SECTION`/`_getpid`）未在本环境验证——需 Windows）
