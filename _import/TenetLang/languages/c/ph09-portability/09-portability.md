...
要点：`uint32_t` 保证字段宽度不随 int/long 平台差异漂移；`_Static_assert` 把"尺寸契约"写进编译期（**编译不过就不许上线**）；结构体存在对齐填充，上"线"要逐字段 memcpy；**位域布局是实现定义的**，跨平台协议里应改用显式掩码位运算；字节序（大端/小端）仍随平台不同，网络传输前还需 htonl——ph12 字节序、内存对齐与二进制格式解析阶段（roadmap 第 12 节）深入。
