...
本阶段直接承接 ph03+ph04：ph03 的指针操作对象从 `int` 变成 `struct Node`，ph04 的 malloc/free 用来在堆上创建/销毁节点。链表节点的 `next` 指针就是 ph03 的核心遗产——**指针指向同类型的下一个节点**，这是所有链式结构的基础。本阶段只涉及内存中的数据结构，**不涉及文件操作、多线程与进程 — 那些是 ph06/ph07 阶段的内容**。
...
完整文件：`examples/ex01-linked-list.c`
...
完整文件：`examples/ex02-array-stack.c`
...
完整文件：`examples/ex03-linked-queue.c`
...
完整文件：`examples/ex04-hash-table.c`
...
完整文件：`examples/ex07-graph-bfs.c`
...
本阶段练习见 [exercises/](./exercises/)（题目在 exercises/README.md，参考实现 sol-* 先别看）：单链表增删查改、数组栈括号匹配、队列模拟任务调度、哈希表统计词频共 4 题。完成 4 题后继续。

### 阶段项目

本阶段综合项目见 [project/](./project/)：**内存表 / HashMap KV 表**（kvstore，链地址法哈希表实现字符串键值存储，支持 put/get/delete/遍历）。
