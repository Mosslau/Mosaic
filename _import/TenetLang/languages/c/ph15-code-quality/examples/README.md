...
验证环境（本机实测）：**Apple clang 21.0.0**（`cc`，macOS arm64，ProductVersion 26.6.2）。全部 C 代码 `cc -Wall -Wextra -std=c11` 零警告；产物一律写 `/tmp/ph15c-ex/`，仓库零残留。跨平台注意：ex01/ex06 平台探测宏中的 `_WIN32`（Windows）分支**未在本环境验证**（需 Windows），POSIX/macOS 分支全部实测；ex05 没有 `_WIN32` 分支——它的时间格式化用 POSIX 的 `localtime_r`（Windows 需换 `localtime_s`，本示例不处理该差异），Windows 下未验证。
...
| 组 | 文件 | 覆盖主题 | 验证状态 |
|----|------|---------|----------|
| ex01 | `ex01-macro-tricks.c` | 宏技巧：副作用陷阱正反例（MAX 双求值 vs inline 函数）、do-while(0)、可变参宏（##__VA_ARGS__ 吞逗号）、X-Macro（枚举+字符串表同源）、条件编译（__STDC_VERSION__/平台宏） | 已验证：零警告，退出码 0；`MAX_BAD(i++,0)` 双求值 i 1→3 结果 2，`imax` 单求值 1→2；平台宏 `_WIN32` 分支未在本环境验证 |
...
```bash
mkdir -p /tmp/ph15c-ex    # 0. 先建产物目录（make all / make exXX 会自动建）
...
- 与 ph09 的边界：ph09 讲"平台差异怎么抽象"（条件编译做平台层），ph15 讲"宏/函数指针/状态机本身怎么写才不咬人"（ex01 的副作用陷阱即 ph15 视角）；ex01/ex06 平台探测宏中的 Windows 分支与 ph09 同款标注——未在本环境验证；ex05 无 Windows 分支（`localtime_r` 在 Windows 需换 `localtime_s`，本示例不处理该差异），Windows 下亦未验证。
