...
// 一句话说明：领域包（依赖图最内层）。Node 的字段与状态是"业务词汇"，
// 全项目只有这里定义；handler/service/store 都引用它，它不引用任何人。
// 说明：本项目的"节点管理"只做 HTTP 管理面（注册/状态/版本/指令受理），
// 实时遥测与节点协议接入（MQTT/WebSocket 上行）属 ph21 通用数据采集与接入网关方向 Go 阶段。
...
type Node struct {
	...
	Version  string    `json:"version"`  // 语义化版本 major.minor.patch，空串 = 未安装
