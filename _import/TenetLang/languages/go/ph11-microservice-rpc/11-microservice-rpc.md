...
这个阶段只涉及微服务起步与 RPC 通信本身（承接 ph09 Web 后端阶段的单体 HTTP、ph10 数据库阶段的数据层拆分），**不涉及云原生与容器部署（Docker、Kubernetes、服务网格属 ph12 云原生与部署阶段，roadmap 第 12 节）、消息队列与异步解耦（Kafka/RabbitMQ 属 [ph19 消息队列与事件驱动深入阶段](../ph19-mq-event-driven/19-mq-event-driven.md)；MQTT 属 [ph21 数据采集与接入网关相关 Go 阶段](../ph21-data-ingest-gateway/21-data-ingest-gateway.md)）、分布式事务与强一致（两阶段提交、Saga 属后续阶段，roadmap 未单列）、API 网关与可观测性的完整部署（Prometheus/OpenTelemetry 工具链接入属 ph12）** — 本阶段是"单体拆分 + RPC 通信"起步。
...
**为什么用 net/rpc 起步**：gRPC 把"接口定义、代码生成、序列化、路由、重试"都封装好了——很好用，但也把 RPC 的本质藏了起来。net/rpc 是唯一"透明"的选择：服务名.方法名的路由规则、反射驱动的参数组装、Gob/JSON 的 wire 格式，全部摊在标准库源码与可读报文里。**先学 net/rpc 把机制看透，再学 gRPC 体会"封装掉了什么"**——这正是本阶段示例 1/2 与示例 6 的顺序逻辑，也是"教材选型"的意义。
...
> 双向流在实时数据推送场景深入展开，属 [ph21 数据采集与接入网关相关 Go 阶段](../ph21-data-ingest-gateway/21-data-ingest-gateway.md)，这里只需理解"流 = 连续消息 + 消息边界 + 生命周期由业务决定"。

**四种模式怎么选（三问决策）**——模式不是越"高级"越好，按调用形态对号入座：

| 问题 | 答案指向 |
|------|---------|
| 一次请求要几个响应？ | 一个 → 一元；多个 → 服务端流 |
| 数据是一次性给完，还是持续产生？ | 一次性 → 一元/客户端流；持续 → 双向流（如实时遥测） |
| 两端要同时自由收发吗？ | 不要 → 单向流即可；要 → 双向流 |

工程上约八成接口是一元（查询/命令），流式出现在"推送/上报/订阅"三类场景——**先按一元设计，出现真实推送需求再上流**，与"先单体后微服务"是同一种克制。
...
**什么时候不该拆（monolith-first 判断清单）**——微服务是"组织与扩展问题"的解法，不是默认架构：

| 判据 | 保持单体的信号 | 该拆的信号 |
|------|--------------|-----------|
| 团队规模 | 一个团队、10 人内 | 多个团队按业务自治，独立发布节奏 |
| 数据关系 | 强一致、跨表事务频繁 | 数据边界清晰、各管各的库 |
| 调用形态 | 内部调用简单、变化慢 | 独立扩展需求明确（某服务流量独高） |
| 发布耦合 | 一起发没摩擦 | 某块发布频繁、被它拖累整体 |

真到要拆时，也常先走**模块化单体**（代码内按业务分模块、接口清晰，但先不拆进程）过渡——它拿到大部分"清晰边界"的好处，暂不付网络与一致性成本；等模块边界与扩展点验证清楚了，再按模块边界拆服务。**"拆出去"容易，"拆对"难**——本阶段的拆分原则与陷阱，配合 ph17 架构设计阶段的分层，就是为"拆对"做的准备。
...
**gRPC 状态码速查（跨服务可编程错误语义的最小集）**——`status.Code(err)` 返回的就是这张表的值，客户端按码决策：

| 状态码 | 含义 | 客户端动作 |
|--------|------|-----------|
| `OK` | 成功 | 正常处理 |
| `InvalidArgument` | 参数错误（业务侧校验失败） | 不重试，修参数 |
| `NotFound` / `AlreadyExists` | 资源不存在 / 已存在（示例 6 与练习 4 用它） | 不重试，走业务分支 |
| `DeadlineExceeded` | 超时（context deadline 到） | 可按策略重试一次（幂等接口） |
| `Unavailable` | 服务不可达（连接/健康问题） | 换实例重试 |
| `ResourceExhausted` | 限流/配额不足（3.5 的 429 语义） | 退避后重试，或降级 |
| `Internal` | 服务端内部错误 | 记录并上报，不重试 |

读表要点：**可重试的只有瞬时类（DeadlineExceeded/Unavailable/ResourceExhausted），业务错误（InvalidArgument/NotFound/AlreadyExists）永远别重试**——这比 net/rpc"错误只传字符串、靠手写判断"进了一大步，是"重试要考虑幂等"在协议层的支撑。
...
| 坑 | 症状 | 修法 |
|----|------|------|
| 忘了给 RPC 加超时 | 下游慢 → 调用方无限等 → 协程堆积 | 任何 RPC 调用套 `callWithTimeout` / `context.WithTimeout`（3.5） |
| 对非幂等接口盲目重试 | 超时重试后出现重复扣款/重复下单 | 写操作设计成幂等（幂等键去重），只对瞬时错误重试（3.5） |
| 把限流误当故障触发熔断 | 下游 429 时熔断器打开，误伤正常流量 | 限流是"忙"，不触发熔断；重试换实例即可（3.5 project 用例） |

三条都指向同一个教训：**可靠性原语要"配对"使用且各司其职**——超时防无限等、重试靠幂等兜底、熔断防雪崩、限流防打爆，缺一个或混用都会在故障演练中暴露。
...
**net/rpc 的现实定位**：官方已冻结，生产新项目不应选用；但它是理解 RPC 的完美教材（反射路由 + 文本 wire 让"RPC 是什么"全程透明），且在"教学/单机内网工具/极简服务"三类场景下依然够用。选型结论一句话：**用 gRPC 上生产，用 net/rpc 学原理**——本阶段两个都写了，正是为了让选型建立在"真懂差异"上，而不是跟风。
...
10. **单体优先、按需再拆**：微服务是组织与扩展问题的解法，不是默认架构——团队/数据/发布耦合不满足就先保持（模块化）单体，拆对比拆快重要（3.6）
