// 来源：ph21-data-ingest-gateway project/e2e_test.go
// 一句话说明：收官 e2e——httptest 起云端接入（api.Handler），真 HTTP 上行；故事：
// ① 采集器采集 3 个数据源×6 条(flushSize=3→6 批)正常上行落库；② 云端"断网"(503) 期间再采
// 一批数据入 spool；③ 恢复后补传 → 水位推进到 12、spool 归零、样本全入库；④ 批级
// 幂等重放不二次生效；⑤ 鉴权失败被拒；⑥ /healthz /version /metrics /sources 可用。
...
func TestRelayPlatformEndToEnd(t *testing.T) {
	core := platform.NewCore(map[string]string{"relay-001": "dev-secret-1"})
	srv := api.NewServer(core)
	var offline atomic.Bool
	cloud := httptest.NewServer(cloudHandler(srv, &offline))
	defer cloud.Close()

	up := collector.NewHTTPUplink(cloud.URL, "relay-001", "dev-secret-1")
	gw := collector.NewRelay("relay-001", 64, up, log.New(io.Discard, "", 0))
	col := collector.NewCollector(3, gw.HandleBatch)
	ctx := context.Background()

	// ① 在线：3 个数据源 × 6 条 = 6 批正常上行。
	for v := 1; v <= 3; v++ {
		sourceID := "src-00" + string(rune('0'+v))
		for s := 1; s <= 6; s++ {
			col.Add("relay-001", model.Sample{SourceID: sourceID, Seq: uint64(v*100 + s), Value: float64(30 + s*3)})
		}
	}
	...
	// ② 断网：再采 3 个数据源 × 3 条（每个数据源一批）→ 入 spool。
	offline.Store(true)
	for v := 1; v <= 3; v++ {
		sourceID := "src-00" + string(rune('0'+v))
		for s := 7; s <= 9; s++ {
	...
	if got := core.Store().Ack("relay-001"); got != 9 {
	...
	if _, err := up.SendBatch(ctx, model.BatchUpload{
		CollectorID: "relay-001", BatchID: 1,
		Samples: []model.Sample{{SourceID: "src-001", Seq: 101, Value: 30}},
	}); err != nil {
	...
	// ⑤ 鉴权失败：坏密钥 401。
	badUp := collector.NewHTTPUplink(cloud.URL, "relay-001", "wrong")
	if _, err := badUp.SendBatch(ctx, model.BatchUpload{CollectorID: "relay-001", BatchID: 99, Samples: []model.Sample{{SourceID: "src-001", Seq: 1, Value: 30}}}); err == nil {
		t.Error("坏密钥应被拒")
	}

	// ⑥ 端点：healthz/version/metrics/sources/latest。
	...
	if resp, err := http.Get(cloud.URL + "/api/v1/sources"); err != nil {
		t.Fatal(err)
	} else {
		var out struct {
		...
		if len(out.Sources) != 3 {
			t.Errorf("应 3 个数据源, got %v", out.Sources)
		}
	}
	valueResp, err := http.Get(cloud.URL + "/api/v1/sources/src-001/value")
	if err != nil {
		t.Fatal(err)
	}
	var valueOut struct {
	...
	// 采集器侧统计闭环：成功批 9（在线 6+补传 3 在 sent 计 9? 实际补传计入 Flush 未入 sent——
	// 校验 sent/queued/rejected 组合）。
	_, queued, rejected := gw.Stats()
	if queued != 3 || rejected != 0 {
		t.Errorf("采集器统计异常: queued=%d rejected=%d", queued, rejected)
	}
}
