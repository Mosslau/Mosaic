// 来源：ph21-data-ingest-gateway examples/ex05-relay-collector/collector.go
// 一句话说明：采集代理主体——聚合器出批 → 尝试上行；失败入 spool；后台 flush 按
// 序补传（云端 ack 水位推进，断点续传）。Uplink 是接口（HTTP/内存假云端都可注入），
// 采集器不关心上行通道是 MQTT 还是 HTTP（主文档 3.11/4.3）。
...
// NewRelay 建采集器（uplink 可先用失败桩，运行中断网入 spool）。
func NewRelay(id string, spoolCap int, up Uplink, l *log.Logger) *Relay {
	return &Relay{ID: id, uplink: up, spool: NewSpool(spoolCap), mu: make(chan struct{}, 1), log: l}
}

// HandleBatch 聚合器回调：先试上行，失败入 spool。
func (g *Relay) HandleBatch(b Batch) {
	...
	if serr := g.spool.Enqueue(b); serr != nil {
		g.log.Printf("采集器 %s: spool 满，批 %d 丢弃: %v", g.ID, b.BatchID, serr)
		return
	}
	g.queued.Add(1)
...
func (g *Relay) Flush(ctx context.Context) (int, bool) {
	...
	for _, b := range pending {
		...
		ack, err := g.uplink.SendBatch(ctx, b)
		if err != nil {
			g.log.Printf("采集器 %s: 补传批 %d 仍失败: %v", g.ID, b.BatchID, err)
...
func (g *Relay) Stats() (sent, queued int64) {
