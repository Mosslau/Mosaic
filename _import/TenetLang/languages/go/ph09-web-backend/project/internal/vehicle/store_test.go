...
// testVehicle 构造种子车辆
func testVehicle(id, status, secret string) Vehicle {
	return Vehicle{ID: id, Status: status, Secret: secret}
}

func newTestStore() Store {
	return NewMemoryStore(
		testVehicle("car-001", "online", "sec-1"),
		testVehicle("car-002", "offline", "sec-2"),
	)
}
...
func TestReport(t *testing.T) {
	...
	if _, err := s.Report("nope", 1, 1, 1); err != ErrNotFound {
		t.Errorf("上报不存在车辆错误 = %v, 期望 ErrNotFound", err)
	}
}
