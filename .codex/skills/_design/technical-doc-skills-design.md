...
3. 第一批先做核心 4 个，第二批根据真实写作需求建立；骨架可以成组先建，模板仍需逐个用真实素材验证。
4. 每个 skill 保持独立可用，不依赖一个未必会加载的父 skill。
5. 每个 skill 都要求先确认读者、目的、文档类型和事实来源。
6. 仓库已有模板时优先沿用仓库模板，没有模板时才使用 skill 自带结构。
...
第二批骨架已经建立，共 5 个 skill。每个 skill 仍应按使用反馈逐个完善，真实素材出现后至少做一次行为级冒烟测试，再决定是否增加或收窄章节。

## 目录结构

### 项目级安装形式

如果直接用于 MindSpring，目录位于：
...
正文应短于模板，模板放在 `references/` 中，避免每次写作都加载全部模板内容。
...
如果项目已有算法文档模板，例如 MindSpring 的 `algorithms/TEMPLATE.md`，应先按仓库模板组织。
...
- 让 `explaining-algorithms` 解释 MindSpring 中一个尚未成文的算法
- 让 `explaining-systems` 解剖 MindSpring 的 `engineering/` 一个项目
- 让 `documenting-language-features` 说明一个具体语言特性
- 让 `documenting-architecture-patterns` 说明一个具体架构模式
- 第二批 skill 在对应真实素材出现后再冒烟测试，例如真实服务契约、迁移 schema、已实现决策、可运行教程或真实故障处置
...
根据冒烟测试结果调整模板，避免只凭想象扩充章节。

## 实施顺序

1. 建立第一批 4 个 skill 目录。
2. 为 `explaining-systems` 建立 `system.md`、`framework.md`、`project.md`。
3. 为其余 skill 建立领域模板。
4. 执行文件级校验。
5. 对每个 skill 执行一次行为级冒烟测试。
6. 根据结果修正模板和描述。
7. 用户明确启动第二批时建立 5 个 skill 骨架。
8. 后续按真实写作需求对每个第二批 skill 执行冒烟测试并修正模板。
